...
Var* ff_basis(vfuncptr func, Var* arg)
{
	...
	if (ptr == NULL) {
		...
		return (NULL);
	} else if (!strcasecmp(ptr, "hadamard"))
		type = 1;
	else if (!strcasecmp(ptr, "haar"))
		type = 2;
	else {
	...
	if (!strcasecmp(format, "float")) {
		...
		for (j = 0; j < y1; j++) {
			for (i = 0; i < x2; i++) {
				...
				for (k = 0; k < x1; k++) {
					float_data[cpos(i, j, 0, v)] += extract_float(ob1, cpos(k, j, 0, ob1)) *
		...
		for (j = 0; j < y1; j++) {
			for (i = 0; i < x2; i++) {
				...
				for (k = 0; k < x1; k++) {
					double_data[cpos(i, j, 0, v)] += extract_double(ob1, cpos(k, j, 0, ob1)) *
