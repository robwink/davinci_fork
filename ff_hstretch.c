...
Var* ff_hstretch(vfuncptr func, Var* arg)
{
	...
	vx = newVal(BSQ, 1, hsize, 1, DV_FLOAT, x), vy = newVal(BSQ, 1, hsize, 1, DV_FLOAT, y),
	vcdf = newVal(BSQ, dnmax - dnmin + 1, 1, 1, DV_FLOAT, cdf);
	out  = hmod(hsize, y, dnmin, dnmax, cdf, npts);
	o    = newVal(BSQ, 1, hsize, 1, DV_FLOAT, out);

	if (obj != NULL) {
		Var* args = create_args(3, NULL, o, NULL, vx, NULL, obj, NULL, NULL);
	...
	if (debug) {
		...
		add_struct(s, "y", vy);
		add_struct(s, "o", o);
		add_struct(s, "cdf", vcdf), add_struct(s, "out", new);
		return (s);
	}

	if (new == NULL) {
		Var* args = create_args(3, NULL, vx, NULL, o, "axis", newString(strdup("x")), NULL, NULL);
