...
typedef struct _objectInfo {
	...
	int Qub_Pad; /*We can't pad the qub data block, because it doesn't get merged with the suffix
	               planes until later, so we need to save the pad amount, and do it AFTER the qube
	               is
	               written to disk */
...
static Var* ProcessIntoLabel(FILE* fp, int record_bytes, Var* v, int depth, size_t* label_ptr,
                             objectInfo* oi);
Var* write_PDS_Qube(Var* core, Var* side, Var* bottom, Var* back, FILE* fp);

static Var* do_key(KEYWORD* key);
static int readDataForObjects(Var* st, dataKey objSize[], int nObj, int load_suffix_data,
                              int continueOnError);
static Var* traverseObj(OBJDESC* top, Var* v, dataKey objSizeMap[], int* nObj, OBJDESC* pFileObj,
                        Var* pFileVar);
...
static const char* handledObjTypes[] = {
    GEN_OBJ_CLASS_HISTOGRAM, GEN_OBJ_CLASS_HISTORY, GEN_OBJ_CLASS_IMAGE, GEN_OBJ_CLASS_QUBE, GEN_OBJ_CLASS_TABLE
...
static const char* dontResolveTypes[] = {GEN_OBJ_CLASS_HISTOGRAM, GEN_OBJ_CLASS_IMAGE, GEN_OBJ_CLASS_QUBE};
		...
		if (num) {
			...
			*/
			...
			case ODL_REAL:
				...
				for (ii = 0; ii < num; ii++) f[ii] = strtod(stuff[ii], NULL);
...
static char* mod_name_if_necessary(char* name)
{
	char* new_name;
	if (name[0] != '^') return (name);

	new_name = (char*)calloc(strlen(&name[1]) + keyword_prefix_length, sizeof(char));
...
static Var* decodePtr(KEYWORD* kw)
{
	char* fname;
	long start_loc;
	short loc_type;
	...
	add_struct(v, "loc_type",
	           newString(strdup(loc_type == ODL_RECORD_LOCATION ? "record" : "byte")));
	return v;
}

static KEYWORD* traverseGroup(KEYWORD* kw, Var* v)
{
...
static size_t getObjSizeQube(Var* obj)
{
	Var* vCoreItems       = NULL;
	...
	if (vCoreItems != NULL && vCoreItemBytes != NULL) {
		...
		for (i = 0; i < n; i++) size[i] = extract_int(vCoreItems, i);
...
static size_t getObjSizeTable(Var* obj)
{
	Var* vRows     = NULL;
	Var* vRowBytes = NULL;
	size_t dsize   = 0;
...
static size_t getObjSizeHistogram(Var* obj)
{
	Var* vItems     = NULL;
	Var* vItemBytes = NULL;
	Var* vItemBits  = NULL;
	...
	if (find_struct(obj, "item_bytes", &vItemBytes) >= 0 || find_struct(obj, "item_bits", &vItemBits) >= 0) {
...
static size_t getObjSizeImage(Var* obj)
{
	Var* vLines      = NULL;
	Var* vSamples    = NULL;
...
static size_t getObjSizeHistory(Var* obj)
{
	Var* vBytes  = NULL;
	size_t dsize = 0;
...
static size_t getObjSize(Var* obj)
{
	Var* vObjType = NULL;
	size_t dsize  = 0;
...
static char* find_obj_name(Var* s)
{
	char* nameKw[] = {"core_name", "name"};
...
static Var* traverseObj(OBJDESC* top, Var* v, dataKey objSizeMap[], int* nObj, OBJDESC* pFileDesc,
                        Var* pFileVar)
{
	OBJDESC* op = NULL;
	KEYWORD* kw = NULL;
	...
	for (op = OdlNextObjDesc(top, 0, &scope); op != NULL; op = OdlNextObjDesc(op, 0, &scope)) {
		...
		objClass =
		    OdlGetObjDescClassName(op); /* returns "SPECTRAL_QUBE" from "OBJECT = SPECTRAL_QUBE" */
...
static int resolvePointers(char* fname, OBJDESC* top, Var* topVar, dataKey objSizeMap[], int n)
{
	char str[1024];
	int i;
...
static void ProcessGroupIntoLabel(FILE* fp, int record_bytes, Var* v, char* name)
{
	...
	get_struct_element(v, 0, &struct_name,
	                   &tmpvar); /*Get the OBJECT Tag and temporarily NULLify it */
...
static char* ProcessVarIntoString(Var* element, char* name)
{
	int flag = 0;
	int idx;
	int count;
	...
	if (flag) {
		if (((flag & 1) && (flag & 2)) || ((flag & 1) && (flag & 4)) || ((flag & 2) && (flag & 4))) {
	...
	case 0: count = GetSamples(V_SIZE(element), V_ORG(element)); break;

	case 1: count = GetLines(V_SIZE(element), V_ORG(element)); break;

	case 2: count = GetBands(V_SIZE(element), V_ORG(element)); break;
	}
	...
	for (idx = 0; idx < count; idx++) {
		...
		case INT: sprintf(tmp_string, "%d, ", extract_int(element, idx)); break;

		case FLOAT: sprintf(tmp_string, "%f, ", extract_float(element, idx)); break;

		case DOUBLE: sprintf(tmp_string, "%.10g, ", extract_double(element, idx)); break;
		}
		strcat(string, tmp_string);
	}
...
static void ProcessHistoryIntoString(Var* v, char** theString, int* ptr)
{
...
static void ProcessObjectIntoLabel(FILE* fp, int record_bytes, Var* v, char* name, objectInfo* oi)
{
	...
	if (!(strcasecmp("table", name))) { /*We're processing a table! */
		...
		count = get_struct_count(v);
		for (i = 0; i < count;
		     i++) { /*We could assume the DATA object is last, but that could lead to trouble */
	...
	*/
		...
		for (i = 0; i < count;
			...
			if (!(strcasecmp(struct_name, "data"))) { /*Found it! */
				...
				oi->obj_size[oi->count] = (V_SIZE(tmpvar)[0] * V_SIZE(tmpvar)[1] *
				...
				*/
				...
				oi->obj_size[oi->count] += bands * lines * oi->total_sample_count * 4 +
				                           bands * samples * oi->total_line_count * 4 +
				                           samples * lines * oi->total_band_count * 4 +
				                           bands * oi->total_sample_count * oi->total_line_count *
				                               4; /*Need to add 2nd corner info! */
		...
		/*We send in nulls because at this point....we SHOULD NOT find any labels or objects that we
		 * want! */
...
Var* ProcessIntoLabel(FILE* fp, int record_bytes, Var* v, int depth, size_t* label_ptrs, objectInfo* oi)
{
	int i;
	...
	while (count > 0) {
		...
		if (!(strcasecmp((name), "PDS_VERSION_ID"))) {
			...
			oi->obj_ptr[oi->ptr_count++] =
			    ftell(fp) + strlen((name + 7)) + 4; /* +4 for the space and ='s */
			...
			if ((!(strcasecmp("sample_suffix", name))) || (!(strcasecmp("line_suffix", name))) ||
			    (!(strcasecmp("band_suffix", name))) || (!(strcasecmp("suffix_data", name)))) {
				count--;
				continue;
			} else {
			...
			if (V_SIZE(data)[0] > 1 || V_SIZE(data)[1] > 1 || V_SIZE(data)[2] > 1) {
				...
				if (axis_count > 1) {
					parse_error(
					    "I'm not writing out anything larger than a vector (ie no planes or qubes "
					    "OTHER than data)");
					count--;
					continue;
				}
				...
				case INT: fprintf(fp, "%s%s = %d\r\n", inset, tmpname, V_INT(data)); break;

				case FLOAT: fprintf(fp, "%s%s = %12.6f\r\n", inset, tmpname, V_FLOAT(data)); break;
...
void Fix_Label(FILE* fp, int record_bytes, size_t* label_ptr, objectInfo* oi)
{
	int rem;
	char* pad;
	size_t size;
	...
	*/
	...
	for (i = 0; i < oi.count; i++) {
		if (oi.obj_type[i] != PDS_QUBE) {
			...
			if (write_PDS_Qube((Var*)oi.obj_data[i], oi.sample_suffix, oi.line_suffix,
...
static void pickFilename(char* outFname, const char* inFname)
{
	...
	if (stat(outFname, &sbuf) == 0) return;
...
static void rfBitField(int* j, char** Bufs, char* tmpbuf, FIELD** f, int ptr, int row, int* size, int num_items)
{
	...
	** now we're going to iterate through each bit-collumn and place it (but first we step one
	*forward
	** so that we are now on the first bit collumn
	*/
	i++;
	while (i < num_items && f[i]->bitfield != NULL) {
		memcpy(Bufs[i] + row * size[i], (tmpbuf + ptr),
		       size[i]); /*Copy the whole thing in we'll fix it later */
		i++;
	}
	*j = (i - 1);
}
...
static int rfTable(dataKey* objSize, Var* ob, LABEL* label)
{
	...
	free(tmpbuf);
	free(size);
	for (j = 0; j < num_items; j++) free(bufs[j]);
	free(bufs);

	return rc;
}

static double Scale(int size, void* ptr, FIELD* f)
{
	unsigned char* ucp;
	unsigned int* uip;
	unsigned short* usp;
	...
	case MSB_UNSIGNED_INTEGER:
		switch (f->size) {
		case 4: return (((double)uip[0]) * f->scale + f->offset);
		case 2: return (((double)usp[0]) * f->scale + f->offset);
		case 1: return (((double)ucp[0]) * f->scale + f->offset);
		}
		break;
...
static char* DoScale(int rows, FIELD* f, char* in)
{
	double* out;
	void* ptr;
	int count = 0;
...
static void Set_Col_Var(Var** Data, FIELD** f, LABEL* label, int* size, char** Bufs)
{
	...
	**/
	...
	for (j = 0; j < num_fields; j++) {
		...
		case LSB_INTEGER:
		case LSB_UNSIGNED_INTEGER:
		case LSB_BIT_FIELD: LSB(Bufs[j], label->nrows * dim, f[j]->size); break;
		case MSB_INTEGER:
		case MSB_UNSIGNED_INTEGER:
		case MSB_BIT_FIELD: MSB(Bufs[j], label->nrows * dim, f[j]->size); break;
		case IEEE_REAL: MSB(Bufs[j], label->nrows * dim, f[j]->size); break;
		case PC_REAL: LSB(Bufs[j], label->nrows * dim, f[j]->size); break;
		}
		...
		if (f[j]->scale != 1.0) {
			...
			*/
			...
			case PC_REAL:
				...
				switch (f[j]->size) {
				case 8: v = newVal(BSQ, dim, label->nrows, 1, DOUBLE, data); break;
				case 4: v = newVal(BSQ, dim, label->nrows, 1, FLOAT, data); break;
				}
				break;
...
static int GetInt(OBJDESC* ob, char* key, int* val)
{
	KEYWORD* kw;
...
static int loadFieldBinary(Var* v, LABEL* label, dataKey* data_key, int use_names, int grp_count,
                           int grp_start, char* parent_name)
{
	int i, j;
	int err = 0, warn = 0;
	int count       = 0;
	...
	for (i = 0; i < count; i++) {
		...
		if (!strcmp(name, FIELD_LOCATION)) {
			...
			if (grp_start > -1) {
				...
				for (j = 0; j < elem_cnt; j++) {
					...
					if (elem_name == NULL || t == NULL) {
						parse_error(
						    "Encountered a NULL davinci child node while loading a field binary "
						    "location record\n");
						err = 1;
						goto FINISH;
						...
						if (errno == ERANGE) {
							parse_error("Math error processing the field location value\n");
							err = 1;
							goto FINISH;
						}
						if (field->start < 0) {
			...
			if (!strcmp(data_type, UNSIGNED_BYTE) || !strcmp(data_type, UNSIGNED_MSB2) ||
			    !strcmp(data_type, UNSIGNED_MSB4)) {
				field->eformat = MSB_UNSIGNED_INTEGER;
				field->iformat = eformat_to_iformat(MSB_UNSIGNED_INTEGER);
			} else if (!strcmp(data_type, UNSIGNED_LSB2) || !strcmp(data_type, UNSIGNED_LSB4)) {
				field->eformat = LSB_UNSIGNED_INTEGER;
				field->iformat = eformat_to_iformat(LSB_UNSIGNED_INTEGER);
			} else if (!strcmp(data_type, SIGNED_LSB2) || !strcmp(data_type, SIGNED_LSB4)) {
				field->eformat = LSB_INTEGER;
				field->iformat = eformat_to_iformat(LSB_INTEGER);
			} else if (!strcmp(data_type, SIGNED_MSB2) || !strcmp(data_type, SIGNED_MSB4)) {
				field->eformat = MSB_INTEGER;
				field->iformat = eformat_to_iformat(MSB_INTEGER);
			} else if (!strcmp(data_type, IEEE_LSB_DOUBLE) || !strcmp(data_type, IEEE_LSB_SINGLE)) {
				field->eformat = PC_REAL;
				field->iformat = eformat_to_iformat(PC_REAL);
			} else if (!strcmp(data_type, IEEE_MSB_SINGLE) || !strcmp(data_type, IEEE_MSB_DOUBLE)) {
			...
			for (j = 0; j < elem_cnt; j++) {
				...
				if (elem_name == NULL || t == NULL) {
					parse_error(
					    "Encountered a NULL davinci child node while loading a field binary length "
					    "record\n");
					err = 1;
					goto FINISH;
				} else if (!strcmp(elem_name, VALUE)) {
					errno = 0;
					if (V_FORMAT(t) != INT) {
						parse_error("Warning!: Field length should be an Integer value\n");
						warn = 1;
						goto FINISH;
					}
					...
					field->size = V_INT(t);
					if (errno == ERANGE) {
						parse_error("Math error processing the field length value\n");
						err = 1;
						goto FINISH;
					}
			...
			if (V_FORMAT(s) == INT) {
				...
				parse_error("Warning: scaling factor should be a integer or a float value\n");
				warn = 1;
				goto FINISH;
			}
			...
			if (V_FORMAT(s) == INT) {
				...
				parse_error("Warning: offset should be a integer or a float value\n");
				warn = 1;
				goto FINISH;
			}
		} else if (field->name == NULL && !use_names && !strcmp(name, NAME_ATTRIBUTE)) {
...
static int loadGroupFieldBinary(Var* v, LABEL* label, dataKey* data_key, int use_names)
{
	...
	for (i = 0; i < count; i++) {
		...
		if (name == NULL || s == NULL) {
			parse_error(
			    "Encountered a NULL davinci child node while loading a group field binary "
			    "record\n");
			err = 1;
			goto FINISH;
		}
		if (!strcmp(name, GROUP_FIELD_BINARY)) {
			parse_error(
			    "Warning!: Davinci does not allow a group field binary element to contain another "
			    "group field binary element!\n");
			warn = 1;
			goto FINISH;
			...
			if (grp_field_cnt != 1) {
				parse_error(
				    "Warning!: Davinci only allows one field per each group field binary "
				    "element!\n");
				warn = 1;
				goto FINISH;
			}
			...
			if (num_groups != 0) {
				parse_error(
				    "ERROR: davinci does not currently allow groups within groups in a PDS4 "
				    "label\n");
				err = 1;
				goto FINISH;
			}
			...
			for (j = 0; j < elem_cnt; j++) {
				...
				if (!strcmp(elem_name, VALUE)) {
					...
					if (errno == ERANGE) {
						parse_error("Math error processing the GROUPS location value\n");
						err = 1;
						goto FINISH;
					}
			...
			for (j = 0; j < elem_cnt; j++) {
				...
				if (elem_name == NULL || t == NULL) {
					parse_error(
					    "Encountered a NULL davinci child node while loading a group length "
					    "record\n");
					err = 1;
					goto FINISH;
				}
				if (!strcmp(elem_name, VALUE)) {
					...
					grp_len = V_INT(t);
					if (errno == ERANGE) {
						parse_error("Math error processing the GROUPS length value\n");
						err = 1;
						goto FINISH;
					}
		...
		} else if (strstr(name, FIELD_BINARY) != NULL && label != NULL && data_key != NULL) {
			if (field_found) {
				parse_error(
				    "Warning!: Davinci only allows one field per each group field binary "
				    "element!\n");
			...
			if (!loadFieldBinary(s, label, data_key, use_names, num_fields, grp_loc, (char*)NULL)) {
				return 0;
			}
			field_found++;
			...
			if (elem_type != NULL && !strcmp(elem_type, FIELD_BINARY)) {
				...
				if (!loadFieldBinary(s, label, data_key, use_names, num_fields, grp_loc, name)) {
					return 0;
				}
				field_found++;
			}
...
static int loadRecordBinary(Var* v, LABEL* label, dataKey* data_key, int use_names)
{
	int i, j;
	int count       = 0;
	int elem_cnt    = 0;
	...
	for (i = 0; i < count; i++) {
		...
		if (name == NULL || s == NULL) {
			// error msg
			parse_error("Encountered a NULL davinci child node while loading a binary record\n");
			return 0;
		}
		if (!strcmp(name, FIELDS)) {
			...
			for (j = 0; j < elem_cnt; j++) {
				...
				if (!strcmp(name, VALUE)) {
					...
					if (errno == ERANGE) {
						// error msg couldn't read the file name
						parse_error("Math error processing the record length value\n");
						return 0;
					}
				}
			}
		} else if (strstr(name, GROUP_FIELD_BINARY) != NULL && label != NULL && data_key != NULL) {
			if (!loadGroupFieldBinary(s, label, data_key, use_names)) {
				return 0;
			}
		} else if (strstr(name, FIELD_BINARY) != NULL && label != NULL && data_key != NULL) {
			if (!loadFieldBinary(s, label, data_key, use_names, 1, -1, (char*)NULL)) {
				return 0;
			}
		} else if (find_struct(s, ELEMENT_TYPE, &u) >= 0) {
			elem_type = strdup(V_STRING(u));
...
static int loadBinaryTable(Var* v, LABEL* label, dataKey* data_key, int use_names, int get_data)
{
	int i, j;
	int count    = 0;
	int elem_cnt = 0;
	...
	for (i = 0; i < count; i++) {
		...
		if (name == NULL || s == NULL) {
			// error msg
			parse_error("Encountered a NULL davinci child node while loading a binary table\n");
			return 0;
		}
		if (!strcmp(name, OFFSET)) {
			...
			for (j = 0; j < elem_cnt; j++) {
				...
				if (!strcmp(name, VALUE)) {
					...
					if (errno == ERANGE) {
						// error msg couldn't read the file name
						parse_error("Error encountered while accessing table offset value\n");
						return 0;
					}
				}
			}
			...
			if ((label->name = strdup(V_STRING(s))) == NULL) {
				parse_error("Error encountered while accessing table description\n");
				return 0;
			}
...
static int loadFileInfo(Var* v, LABEL* label, dataKey* data_key)
{
	...
	for (i = 0; i < count; i++) {
		...
		if (name == NULL || s == NULL) {
			parse_error(
			    "Encountered a NULL davinci child node while searching for a PDS4 table file "
			    "name\n");
			return 0;
		}
		if (!strcmp(name, FILE_NAME)) {
			...
			if (data_key->FileName != NULL) {
				if ((tmp = (char*)malloc(strlen(fname) + strlen(data_key->FileName) + sizeof(char))) == NULL) {
					parse_error("Unable to allocate memory for table file name expansion\n");
					if (fname != NULL) {
						free(fname);
					}
					return 0;
...
static Var* xmlParseLabelFiles(Var* v, LABEL* label, dataKey* data_key, int use_names, int get_data,
                               char* file_path)
{
	char* elem_type  = NULL;
	char* node_name  = NULL;
	...
	for (i = 0; i < count; i++) {
		...
		if (name != NULL) {
			...
			if (node_name == NULL) {
				...
				parse_error("Failure trying to parse the PDS4 xml label files\n");
				err = 1;
				break;
			}
			// if this node is a file
			if (strstr(node_name, EXTERNAL_FILE) != NULL &&
			...
			} else if (strstr(node_name, TABLE_BINARY) != NULL && label != NULL && data_key != NULL) {
			...
			} else if (strstr(node_name, ELEMENT_TYPE) != NULL && label != NULL && data_key != NULL) {
				elem_type = strdup(V_STRING(s));
				if (elem_type != NULL && strstr(elem_type, TABLE_BINARY) != NULL) {
					free(elem_type);
					if (!loadBinaryTable(v, label, data_key, use_names, get_data)) {
						err = 1;
						break;
					}
				}
			} else if (use_names) // looking for the name of the potential binary table
			{
				if (V_TYPE(s) == ID_STRUCT) {
					if (find_struct(s, ELEMENT_TYPE, &u) >= 0) {
						elem_type = strdup(V_STRING(u));
						if (elem_type != NULL && !strcmp(elem_type, TABLE_BINARY)) {
							if (data_key == NULL) {
								// create a new LABEL and pass it in
								label = malloc(sizeof(LABEL));
								if (label == NULL) {
									parse_error(
									    "Memory allocation error while trying to parse the PDS4 "
									    "xml label files\n");
									err = 1;
									break;
								}
		...
		if (s != NULL) {
			if (V_TYPE(s) == ID_STRUCT) {
				if (!xmlParseLabelFiles(s, label, data_key, use_names, get_data, file_path)) {
					// error message;
					err = 1;
					break;
				}
			...
			parse_error("Encountered a NULL davinci child node while parsing an PDS4 XML label\n");
			err = 1;
		}
	}
	if (err) {
 ...
 * are met.
 */
Var* do_loadPDS4(vfuncptr func, char* filename, int use_names, int get_data)
{
	char* fname;
	...
	if (labelFile != NULL) {
		...
		labelFile =
		    xmlParseLabelFiles(labelFile, (LABEL*)NULL, (dataKey*)NULL, use_names, get_data, file_path);
	}
