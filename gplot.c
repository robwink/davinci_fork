...
XB_CALLBACK(SetCont)
{
	...
	if (i == 0) {
		sprintf(buf, "set cntrparam levels incr %s,%s,%s\n", GetButtonText(Cont_start),
		        GetButtonText(Cont_incr), GetButtonText(Cont_end));
	} else if (i == 1) {
...
XB_CALLBACK(SetView)
{
	...
	sprintf(buf, "set view %s,%s,%s,%s\n", GetButtonText(Surf_Xrot), GetButtonText(Surf_Zrot),
	        GetButtonText(Surf_Scale), GetButtonText(Surf_ZScale));
	SendToPlot(buf);
}
XB_CALLBACK(SetLog)
{
...
XB_CALLBACK(SetLabel)
{
	...
	if (B == Xlabel)
		sprintf(buf, "set xlabel \"%s\"\n", GetButtonText(B));
	else if (B == Ylabel)
		sprintf(buf, "set ylabel \"%s\"\n", GetButtonText(B));
	else if (B == Zlabel)
		sprintf(buf, "set zlabel \"%s\"\n", GetButtonText(B));
...
XB_CALLBACK(SetRange)
{
	...
	if (B == Xlow)
		sprintf(buf, "set xrange [%s:]\n", GetButtonText(B));
	else if (B == Xhigh)
		sprintf(buf, "set xrange [:%s]\n", GetButtonText(B));
	else if (B == Ylow)
		sprintf(buf, "set yrange [%s:]\n", GetButtonText(B));
	else if (B == Yhigh)
		sprintf(buf, "set yrange [:%s]\n", GetButtonText(B));
	else if (B == Zlow)
		sprintf(buf, "set zrange [%s:]\n", GetButtonText(B));
	else if (B == Zhigh)
		sprintf(buf, "set zrange [:%s]\n", GetButtonText(B));
	...
	for (i = 0; i < NSECTIONS; i++) {
		if (PB == PBClosed[i]) {
			...
			for (j = i + 1; j < NSECTIONS; j++) {
				XGetGeometry(disp, PBSection[j - 1]->window, &r, &x, &y, &w, &h, &bw, &d);
				XfMovePB(PBSection[j], x - bw, y + h + bw * 2 + SECTION_SPACE);
			}
			XGetGeometry(disp, PBSection[NSECTIONS - 1]->window, &r, &x, &y, &w, &h, &bw, &d);
...
CreateSection_Objects(parent) PButton parent;
{
}
CreateSection_Axis(parent) PButton parent;
{
	int w = parent->width;
	LButton lb;
	...
	XfActivateLB(XfCreateLB(d, win, 185, 20, 0, 0, DEFCOLORS, LEFTTEXT("Axis Label")), 0);
...
CreateSection_Surfaces(parent) PButton parent;
{
	int w = parent->width;
	LButton lb;
	Window win = parent->window;
	...
	Surf_Hidden3D =
	    XfCreateCB(d, win, 10, 60, 170, 20, DEFCOLORS, LEFTTEXT("Hidden Line Removal"), SetHidden3D);
	XfActivatePB(Surf_Hidden3D, 0);

	XfResizePB(parent, parent->width, 90);
}
CreateSection_Contours(parent) PButton parent;
{
	int w = parent->width;
	LButton lb;
	Window win = parent->window;
	...
	Cont_Surf = cb = XfCreateCB(d, win, 10, 20, 70, 20, DEFCOLORS, LEFTTEXT("Surface"), SetCntr);
	XfActivateCB(cb, 0);

	Cont_Base = cb = XfCreateCB(d, win, 10, 45, 60, 20, DEFCOLORS, LEFTTEXT("Base"), SetCntr);
	XfActivateCB(cb, 0);
	...
	Cont_rb = rb = XfCreateRB(d, win, 10, 90, 90, 20, DEFCOLORS, LT("Incremental"), SetCont, NULL);
...
CreateSection_Postscript(parent) PButton parent;
{
	int w = parent->width;
	LButton lb;
	Window win = parent->window;
	...
	PS_write = XfCreatePB(d, win, 210, 25, 50, 22, APPCOLORS, CENTERTEXT("Write"), DoPSWrite),
...
CreateSection_Quit(parent) PButton parent;
{
	int w = parent->width;
	LButton lb;
	Window win = parent->window;
	...
	XfSetDefaultFont(NULL, XfFont(NULL, "helvb12"));
	PB = XfCreatePB(d, win, 0, 0, w / 2, 25, APPCOLORS, CENTERTEXT("Really Quit?"), quit_cb);
	XfActivatePB(PB, 0);

	PB = XfCreatePB(d, win, w / 2, 0, w / 2, 25, APPCOLORS, CENTERTEXT("Don't Quit."), dont_quit);
	XfActivatePB(PB, 0);
...
XB_CALLBACK(get_text)
{
	...
	if (GetText(B, XfGetCallbackEvent(), buf, 256, 0) == -1) return;

	SetButtonText(B, buf);
	return;
}

Button MakeTextInput(d, win, x, y, w, h, fg, bg, text, align, func) Display* d;
Window win;
int x, y, w, h, fg, bg;
char* text;
CallBack func;
int align;
{
	...
	XfAddButtonVisual(B, 0, XfCreateVisual(B, align, ht, 0, 0, fg, bg, XfTextVisual, text, fs, align));
	XfAddButtonVisual(B, 1, XfCreateVisual(B, 0, 0, 0, 0, fg, bg, XfStippledVisual, "\252\125", 2, 2));
...
send_to_plot(str) char* str;
{
	write(writefd, str, strlen(str));
}

SendToPlot(str) char* str;
{
	send_to_plot(str);
	send_to_plot("replot\n");
}
...
get_xrange()
{
	...
	sprintf(buf, "%g", range[0]);
	SetButtonText(Xlow, buf);
	sprintf(buf, "%g", range[1]);
	SetButtonText(Xhigh, buf);
...
get_yrange()
{
	...
	sprintf(buf, "%g", range[0]);
	SetButtonText(Ylow, buf);
	sprintf(buf, "%g", range[1]);
	SetButtonText(Yhigh, buf);
...
get_zrange()
{
	...
	sprintf(buf, "%g", range[0]);
	SetButtonText(Zlow, buf);
	sprintf(buf, "%g", range[1]);
	SetButtonText(Zhigh, buf);
