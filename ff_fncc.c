...
void build_t_constants(double* t, int t_row, int t_col, double* t_avg, double* t_prime,
                       double* t2_prime, double* ignore);

double* fncc(double* r, int g_row, int g_col, int t_row, int t_col, int f_row, int f_col, double* f,
             double t_var, double t_avg, double* ignore, double* rf, double* rf2, int rec);

double f_sum(double* f, int f_row, int f_col, int x, int y, int t_cen_row, int t_cen_col,
             int row_even_mod, int col_even_mod, double* sots, int* ignore_count, double* ingnore);

void find_max_point(int col, int row, double* cc, int* p, double* val);

int build_running_sums(int trow, int tcol, int frow, int fcol, int grow, int gcol, double* f,
...
Var* ff_fncc(vfuncptr func, Var* arg)
{
	...
	if (rf == NULL) { /* by the query above, we know BOTH are NULL */
		...
		if (GetX(rf) != x || GetX(rf2) != x || GetY(rf) != y || GetY(rf2) != y) {
...
double* FFT_2D_Convolve(int trow, int tcol, int frow, int fcol, double* t, double* f)
{
	double* pad_t;
	double* pad_f;
	...
	*/
	...
	for (j = 0; j < col_pad; j++) {
		...
		for (i = 0; i < row_pad; i++) {
			//            f_fft[i*col_pad+j] = f_vector_col[i];// * t_vector_col[i];// +
			//            f_vector_col_im[i] * t_vector_col_im[i]*-1.;

			f_fft[i * col_pad + j] =
			    f_vector_col[i] * t_vector_col[i] + f_vector_col_im[i] * t_vector_col_im[i] * -1.;
			f_fft_im[i * col_pad + j] =
			    f_vector_col[i] * t_vector_col_im[i] + f_vector_col_im[i] * t_vector_col[i];
		}
	}
	printf("+");
	...
	*/
	...
	for (j = 0; j < col_pad; j++) {
		...
		for (i = 0; i < row_pad; i++) {
			...
			if (r[i * col_pad + j] < tol && r[i * col_pad + j] > -tol) r[i * col_pad + j] = 0.;
		}
	}
	printf("+\n");
...
double* pad_template(int trow, int tcol, int frow, int fcol, double* t)
{
	double* pad_t;
	int i, j;
...
double f_sum(double* f, int f_row, int f_col, int x, int y, int t_cen_row, int t_cen_col,
             int row_even_mod, int col_even_mod, double* sots, int* ignore_count, double* ignore)
{
	int i, j;
	double f_sum              = 0.0;
	...
	for (i = y - t_cen_row + row_even_mod; i <= (y + t_cen_row); i++) {
		for (j = x - t_cen_col + col_even_mod; j <= (x + t_cen_col); j++) {
			...
			else if (ig_flag && f[i * f_col + j] == ig)
				ic++;
...
double* fncc(double* r, int g_row, int g_col, int t_row, int t_col, int f_row, int f_col, double* f,
             double t_var, double t_avg, double* ignore, double* running_f, double* running_f2, int rec)
{
	...
	for (i = 0; i < g_row; i++) {
		...
		if (!((i + 1) % ten_p)) parse_error(".");

		for (j = 0; j < g_col; j++) {
			x = j - t_cen_col;

			fsum = f_sum(f, f_row, f_col, x, y, t_cen_row, t_cen_col, row_even_mod, col_even_mod,
			             &sots, &f_ic, ignore);
			...
			if (fabs(fsum - running_f[i * g_col + j]) > FSUM_error_abs_max)
				FSUM_error_abs_max = fabs(fsum - running_f[i * g_col + j]);

			//                fprintf(stderr,"%d,%d (row,col): FSUM = %16.12lf while running_f =
			//                %16.12lf (delta=%16.12lf)\n",
			...
			if (fabs(sots - running_f2[i * g_col + j]) > SOTS_error_abs_max)
				SOTS_error_abs_max = fabs(sots - running_f2[i * g_col + j]);
			//                fprintf(stderr,"%d,%d (row,col): SOTS = %16.12lf while running_f2 =
			//                %16.12lf (delta=%16.12lf)\n",
			//                i,j,sots,running_f2[i*g_col+j],fabs(sots-running_f2[i*g_col+j]));
			f2_error[i * g_col + j] = fabs(sots - running_f2[i * g_col + j]);
			//            }

			{ /* For debugging purposes, I've broken up the expression to make sure we don't have
			     dreck! */
				double p3;
	...
	if (FSUM_error_count) {
		fprintf(stderr, "FSUM: %d errors encountered out of %d calculations (--> %f%%)\n", FSUM_error_count,
		        (g_col * g_row), ((double)FSUM_error_count / (double)(g_col * g_row)) * 100.);
	...
	if (SOTS_error_count) {
		fprintf(stderr, "SOTS: %d errors encountered out of %d calculations (--> %f%%)\n", SOTS_error_count,
		        (g_col * g_row), ((double)SOTS_error_count / (double)(g_col * g_row)) * 100.);
                       ...
                       double* t_var, double* ignore)
{
	...
	** *t_var:        Pointer, set value to sum of each value in t, minus t_avg, squared (ie the
	*variance of t)
	...
	*/
	...
	for (v = 0; v < grow; v++) {
		for (u = 0; u < gcol; u++) {
			...
			/* SUM of f^2 */
			ss[v * gcol + u] =
			    (((u >= fcol) || (v >= frow)) ? (double)0.0 : (f[v * fcol + u] * f[v * fcol + u]));
	...
	*/
	...
	for (y = -(trow - 1); y < grow - (trow - 1); y++) {
		...
		for (x = -(tcol - 1); x < gcol - (tcol - 1); x++) {
			...
			running_f[v * gcol + u] +=
			    (((x < 1) || (y < 1)) ? (double)0.0 : s[(y - 1) * gcol + (x - 1)]);
			...
			running_f2[v * gcol + u] +=
			    (((x < 1) || (y < 1)) ? (double)0.0 : ss[(y - 1) * gcol + (x - 1)]);
		}
	}
...
double* TwoD_Convolve(int trow, int tcol, int frow, int fcol, double* t, double* f, double* ignore)
{
	/*
				...
				for (j = x - t_cen_col + col_even_mod; j <= (x + t_cen_col); j++) {
					...
					*********************************************************************/
					...
					if (i < 0 || i >= f_row || /* skip invalid locations in f */
					    j < 0 ||
					    j >= f_col) /* We can also add an INGORE value for f here as well */
						;
					else if (ig_flag && (t[ti * t_col + tj] == ig || f[i * f_col + j] == ig))
						;
					else
...
Var* ff_fncc_write_mat(vfuncptr func, Var* arg)
{
	...
	if (obj == NULL) {
		parse_error("%s: missing obj.", func->name);
		return (newInt(0));
	}
...
static double* fft_convolve_real(double* a, int arows, int acols, double* b, int brows, int bcols,
                                 int* rows, int* cols);
...
Var* ff_fncc_cmplx_mul(vfuncptr func, Var* arg)
{
	...
	for (k = 0; k < 2; k++) {
		for (j = 0; j < rows; j++) {
			for (i = 0; i < cols; i++) {
				output[k * rows * cols + j * cols + i] =
				    k == 0 ? result[j * cols + i].re : result[j * cols + i].im;
			}
		}
	}
	free(result);
