...
Var* cut(Var* img, int x, int y, int z, int w, int h, int d)
{
	...
	MakeSize(dim, GetX(img), GetY(img), GetZ(img));
	...
	for (i = 0; i < 3; i++) {
		...
		*/
		if (start[i] < -dim[i]) {
			parse_error("Start %s is too negative (%d).  Image's %s is %d", axis[i], start[i],
			            axis[i], dim[i]);
			return (NULL);
		}
		if (start[i] > dim[i]) {
			parse_error("Start %s is too large (%d).  Image's %s is %d", axis[i], start[i], axis[i],
			            dim[i]);
			return (NULL);
		}
		/*
		...
		*/
		...
		if (size[i] < 0) {
			if (size[i] <= -dim[i]) {
				parse_error("%s size is too small (%d).  Image's %s is %d", axis[i], size[i],
				            axis[i], dim[i]);
				return (NULL);
			}
		...
		if (start[i] + size[i] - 1 > dim[i]) {
			parse_error("%s size is too large (%d).  Image's %s is %d", axis[i],
			            start[i] + size[i] - 1, axis[i], dim[i]);
			return (NULL);
		}
	}
	for (i = 0; i < 3; i++) {
...
/* scale an image */
Var* scale(Var* img, int xscale, int yscale)
{
	return (NULL);
}

/* add padding to an image */
Var* pad(Var* img, Var* color, int left, int right, int top, int bottom, int front, int back)
{
...
Var* paste(Var* src, Var* dst, int x, int y, int z)
{
