...
Var* cubic_interp(Var* v0, Var* v1, Var* v2, char* type, float ignore)
{
	...
	free(xp);
	free(yp);
	return (newVal(V_ORG(v2), V_SIZE(v2)[0], V_SIZE(v2)[1], V_SIZE(v2)[2], FLOAT, out));
}
...
*****************************************************************************/
{
	...
	for (i = 1; i < n; i++) yd[i][2] = (y[i] - y[i - 1]) / (x[i] - x[i - 1]);

	/* compute weights and store in yd[.][3] */
	for (i = 1; i < n - 1; i++) yd[i][3] = fabs(yd[i + 1][2] - yd[i][2]);
	yd[0][3]                             = yd[1][3];
	yd[n - 1][3]                         = yd[n - 2][3];
...
*****************************************************************************/
{
	...
	for (i = 1; i < n - 1; i++) {
		...
		yd[i][1] =
		    0.5 * (y[i] - y[i - 1]) / (x[i] - x[i - 1]) + 0.5 * (y[i + 1] - y[i]) / (x[i + 1] - x[i]);
	}
