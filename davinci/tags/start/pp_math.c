...
{\
	...
	for (i = 0 ; i < dsize ; i++) {\
		...
		switch(op) {\
			...
			case ID_MOD: idata[i] = _S_(fmod((double)v1, (double)v2)); break;\
			case ID_POW: idata[i] = _S_(pow((double)v1, (double)v2)); break;\
		}\
	}\
}
...
/** size was extracted as x,y,z.  put it back appropriately **/
	...
	if (is_relop(op)) {
		switch (in_format) {
			...
			case SHORT:
			case INT:
				DO_MATH_LOOP(int, u_char, extract_int, (int));
				break;
			case FLOAT:
				DO_MATH_LOOP(float, u_char, extract_float, (float));
				break;
			case DOUBLE:
				DO_MATH_LOOP(double, u_char, extract_double, (double));
				break;
		}
	} else {
		/**
