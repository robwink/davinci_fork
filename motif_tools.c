...
#else
    MtCreateXmStringFromStringArray(text) String* text;
#endif
{
	int line;
...
#else
    MtCvtStringToPixel(w, color) Widget w;
String color;
#endif
{
	XrmValue from, to;
...
#else
    MtCvtPixelToString(w, pixel) Widget w;
Pixel pixel;
#endif
{
	char string[50];
...
#else
    MtCvtFontStringToFontList(parent, font_str, type) Widget parent;
char* font_str;
XmFontType type;
#endif
{
	...
	entry = XmFontListEntryLoad(XtDisplay(parent), font_str, type, XmFONTLIST_DEFAULT_TAG);
	if (NULL == entry) {
		char buf[128];

		sprintf(buf, "Can't create font list entry using '%s'.\nUsing default font list.\n", font_str);
		XtWarning(buf);
		vendor_shell = parent;
		...
		if (NULL == vendor_shell) {
			sprintf(buf, "Cannot find vendor shell to get the default fontlist. Exiting...\n");
			XtError(buf);
			exit(1);
		}
...
#else
    MtCreatePopupShell(parent, create_proc, title, data) Widget parent;
...
#endif
{
	...
	/* Create the dialog */
	dialog = XtVaCreatePopupShell(title, xmDialogShellWidgetClass, MtGetShell(parent), XmNtransient, False,
	                              XmNallowShellResize, True, XmNdeleteResponse, XmDO_NOTHING, NULL);
	page             = create_proc(dialog, title, data);
	WM_DELETE_WINDOW = XmInternAtom(XtDisplay(dialog), "WM_DELETE_WINDOW", False);
	XmAddWMProtocolCallback(dialog, WM_DELETE_WINDOW, (XtCallbackProc)MtCloseDialogCB, (XtPointer)0);
...
#if __STDC__
	va_start(pvar, dummy);
	XtVaSetValues(dummy, XmNwidth, max, XmNalignment, XmALIGNMENT_END, XmNrecomputeSize, FALSE, NULL);
#else
	va_start(pvar);
#endif
	while ((cur_label = (Widget)va_arg(pvar, Widget))) {
		XtVaSetValues(cur_label, XmNwidth, max, XmNalignment, XmALIGNMENT_END, XmNrecomputeSize,
		              FALSE, NULL);
	}
	va_end(pvar);
}
...
#else
    MtExpandFilename(filename, new_filename) String filename,
    new_filename;
#endif
{
	int bufflen;
...
#else
    MtStringCompare(in, test, num_char) String in,
    test;
int num_char;
#endif
{
...
#else
    MtGetToken(string, delim, token) char **string,
    delim, **token;
#endif
{
	String tmp;
...
#else
    MtFloatToString(value) float value;
#endif
{
	static char buf[20];
...
#else
    MtBooleanToString(value) Boolean value;
#endif
{
	if (value) {
		return ("True");
...
#else
    MtCreateName(name, type) String name,
    type;
#endif
{
	static char buf[100];
...
#else
    MtFindChildren(parent, children, normal, popup) Widget parent,
    **children;
...
MtGetShell(Widget w)
#else
    MtGetShell(w) Widget w;
#endif
{
	Widget parent = w;
	while (!XtIsVendorShell(parent) && !XmIsDialogShell(parent) && (w = XtParent(parent))) {
		parent = w;
	}
	return (parent);
}
...
#else
    MtGetChild(parent, name) Widget parent;
String name;
#endif
{
	int i, num_children;
...
#else
    GetChildByUserData(parent, userData) Widget parent;
XtPointer userData;
#endif
{
	...
	XtVaGetValues(parent, XmNchildren, &children, XmNnumChildren, &num_children, NULL);
	for (i = 0; children && i < num_children; i++) {
...
#else
    OKcallback(w, client_data, cbs) Widget w;
...
#else
    fieldRadioBoxCB(w, client_data) Widget w;
XtPointer client_data;
#endif
{
	int i;
...
#else
    MtCloseFileSelectorCB(w) Widget w;
#endif
{
	XtUnmanageChild(w);
}
...
#else
    MtCreateActionArea(parent, buttons) Widget parent;
ButtonItem* buttons;
#endif
{
	int num_buttons = 0, i;
	...
	action_area = XtVaCreateWidget("action_area", xmFormWidgetClass, parent, XmNskipAdjust, True,
	                               XmNfractionBase, TIGHTNESS * num_buttons - 1, NULL);

	for (i = 0; i < num_buttons; i++) {
		button = XtVaCreateManagedWidget(
		    MtCreateName(buttons[i].name, "button"), xmPushButtonWidgetClass, action_area,
		    XmNleftAttachment, i ? XmATTACH_POSITION : XmATTACH_FORM, XmNleftPosition,
		    TIGHTNESS * i, XmNtopAttachment, XmATTACH_FORM, XmNbottomAttachment, XmATTACH_FORM,
		    XmNrightAttachment, i != num_buttons - 1 ? XmATTACH_POSITION : XmATTACH_FORM,
		    XmNrightPosition, TIGHTNESS * i + TIGHTNESS - 1, XmNshowAsDefault,
		    i == 0 || buttons[i].set, XmNdefaultButtonShadowThickness, 1, NULL);

		/* Set label if not provided in resource file */
		MtSetLabel(button, buttons[i].label);

		if (buttons[i].callback) {
			XtAddCallback(button, XmNactivateCallback, buttons[i].callback, buttons[i].data);
		}
		/* If "Cancel" button, set popdown callback if not supplied */
		else if (strcmp(buttons[i].name, CANCEL) == 0) {
			XtAddCallback(button, XmNactivateCallback, (XtCallbackProc)MtCloseDialogCB, NULL);
		}
		if (XtClass(parent) == xmPanedWindowWidgetClass && i == 0) {
			...
			height += 2 * h;
			XtVaSetValues(action_area, XmNdefaultButton, button, XmNpaneMaximum, height,
			              XmNpaneMinimum, height, NULL);
		}
	}
...
#else
    MtGetValue(widget, name, type, value) Widget widget;
String name;
int type;
XtArgVal* value;
#endif
...
#else
    MtSetSensitive(w, name, value) Widget w;
String name;
Boolean value;
#endif
{
...
#else
    MtSetValue(widget, name, type, value) Widget widget;
String name;
int type;
XtArgVal value;
#endif
...
#else
    MtGetWidgetValue(widget, datatype, value) Widget widget;
int datatype;
XtArgVal* value;
#endif
{
	...
	if (widget_class == xmTextWidgetClass || widget_class == xmTextFieldWidgetClass) {
		...
		XtVaGetValues(widget, XmNlabelString, &xm_string, NULL);
		...
		switch (datatype) {
		case TOOL_INT: *value = (XtArgVal)iValue; break;
		case TOOL_FLOAT:
			assert(sizeof(XtArgVal) >= sizeof(float));
			fValue = iValue;
		...
		XtVaGetValues(widget, XmNselectedItemCount, &count, XmNselectedItems, &items, NULL);
		if (count == 0) {
			return (False);
		}
...
#else
    MtSetWidgetValue(widget, datatype, value) Widget widget;
int datatype;
XtArgVal value;
#endif
{
	...
	if (!widget || !XtIsWidget(widget)) return (False);
...
#define CONVERT                                              \
	...
	if (widget_class == xmListWidgetClass) {
		...
		if (XtClass(button) == xmToggleButtonWidgetClass) {
			XtVaGetValues(XtParent(widget), XmNradioBehavior, &isRadio, NULL);
			if (isRadio) {
				XtVaSetValues(button, XmNset, True, XmNmenuHistory, button, NULL);
			} else {
			...
			if (cascade) XtVaSetValues(cascade, XmNmenuHistory, button, NULL);
		}
	}
		...
		if ((Boolean)value && XtClass(XtParent(widget)) == xmRowColumnWidgetClass)
...
#else
    counter_verify_text_value(counter, value) Counter* counter;
int value;
#endif
{
	char new_val[32];
...
#else
    counter_activate_cb(w, counter, call_data) Widget w;
Counter* counter;
XtPointer call_data;
#endif
{
...
#else
    counter_up_cb(w, counter, call_data) Widget w;
Counter* counter;
XtPointer call_data;
#endif
{
...
#else
    counter_down_cb(w, counter, call_data) Widget w;
Counter* counter;
XtPointer call_data;
#endif
{
...
Widget
#ifndef _NO_PROTO
MtBuildMenu(Widget parent, int menu_type, String name, String title, MenuItem* items)
#else
    MtBuildMenu(parent, menu_type, name, title, items) Widget parent;
int menu_type;
char *name, *title;
MenuItem* items;
#endif
{
	...
	if (menu_type == XmMENU_PULLDOWN) {
		cascade = XtCreateManagedWidget(MtCreateName(name, "cascade"), xmCascadeButtonWidgetClass,
		                                parent, args, 2);
	} else if (menu_type == XmMENU_OPTION) {
		cascade = XmCreateOptionMenu(parent, MtCreateName(name, "cascade"), args, 2);
	}
	XmStringFree(string);
	...
	for (i = 0; items[i].name; i++) {
		if (items[i].subitems) {
			if (menu_type != XmMENU_OPTION) {
				widget = MtBuildMenu(menu, XmMENU_PULLDOWN, items[i].name, items[i].label, items[i].subitems);
			} else {
				XtWarning("Option menu items can't have submenus");
				continue;
			}
		} else {
			WidgetClass widget_class = items[i].w_class ? *items[i].w_class : xmPushButtonWidgetClass;
			widget = XtVaCreateManagedWidget(MtCreateName(items[i].name, "button"), widget_class,
			                                 menu, XmNuserData, items[i].data, NULL);
			if (widget_class == xmToggleButtonWidgetClass) {
				XtVaSetValues(widget, XmNindicatorType,
				              items[i].toggle_button_type == XmRADIOBUTTON ? XmONE_OF_MANY : XmN_OF_MANY,
		...
		if (items[i].callback) {
			XtAddCallback(widget, items[i].w_class == &xmToggleButtonWidgetClass ? XmNvalueChangedCallback
			                                                                     : XmNactivateCallback,
	...
	if (had_toggle) {
		XtVaSetValues(menu, XmNradioBehavior, had_toggle, XmNradioAlwaysOne, True, NULL);
	}
...
Widget
#ifndef _NO_PROTO
MtBuildRadioBox(Widget parent, String name, String title, ButtonItem* buttons, int orientation, int button_type)
#else
    MtBuildRadioBox(parent, name, title, buttons, orientation, button_type) Widget parent;
char *name, *title;
ButtonItem* buttons;
...
#endif
{
	...
	XtVaSetValues(menu, XmNradioBehavior, button_type == XmRADIOBUTTON, XmNorientation, orientation, NULL);

	for (i = 0; buttons[i].name; i++) {
		button = XtVaCreateManagedWidget(MtCreateName(buttons[i].name, "button"),
		                                 xmToggleButtonWidgetClass, menu, XmNuserData,
		                                 buttons[i].data, XmNset, buttons[i].set, NULL);
		MtSetLabel(button, buttons[i].label);
		if (i == 0) button0 = button;
		if (buttons[i].set) {
			XtVaSetValues(menu, XmNmenuHistory, button, NULL);
			if (!set) set = True;
		}
		if (buttons[i].callback) {
			XtAddCallback(button, XmNvalueChangedCallback, buttons[i].callback, buttons[i].data);
		}
	}
...
#else
    MtCreateLabeledBox(parent, name, label, box) Widget parent,
    *box;
char *name, *label;
#endif
{
	...
	*box    = XtVaCreateManagedWidget(MtCreateName(name, "box"), xmFrameWidgetClass, parent, NULL);
	label_w = XtVaCreateManagedWidget(MtCreateName(name, "label"), xmLabelWidgetClass, *box,
	                                  XmNchildType, XmFRAME_TITLE_CHILD, NULL);
	MtSetLabel(label_w, label);
	container = XtVaCreateManagedWidget("containedRC", xmRowColumnWidgetClass, *box, XmNorientation,
	                                    XmVERTICAL, XmNnumColumns, 1, NULL);
exit:
	return (container);
}
...
Widget
#ifndef _NO_PROTO
MtCreateLabeledContainer(Widget parent, String name, String label, WidgetClass containerWidgetClass,
                         Widget* frame)
#else
    MtCreateLabeledContainer(parent, name, label, containerWidgetClass, frame) Widget parent,
    *frame;
char *name, *label;
...
#endif
{
	...
	frame_w = XtVaCreateManagedWidget(MtCreateName(name, "frame"), xmFrameWidgetClass, parent, NULL);
	label_w = XtVaCreateManagedWidget(MtCreateName(name, "label"), xmLabelWidgetClass, frame_w,
	                                  XmNchildType, XmFRAME_TITLE_CHILD, NULL);
	MtSetLabel(label_w, label);
	container =
	    XtVaCreateManagedWidget(MtCreateName(name, "container"), containerWidgetClass, frame_w, NULL);
exit:
	if (frame) *frame = frame_w;
	return (container);
}
 ...
 */
Widget MtCreateShell(Widget parent, String name, String title, WorkAreaFunc work_area_func,
                     MenuItem* menu_items, ButtonItem* action_area_items, Boolean display_msg_area)
{
	int i;
	...
	if (name) {
		base = XtVaCreatePopupShell(MtCreateName(name, "shell"), vendorShellWidgetClass,
		                            MtGetShell(parent), XmNtitle, title, XmNtraversalOn, True,
	...
	XmAddWMProtocolCallback(base, WM_DELETE_WINDOW, (XtCallbackProc)MtCloseDialogCB, (XtPointer)0);

	/* Use a Motif XmMainWindow widget to handle window layout */
	mainWindow = XtCreateManagedWidget("mainWindow", xmMainWindowWidgetClass, base, NULL, 0);
	...
	if (menu_items) {
		...
		for (i = 0; menu_items[i].name; i++) {
			if (menu_items[i].subitems != NULL) {
				button = MtBuildMenu(menubar, XmMENU_PULLDOWN, menu_items[i].name,
				                     menu_items[i].label, menu_items[i].subitems);
			} else {
				widget_class =
				    (menu_items[i].w_class ? *menu_items[i].w_class : xmCascadeButtonWidgetClass);
				button = XtVaCreateManagedWidget(MtCreateName(menu_items[i].name, "button"), widget_class,
				                                 menubar, XmNuserData, menu_items[i].data, NULL);
				MtSetLabel(button, menu_items[i].label);
				if (menu_items[i].callback) {
					XtAddCallback(button, ((menu_items[i].w_class == &xmPushButtonWidgetClass)
	...
	separator = XtVaCreateManagedWidget("separator", xmSeparatorGadgetClass, form, NULL);

	/* Create message area and attach to bottom of window */
	frame = XtCreateWidget("frame", xmFrameWidgetClass, form, NULL, 0);

	if (!empty_label) {
		empty_label = XmStringCreateSimple(" ");
	}
	message = XtVaCreateWidget("message_label", xmLabelWidgetClass, frame, XmNalignment,
	...
	XtVaSetValues(workArea, XmNtopAttachment, XmATTACH_FORM, XmNleftAttachment, XmATTACH_FORM,
	              XmNrightAttachment, XmATTACH_FORM, XmNbottomAttachment, XmATTACH_WIDGET,
	              XmNbottomWidget, separator, NULL);

	XtVaSetValues(separator, XmNleftAttachment, XmATTACH_FORM, XmNrightAttachment, XmATTACH_FORM, NULL);

	if (action_area_items) {
		XtVaSetValues(separator, XmNbottomAttachment, XmATTACH_WIDGET, XmNbottomWidget, action_area, NULL);

		XtVaSetValues(action_area, XmNleftAttachment, XmATTACH_FORM, XmNrightAttachment, XmATTACH_FORM,
		              XmNbottomAttachment, XmATTACH_WIDGET, XmNbottomWidget, frame, NULL);
	} else {
		XtVaSetValues(separator, XmNbottomAttachment, XmATTACH_FORM, NULL);
	}
	XtVaSetValues(frame, XmNshadowType, XmSHADOW_IN, XmNleftAttachment, XmATTACH_FORM,
...
#else
    MtCreateTextField(parent, fielditem) Widget parent;
FieldItem* fielditem;
#endif
{
	...
	container_rc =
	    XtVaCreateManagedWidget(MtCreateName(fielditem->name, "container"), xmRowColumnWidgetClass,
	                            parent, XmNorientation, XmHORIZONTAL, XmNnumColumns, 2,
	                            XmNisAligned, TRUE, XmNentryAlignment, XmALIGNMENT_END, NULL);
	label_w = XtVaCreateManagedWidget(MtCreateName(fielditem->name, "label"), xmLabelWidgetClass,
	                                  container_rc, XmNalignment, XmALIGNMENT_END, NULL);

	MtSetLabel(label_w, fielditem->label);
	field = XtVaCreateManagedWidget(MtCreateName(fielditem->name, "field"), xmTextWidgetClass,
	                                container_rc, XmNcolumns, fielditem->length, XmNsensitive,
	                                fielditem->sensitive, NULL);
	if (fielditem->value) XtVaSetValues(field, XmNvalue, fielditem->value, NULL);
	if (fielditem->callback)
		XtAddCallback(field, XmNactivateCallback, fielditem->callback, fielditem->data);
	else
		XtAddCallback(field, XmNactivateCallback, (XtCallbackProc)OKcallback, NULL);
...
#else
    MtCreateSliderField(parent, slideritem) Widget parent;
...
#endif
{
	...
	*/
	...
	container_rc =
	    XtVaCreateManagedWidget(MtCreateName(slideritem->name, "container"), xmRowColumnWidgetClass,
	                            parent, XmNorientation, XmHORIZONTAL, XmNnumColumns, 2,
	                            XmNisAligned, TRUE, XmNentryAlignment, XmALIGNMENT_END, NULL);
	label_w = XtVaCreateManagedWidget(MtCreateName(slideritem->name, "label"), xmLabelWidgetClass,
	                                  container_rc, XmNalignment, XmALIGNMENT_END, NULL);

	MtSetLabel(label_w, slideritem->label);
	slider = XtVaCreateManagedWidget(MtCreateName(slideritem->name, "slider"), xmScaleWidgetClass,
	                                 container_rc, XmNorientation, XmHORIZONTAL, XmNminimum,
	                                 slideritem->min, XmNmaximum, slideritem->max, XmNvalue,
	                                 slideritem->value, NULL);
	if (slideritem->drag_callback) {
		XtAddCallback(slider, XmNdragCallback, slideritem->drag_callback, slideritem->data);
	}
	if (slideritem->value_callback) {
		XtAddCallback(slider, XmNvalueChangedCallback, slideritem->value_callback, slideritem->data);
	}
...
#else
    MtCreateCounter(parent, fielditem, min, max) Widget parent;
FieldItem* fielditem;
int min, max;
#endif
{
	...
	XtAddCallback(text, XmNlosePrimaryCallback, (XtCallbackProc)counter_activate_cb, counter);
	XtAddCallback(text, XmNactivateCallback, (XtCallbackProc)counter_activate_cb, counter);
	XtAddCallback(text, XmNlosingFocusCallback, (XtCallbackProc)counter_activate_cb, counter);

	rc = XtVaCreateManagedWidget(MtCreateName(fielditem->name, "arrowrc"), xmRowColumnWidgetClass,
	                             container, XmNmarginHeight, 0, XmNspacing, 0, NULL);

	up_arrow = XtVaCreateManagedWidget(MtCreateName(fielditem->name, "up"),
	                                   xmArrowButtonWidgetClass, rc, XmNarrowDirection, XmARROW_UP,
	                                   XmNshadowThickness, 0, XmNhighlightThickness, 0, NULL);
	XtAddCallback(up_arrow, XmNactivateCallback, (XtCallbackProc)counter_up_cb, counter);

	down_arrow = XtVaCreateManagedWidget(MtCreateName(fielditem->name, "down"),
	                                     xmArrowButtonWidgetClass, rc, XmNarrowDirection, XmARROW_DOWN,
	                                     XmNshadowThickness, 0, XmNhighlightThickness, 0, NULL);
	XtAddCallback(down_arrow, XmNactivateCallback, (XtCallbackProc)counter_down_cb, counter);
...
Widget
#ifndef _NO_PROTO
MtCreateFieldBox(Widget parent, String name, String title, FieldItem* fields, int orientation)
#else
    MtCreateFieldBox(parent, name, title, fields, orientation) Widget parent;
String name, title;
FieldItem* fields;
int orientation;
#endif
{
	...
	rc = XtVaCreateManagedWidget(MtCreateName(name, "box"), xmRowColumnWidgetClass, frame,
	                             XmNorientation, orientation, XmNnumColumns, 1, NULL);
	if (!title) box = rc;
	if (XtClass(parent) == xmFormWidgetClass)
		XtVaSetValues(rc, XmNleftAttachment, XmATTACH_FORM, XmNrightAttachment, XmATTACH_FORM, NULL);

	/* Add label and text field to rc */
	for (i = 0; fields[i].name; i++) container_rc = MtCreateTextField(rc, &fields[i]);
	...
	for (i = 0; fields[i].name; i++) {
		label = MtGetChild(rc, MtCreateName(fields[i].name, "label"));
		XtVaSetValues(label, XmNwidth, max, XmNrecomputeSize, False, NULL);
	}
	return (box);
}
...
Widget
#ifndef _NO_PROTO
MtCreateFieldRadioBox(Widget parent, String name, String title, FieldItem* fields, int orientation)
#else
    MtCreateFieldRadioBox(parent, name, title, fields, orientation) Widget parent;
String name, title;
FieldItem* fields;
int orientation;
#endif
{
	...
	/* Add label and text field to rc */
	for (i = 0; fields[i].name; i++) {
		container_rc = XtVaCreateManagedWidget(fields[i].name, xmRowColumnWidgetClass, rc, XmNorientation,
		                                       XmHORIZONTAL, XmNnumColumns, 2, XmNisAligned, TRUE,
		                                       XmNentryAlignment, XmALIGNMENT_END, NULL);
		button = XtVaCreateManagedWidget(MtCreateName(fields[i].name, "button"),
		                                 xmToggleButtonWidgetClass, container_rc, XmNalignment,
		                                 XmALIGNMENT_BEGINNING, XmNset, fields[i].sensitive,
		                                 XmNindicatorType, XmONE_OF_MANY, XmNuserData, i + 1, NULL);
		MtSetLabel(button, fields[i].label);
		XtAddCallback(button, XmNvalueChangedCallback, (XtCallbackProc)fieldRadioBoxCB, (XtPointer)rc);
		field = XtVaCreateManagedWidget(MtCreateName(fields[i].name, "field"), xmTextWidgetClass,
		                                container_rc, XmNcolumns, Max(fields[i].length, 1),
		                                XmNsensitive, fields[i].sensitive, NULL);
		if (fields[i].length == 0) XtVaSetValues(field, XmNmappedWhenManaged, False, NULL);
		if (fields[i].value) XtVaSetValues(field, XmNvalue, fields[i].value, NULL);
		if (fields[i].callback)
			XtAddCallback(field, XmNactivateCallback, (XtCallbackProc)fields[i].callback, fields[i].data);
		else
			XtAddCallback(field, XmNactivateCallback, (XtCallbackProc)OKcallback, NULL);
	}
...
Widget
#ifndef _NO_PROTO
MtCreateCascadeButtonBox(Widget parent, String name, String title, CascadeButtonItem* buttons,
                         int orientation, int num_columns)
#else
    MtCreateCascadeButtonBox(parent, name, title, buttons, orientation, num_columns) Widget parent;
String name;
String title;
...
#endif
{
	...
	rc = XtVaCreateManagedWidget(MtCreateName(name, "box"), xmRowColumnWidgetClass, frame,
	                             XmNorientation, orientation, XmNnumColumns, num_columns,
	                             XmNisAligned, TRUE, XmNentryAlignment, XmALIGNMENT_END, NULL);
	...
	for (i = 0; buttons[i].name; i++) {
		if ((i % num_columns) == 0) {
			column = XtVaCreateManagedWidget("column", xmRowColumnWidgetClass, rc, XmNorientation,
			                                 (orientation == XmVERTICAL ? XmHORIZONTAL : XmVERTICAL),
			                                 XmNnumColumns, 2 * num_columns, XmNisAligned, TRUE,
			                                 XmNentryAlignment, XmALIGNMENT_END, NULL);
		}

		if (buttons[i].menu) {
			xmstring = XmStringCreateSimple(buttons[i].label);
			XtVaCreateManagedWidget(MtCreateName(buttons[i].name, "label"), xmLabelWidgetClass,
			                        column, XmNlabelString, xmstring, NULL);
			XmStringFree(xmstring);
			MtBuildMenu(column, XmMENU_OPTION, buttons[i].name, NULL, buttons[i].menu);
		} else {
			xmstring = XmStringCreateSimple((buttons[i].label ? buttons[i].label : ""));
			XtVaCreateManagedWidget(MtCreateName(buttons[i].name, "label"), xmLabelWidgetClass,
			                        column, XmNlabelString, xmstring, NULL);
			XmStringFree(xmstring);
			xmstring = XmStringCreateSimple("");
			XtVaCreateManagedWidget(MtCreateName(buttons[i].name, "cascade"), xmLabelWidgetClass,
			                        column, XmNlabelString, xmstring, NULL);
			XmStringFree(xmstring);
		}
	}
...
#else
    MtDisplayModal(shell, action_buttons) Widget shell;
...
#endif
{
	...
	for (i = 0, nbuttons = 0; action_buttons[i].name; i++) {
		if ((button = MtGetChild(shell, MtCreateName(action_buttons[i].name, "button")))) {
			windows           = (Window*)XtRealloc((char*)windows, (nbuttons + 1) * sizeof(Window));
...
#else
    MtDisplayFileSelector(parent, title, data) Widget parent;
char* title;
...
#endif
{
	...
	if (!dialog) {
		...
		cancel_button = XmFileSelectionBoxGetChild(dialog, XmDIALOG_CANCEL_BUTTON);
	}
	...
	XtVaSetValues(dialog, XmNpattern, mask, XmNhelpLabelString, help_label, XmNcancelLabelString,
	              cancel_label, XmNokLabelString, ok_label, NULL);
	XmStringFree(mask);
	...
	if (data->help_callback) {
		...
		XtAddCallback(dialog, XmNhelpCallback, data->help_callback, data->help_data);
	} else {
		XtUnmanageChild(help_button);
	}
	if (data->cancel_callback) {
		XtAddCallback(dialog, XmNcancelCallback, data->cancel_callback, data->cancel_data);
	} else {
		XtAddCallback(dialog, XmNcancelCallback, (XtCallbackProc)MtCloseFileSelectorCB, NULL);
	}
...
#else
    MtDisplayMessageDialog(parent, title, message, type, buttons) Widget parent;
...
#endif
{
	...
	XtVaSetValues(dialog, XmNdialogType, type, XmNdialogTitle, xm_title, XmNmessageString, xm_message, NULL);
	...
	if (!cancelCB)
		XtAddCallback(XmMessageBoxGetChild(dialog, XmDIALOG_CANCEL_BUTTON), XmNactivateCallback,
...
#else
    MtCreatePromptDialog(parent) Widget parent;
#endif
{
	Arg args[2];
...
MtDisplayPromptDialog(Widget dialog, String title, String message,
                      ButtonItem *buttons)
#else
    MtDisplayPromptDialog(dialog, title, message, buttons) Widget dialog;
String title, message;
ButtonItem* buttons;
#endif
{
	...
	Widget button;
	Boolean cancelCB      = False;
	Atom WM_DELETE_WINDOW = XmInternAtom(XtDisplay(dialog), "WM_DELETE_WINDOW", False);
	int i;

	XtVaSetValues(dialog, XmNdialogTitle, xm_title, XmNselectionLabelString, xm_message, NULL);
	XmStringFree(xm_message);
	XmStringFree(xm_title);
	...
	for (i = 0; buttons[i].name; i++) {
		...
		XtVaSetValues(button, XmNlabelString, label, XmNset, buttons[i].set, NULL);
		XmStringFree(label);
		if (buttons[i].callback) {
			if (!strcmp(buttons[i].name, OK))
				XtAddCallback(dialog, XmNokCallback, buttons[i].callback, buttons[i].data);
			else if (!strcmp(buttons[i].name, CANCEL)) {
				cancelCB = True;
				XtAddCallback(dialog, XmNcancelCallback, buttons[i].callback, buttons[i].data);
				XmAddWMProtocolCallback(XtParent(dialog), WM_DELETE_WINDOW,
				                        (XtCallbackProc)buttons[i].callback, buttons[i].data);
			} else if (!strcmp(buttons[i].name, HELP))
				XtAddCallback(dialog, XmNhelpCallback, buttons[i].callback, buttons[i].data);
			XtManageChild(button);
		} else if (strcmp(buttons[i].name, CANCEL))
			XtUnmanageChild(button);
	}
	if (!cancelCB) {
		XtAddCallback(dialog, XmNcancelCallback, (XtCallbackProc)MtCloseDialogCB, NULL);
		XmAddWMProtocolCallback(XtParent(dialog), WM_DELETE_WINDOW, (XtCallbackProc)MtCloseDialogCB, NULL);
	}
	XtManageChild(dialog);
	XtPopup(XtParent(dialog), XtGrabNone);
}
...
#else
    MtMonoDisplay(display) Display* display;
#endif
{
	/*
...
#else
    MtCvtXmStringToString(string) XmString string;
#endif
{
	...
	if ((length = XmStringLength(string)) == 0) return (XtNewString(""));

	if (!XmStringInitContext(&context, string)) return (XtNewString(""));

	status = XmStringGetNextSegment(context, &substring, &charset, &direction, &separator);
	if (!status) return (XtNewString(""));
...
static XmString
#ifdef _NO_PROTO
    create_simple_xmstring(str) String str;
#else
...
XmString
#ifdef _NO_PROTO
    MtCreateXmString(string) String string;
#else
...
#endif
{
	...
	if ((p = strstr(str, "\\"))) {
		do {
			length = strlen(p);
			if (length >= 4 && isdigit(p[1]) && isdigit(p[2]) && isdigit(p[3])) {
				strncpy(buf, &p[1], 3);
				buf[3] = '\0';
				sscanf(buf, "%o", &value);
				p[0] = value;
...
static Boolean
#ifndef _NO_PROTO
CvtStringToXmString(Display* dpy, XrmValuePtr args, Cardinal* num_args, XrmValuePtr from,
                    XrmValuePtr to, XtPointer* data)
#else
    CvtStringToXmString(dpy, args, num_args, from, to, data) Display* dpy;
XrmValuePtr args;
Cardinal* num_args;
XrmValuePtr from, to;
XtPointer* data;
...
#endif
{
	XtSetTypeConverter(XmRString, XmRXmString, CvtStringToXmString, NULL, 0, XtCacheNone, NULL);
}
...
#endif
{
	...
	if (!XtIsRealized(w) || !XDoesBackingStore(XtScreen(w))) return;
	num_child          = MtFindChildren(w, &child, True, False);
	attr.backing_store = WhenMapped;
	XChangeWindowAttributes(XtDisplay(w), XtWindow(w), CWBackingStore, &attr);
	for (i = 0; i < num_child; i++)
		XChangeWindowAttributes(XtDisplay(w), XtWindow(child[i]), CWBackingStore, &attr);
	if (child) XTFREE(child);
}
