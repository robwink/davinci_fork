...
#define XTMALLOC(type, size) ((size) > 0 ? (type*)XtMalloc((size) * sizeof(type)) : NULL);
#define XTREALLOC(ptr, type, size) \
	(ptr ? (type*)XtRealloc((char*)ptr, (size) * sizeof(type)) : (type*)XtMalloc((size) * sizeof(type)))
...
typedef enum { TOOL_INT = 1, TOOL_FLOAT, TOOL_STRING, TOOL_BOOLEAN } Datatype;

typedef Widget (*MtCreatePopupChildProc)(
#ifndef _NO_PROTO
    Widget parent, String label, XtPointer data
#endif
    );
...
extern Widget MtBuildMenu(Widget parent, int menu_type, String name, String title, MenuItem* items);
extern Widget MtBuildRadioBox(Widget parent, String name, String title, ButtonItem* buttons,
                              int orientation, int button_type);
...
extern Widget MtCreateCascadeButtonBox(Widget parent, String name, String title,
                                       CascadeButtonItem* buttons, int orientation, int num_columns);
extern CounterItem* MtCreateCounter(Widget parent, FieldItem* fielditem, int min, int max);
extern Widget MtCreateFieldBox(Widget parent, String name, String title, FieldItem* fields, int orientation);
extern Widget MtCreateFieldRadioBox(Widget parent, String name, String title, FieldItem* fields,
                                    int orientation);
extern Widget MtCreateLabeledBox(Widget parent, String name, String label, Widget* box);
extern Widget MtCreateLabeledContainer(Widget parent, String name, String label,
                                       WidgetClass containerWidgetClass, Widget* frame);
extern String MtCreateName(String name, String type);
extern Widget MtCreateOptionMenuBox(Widget, String, String, CascadeButtonItem*, int, int);
extern void MtCreatePopupShell(Widget parent, MtCreatePopupChildProc create_proc, String title,
                               XtPointer data);
extern Widget MtCreatePromptDialog(Widget parent);
extern Widget MtCreateShell(Widget parent, String name, String title, WorkAreaFunc work_area_func,
...
extern void MtDisplayFileSelector(Widget parent, String title, FileSelectionData* data);
extern void MtDisplayMessageDialog(Widget parent, String title, String message, int type,
                                   ButtonItem* buttons);
extern String MtDisplayModal(Widget shell, ButtonItem* action_buttons);
extern void MtDisplayPromptDialog(Widget dialog, String title, String message, ButtonItem* buttons);
extern void MtExpandFilename(String filename, String new_filename);
extern int MtFindChildren(Widget parent, Widget** children, Boolean normal, Boolean popup);
...
extern Boolean MtGetValue(Widget widget, String name, int type, XtArgVal* value);
extern Boolean MtGetWidgetValue(Widget widget, int datatype, XtArgVal* value);
...
extern Boolean MtSetValue(Widget w, String name, int type, XtArgVal value);
extern Boolean MtSetWidgetValue(Widget widget, int datatype, XtArgVal value);
