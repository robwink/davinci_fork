...
Var* pp_set_var(Var* id, Var* range, Var* exp)
{
	Var *v, *e;
	Range *r, rout;
...
Var* pp_inc_var(Var* id, Var* range, Var* exp)
{
	Var *v, *e;
	Range *r, rout;
	...
	for (k = 0; k < size[2]; k++) {
		for (j = 0; j < size[1]; j++) {
			for (i = 0; i < size[0]; i++) {
				...
				d = cpos(i * r->step[0] + r->lo[0], j * r->step[1] + r->lo[1],
				...
				*/
				...
				if (V_FORMAT(dst) == V_FORMAT(src)) {
					...
					case SHORT:
						...
						break;
					case INT: ((int*)V_DATA(dst))[d]     = saturate_int(extract_int(src, s)); break;
					case FLOAT: ((float*)V_DATA(dst))[d] = extract_float(src, s); break;
					case DOUBLE: ((double*)V_DATA(dst))[d] = extract_double(src, s); break;
					}
				}
			}
		}
...
Var* pp_func(Var* function, Var* arglist)
{
	Var* p;
	char* ptr;
 ...
 **/
Var* pp_mk_arglist(Var* arglist, Var* arg)
{
	/*
	*/
	if (arglist == NULL) {
 ...
 */
Var* pp_keyword_to_arg(Var* keyword, Var* ex)
{
...
int compare_strings(char* s1, int op, char* s2)
{
	int i, k = 0;
	i = strcmp(s1, s2);
	switch (op) {
...
Var* pp_math_strings(Var* exp1, int op, Var* exp2)
{
	Var *s, *e, *text;
	...
	*/
	...
	if (V_TYPE(exp1) == ID_STRING && V_TYPE(exp2) == ID_STRING) {
		...
		*/
		...
		for (i = 0; i < rows; i++) {
			data[i] = compare_strings(V_TEXT(exp1).text[i], op, V_TEXT(exp2).text[i]);
		}
...
Var* pp_set_where(Var* id, Var* where, Var* exp)
{
	Var* v;
	...
	if (V_TYPE(id) == ID_TEXT && (V_TYPE(exp) == ID_STRING || V_TYPE(exp) == ID_TEXT) &&
	    V_TYPE(where) == ID_VAL)
	...
	if (V_DSIZE(exp) == 1) {
		...
		for (i = 0; i < dsize; i++) {
			...
			if (extract_int(where, j)) {
				k = rpos(i, id, exp);
				switch (format) {
				case BYTE: ((u_char*)V_DATA(id))[i]   = extract_int(exp, k); break;
				case SHORT: ((short*)V_DATA(id))[i]   = extract_int(exp, k); break;
				case INT: ((int*)V_DATA(id))[i]       = extract_int(exp, k); break;
				case FLOAT: ((float*)V_DATA(id))[i]   = extract_double(exp, k); break;
				case DOUBLE: ((double*)V_DATA(id))[i] = extract_double(exp, k); break;
				}
			}
		}
	}
