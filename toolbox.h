...
#define RingThatBell(znumz)                                      \
	{                                                            \
		int ziz;                                                 \
		for (ziz = 0; ziz < znumz; ++ziz) printf("%c", (char)7); \
	}
...
#define NewString(zstrz, zsizez)                                             \
	{                                                                        \
		...
		if (zstrz == NULL) SayGoodbye() else* zstrz = '\0';                  \
	}
...
#define AppendString(zstrz, znewz)                                            \
	{                                                                         \
		if (znewz != NULL) {                                                  \
			...
			if (zstrz == NULL) SayGoodbye() else strcat(zstrz, znewz);        \
		}                                                                     \
	}
...
#define StripTrailing(zstrz, zstripz)                                                          \
	{                                                                                          \
		char* zcz;                                                                             \
		for (zcz = LastChar(zstrz); ((zcz >= zstrz) && (*zcz == zstripz)); --zcz) *zcz = '\0'; \
	}
...
#define NewStringList(zstrz, zlistz)                                                           \
	{                                                                                          \
		...
		zlistz->next                        = NULL;                                            \
		if (zlistz == NULL)                                                                    \
			SayGoodbye() if (zstrz == NULL)                                                    \
			    NewString(zlistz->text, 1) else CopyString(zlistz->text, zstrz)                \
	}

#define AddStringToList(zstrz, zlistz)                                 \
	{                                                                  \
		TB_STRING_LIST *znz = NULL, *znnz = NULL;                      \
		if (zlistz == NULL) NewStringList(zstrz, zlistz) else          \
			{                                                          \
