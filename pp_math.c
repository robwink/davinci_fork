...
Var* pp_math(Var* a, int op, Var* b)
{
	...
	**/
	...
	if (is_relop(op)) {
		...
		case BYTE:
		case SHORT:
		case INT: DO_RELOP_LOOP(int, u_char, extract_int, (int)); break;
		case FLOAT: DO_RELOP_LOOP(float, u_char, extract_float, (float)); break;
		case DOUBLE: DO_RELOP_LOOP(double, u_char, extract_double, (double)); break;
		}
	} else if (op == ID_LSHIFT || op == ID_RSHIFT) {
		if (in_format <= INT) {
			switch (V_FORMAT(a)) {
			case BYTE: DO_SHIFT_LOOP(int, u_char, extract_int, saturate_byte); break;
			case SHORT: DO_SHIFT_LOOP(int, short, extract_int, saturate_short); break;
			case INT: DO_SHIFT_LOOP(int, int, extract_int, (int)); break;
			}
		} else {
		...
		**/
		switch (in_format) {
		case BYTE: DO_MATH_LOOP(int, u_char, extract_int, saturate_byte); break;
		case SHORT: DO_MATH_LOOP(int, short, extract_int, saturate_short); break;
		case INT: DO_MATH_LOOP(int, int, extract_int, (int)); break;
		case FLOAT: DO_MATH_LOOP(float, float, extract_float, (float)); break;
		case DOUBLE: DO_MATH_LOOP(double, double, extract_double, (double)); break;
		}
		if (dzero) {
...
int pp_compare(Var* a, Var* b)
{
	...
	*/
	...
	case BYTE:
	case SHORT:
	case INT: DO_CMP_LOOP(int, extract_int); break;
	case FLOAT: DO_CMP_LOOP(float, extract_float); break;
	case DOUBLE: DO_CMP_LOOP(double, extract_double); break;
	}
	return (1);
}
...
{
	switch (V_FORMAT(v)) {
	case BYTE: return ((int)((u_char*)V_DATA(v))[i]);
	case SHORT: return ((int)((short*)V_DATA(v))[i]);
	case INT: return ((int)((int*)V_DATA(v))[i]);
	case FLOAT: return ((int)((float*)V_DATA(v))[i]);
	case DOUBLE: return ((int)((double*)V_DATA(v))[i]);
	}
	return (0);
}
...
{
	switch (V_FORMAT(v)) {
	case BYTE: return ((float)((u_char*)V_DATA(v))[i]);
	case SHORT: return ((float)((short*)V_DATA(v))[i]);
	case INT: return ((float)((int*)V_DATA(v))[i]);
	case FLOAT: return ((float)((float*)V_DATA(v))[i]);
	case DOUBLE: return ((float)((double*)V_DATA(v))[i]);
	}
	return (0);
}
...
{
	switch (V_FORMAT(v)) {
	case BYTE: return (((u_char*)V_DATA(v))[i]);
	case SHORT: return (((short*)V_DATA(v))[i]);
	case INT: return (((int*)V_DATA(v))[i]);
	case FLOAT: return (((float*)V_DATA(v))[i]);
	case DOUBLE: return (((double*)V_DATA(v))[i]);
	}
	return (0);
}
