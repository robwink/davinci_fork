...
int mrqfit(double**, struct data_order, int, int, int, double*, int, int, double**, double*, ifptr, int);
int alpha_beta_chisq(double**, struct data_order, int, int, double*, int, int, double**, double*,
                     double*, ifptr);
...
Var* ff_fit(vfuncptr func, Var* arg)
{
	...
	const char* fits[] = {"gauss",  "gaussc",     "gaussl", "ngauss", "lorenz", "2lorenz",
	                      "linear", "linear_chi", "quad",   "cube",   "poly",   "nexp",
	                      "xyquad", "xygauss",    "sincos", NULL};
	...
	/* NOTE(rswinkle) Not having this is precarious, already fixed one segfault
	 * maybe we should actually create x instead of having lin_fit/dfit etc. gen
	 * data on the fly checking for NULL every iteration?
	** if (x == NULL) {
	...
	*/
	...
	if (x && (V_DSIZE(x) != V_DSIZE(y))) {
		sprintf(error_buf, "%s: X axis for data [%ld points] not same size as Y axis [%ld points]",
	...
	if (!(strcmp(ftype, "linear"))) {
		s = lin_fit(x, y, V_DSIZE(y), plot, ignore);
		return s;
	}
...
int dfit(Var* x, Var* y, Var* ip, const char* fname, int iter, double** op, int* nparam, int plot,
         int verbose, double ignore)
{
	FILE* fp;
	char buf[256];
	char buf2[256], *p;
	...
	*nparam = ma;
	...
	for (i = 0; i < ma; i++) {
		if (ip && i < V_DSIZE(ip))
			a[i] = extract_double(ip, i);
		else
			a[i] = 0.0;
	}
	first_guess(a, (char*)fname, verbose);

	if (ip && V_DSIZE(ip) == ma + 2) {
		alamda = extract_double(ip, ma + 1);
	} else {
		/* start alamda small */
		alamda = 1e-3;
	}
...
void first_guess(double* a, char* fname, int verbose)
{
	int i;
	...
	if (verbose) fprintf(stderr, "guess: %f %f %f\n", a[0], a[1], a[2]);
}

void gd(Var* x, Var* y, const char* fname, double ignore)
{
...
int fit(int itmax, int verbose)
{
	/* nonzero returns 1 if all sigma's are non-zero */
	if (nonzero(data[order.sig], ndata)) {
		failed = mrqfit(data, order, num_indep, ndata, itmax, a, ma, mfit, covar, &chisq, func, verbose);
	} else {
		failed = 1;
...
int mrqfit(double** data, struct data_order order, int num_indep, int ndata, int itmax, double* a,
           int ma, int mfit, double** covar, double* chisq, ifptr func, int verbose)
{
	...
	if (alpha_beta_chisq(data, order, num_indep, ndata, a, ma, mfit, alpha, beta, chisq, func)) {
	...
	while (i <= itmax) {
		...
		if (alpha_beta_chisq(data, order, num_indep, ndata, atry, ma, mfit, alpha, beta, chisq, func)) {
			free_dmatrix(alpha, ma, ma);
			free(atry);
			free_dmatrix(alpha_try, ma, ma);
			free(beta);
		...
		if (*chisq >= ochisq) {
			...
			*chisq = ochisq;
			...
			for (j = 0; j < mfit; j++) a[j] = atry[j];
		}

		if (verbose) {
			for (j = 0; j < ma; j++) printf("a%d= %g\t", j, a[j]);
			printf("\nchisqr = %g", *chisq);
			printf("\nalamda = %g", alamda);
			printf("\n");
		} else if (verbose)
...
int alpha_beta_chisq(double** data, struct data_order order, int num_indep, int ndata, double* a,
                     int ma, int mfit, double** alpha, double* beta, double* chisq, ifptr funcs)
{
	int i, j, k;
	double *x, *y, *sig, *xsig;
	...
	*chisq = 0;
	...
	for (i = 0; i < ndata; i++) {
		...
		for (j = 0; j < mfit; j++) {
			wt = dyda[j] / sig2i;
			for (k = 0; k <= j; k++) alpha[j][k] += wt * dyda[k];
			beta[j] += dy * wt;
		}
		*chisq += dy * dy / sig2i;
	}
...
void solve_for_da(double** alpha, double** covar, double* beta, double* da, int mfit)
{
	int i, j;
	double** mbeta;
...
/* This file contains a bunch of functions for allocating */
/* vectors and matrices.  */

// TODO(rswinkle) replace with functions in matrix.c
// rename matric.c ff_matrix.c and extract common funcs to
// ff_matrix.h
...
void free_imatrix(int** m, int nr, int nc)
{
	int i;
	for (i = nr - 1; i >= 0; i--) free(m[i]);
	free(m);
...
int fgauss(double* x, double* a, double* y, double* dyda, int na, int dyda_flag, int* fita,
           int* dydx_flag, double* dydx, double ydat);
int fgaussc(double* x, double* a, double* y, double* dyda, int na, int dyda_flag, int* fita,
            int* dydx_flag, double* dydx, double ydat);
int fgaussl(double* x, double* a, double* y, double* dyda, int na, int dyda_flag, int* fita,
            int* dydx_flag, double* dydx, double ydat);
int fgaussn(double* x, double* a, double* y, double* dyda, int na, int dyda_flag, int* fita,
            int* dydx_flag, double* dydx, double ydat);
int florenz(double* x, double* a, double* y, double* dyda, int na, int dyda_flag, int* fita,
            int* dydx_flag, double* dydx, double ydat);
int florenz2(double* x, double* a, double* y, double* dyda, int na, int dyda_flag, int* fita,
             int* dydx_flag, double* dydx, double ydat);
int fline(double* x, double* a, double* y, double* dyda, int na, int dyda_flag, int* fita,
          int* dydx_flag, double* dydx, double ydat);
int fquad();
int fpoly(double* x, double* a, double* y, double* dyda, int na, int dyda_flag, int* fita,
          int* dydx_flag, double* dydx, double ydat);
int fexpn(double* x, double* a, double* y, double* dyda, int na, int dyda_flag, int* fita,
          int* dydx_flag, double* dydx, double ydat);
int fxyquad(double* x, double* a, double* y, double* dyda, int na, int dyda_flag, int* fita,
            int* dydx_flag, double* dydx, double ydat);
int fxygauss(double* x, double* a, double* y, double* dyda, int na, int dyda_flag, int* fita,
             int* dydx_flag, double* dydx, double ydat);
int fconic();
int fsincos(double* x, double* a, double* y, double* dyda, int na, int dyda_flag, int* fita,
            int* dydx_flag, double* dydx, double ydat);
...
struct routine {
    ...
    {"2lorenz", 1, 0, &florenz2, 7,
     "f(x) = a1*a2/(4*(x-a0)**2 + a1) + a4*a5/(4*(x-a3)**2 + a4) + a6"},
...
/* pointer corresponding to the function specified  */

ifptr getfcnptr(const char* name, int* num_indep, int* linflag, int* na, char* comment)
{
	int i = 0;
	while (fcn[i].na != 0) {
