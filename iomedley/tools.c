...
void* list_copy(LIST* list, void* ptr, int size)
{
	void* p = NULL;
...
{
	if (list->ptr != NULL) free(list->ptr);
	free(list);
}

/* USE WITH CAUTION */
void list_kill(LIST* list)
{
	while (list->number) {
		(list->number)--;
...
void* list_ptr(LIST* list, int whence)
{
	if (whence >= list->number) return (NULL);
	if (whence >= 0) {
 ...
 **/
void list_replace(LIST* l1, LIST* l2)
{
	free(l1->ptr);
	l1->ptr    = l2->ptr;
	l1->number = l2->number;
...
void add_queue_head(QUEUE** l, void* data)
{
	QUEUE* new;
	new       = new_queue(data);
	new->next = *l;
...
void add_queue_tail(QUEUE** l, void* data)
{
	if (*l)
...
void remove_from_queue(QUEUE** l, void* data)
{
	QUEUE* t;
	if ((*l)->data == data) {
		t    = (*l);
		...
		while ((*l)->next && (*l)->next->data != data) *l = (*l)->next;
		if ((*l)->next->data == data) {
			t          = (*l)->next;
			(*l)->next = (*l)->next->next;
			free(t);
...
QUEUE* next_in_queue(QUEUE* l)
{
	return (l->next);
}

void* queue_data(QUEUE* l)
{
	return (l->data);
}
