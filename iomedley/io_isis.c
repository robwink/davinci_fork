...
int iom_GetISISHeader(FILE* fp, char* filename, struct iom_iheader* h,
                      ...
                      )
{
	...
	ob = (OBJDESC*)OdlParseLabelFile(filename, err_file, ODL_EXPAND_STRUCTURE, iom_VERBOSITY <= 3);
	...
	if (qube == NULL) qube = OdlFindObjDesc(ob, (char*)"SPECTRAL_QUBE", NULL, 0, 0, 0);
	...
	if (qube != NULL) {
		...
		if (iom_is_ok2print_details()) {
			...
			fprintf(stderr, "%dx%dx%d %s %s\t\n", iom_GetSamples(size, org), iom_GetLines(size, org),
			        iom_GetBands(size, org), iom_Org2Str(org), iom_EFormat2Str(format));
		...
		*/
		...
		if (key == NULL) key = OdlFindKwd(ob, "^SPECTRAL_QUBE", NULL, 0, 0);
		...
		if ((image = OdlFindObjDesc(ob, (char*)"IMAGE", NULL, 0, 0, 0)) != NULL) {
			...
			**/
			...
			format = iom_ConvertISISType(key1 ? key1->value : NULL, key2 ? key2->value : NULL, NULL);
...
/* Always output native-endian data. */
	...
	case iom_BYTE:
	case iom_SHORT:
	case iom_INT: sprintf(buf + strlen(buf), "    CORE_ITEM_TYPE = SUN_INTEGER\r\n"); break;
	case iom_FLOAT:
	case iom_DOUBLE: sprintf(buf + strlen(buf), "    CORE_ITEM_TYPE = SUN_REAL\r\n"); break;
	}
	...
	case iom_BYTE:
	case iom_SHORT:
	case iom_INT: sprintf(buf + strlen(buf), "    CORE_ITEM_TYPE = PC_INTEGER\r\n"); break;

	case iom_FLOAT:
	case iom_DOUBLE: sprintf(buf + strlen(buf), "    CORE_ITEM_TYPE = PC_REAL\r\n"); break;
	}
...
iom_edf iomConvertISISType(char* type, int item_bytes)
{
	...
	} else if (!strcmp(q, "SUN_INTEGER") || (!strcmp(q, "SUN_UNSIGNED_INTEGER")) ||
	           (!strcmp(q, "MSB_UNSIGNED_INTEGER")) || (!strcmp(q, "MSB_SIGNED_INTEGER")) ||
	...
	} else if (!strcmp(q, "PC_INTEGER") || (!strcmp(q, "PC_UNSIGNED_INTEGER")) ||
	           (!strcmp(q, "LSB_UNSIGNED_INTEGER")) || (!strcmp(q, "LSB_SIGNED_INTEGER")) ||
	           (!strcmp(q, "LSB_INTEGER")) || (!strcmp(q, "VAX_INT")) ||
