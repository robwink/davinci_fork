...
**   s2 is the string
*/
static char* envi_get_value(const char* s1, const char* s2)
{
	char *p, *q;
...
int iom_GetENVIHeader(FILE* fp, char* fname, struct iom_iheader* h)
{
	char *p, *q;
	int i;
...
void stradd(char* ptr, const char* fmt, ...)
{
	va_list ap;
...
char envi_header_init[] =
    "ENVI\n"
    "description = { davinci product }\n"
    "file type = ENVI standard\n"
    "header offset = 0     \n";

int iom_WriteENVI(char* filename,        /* File name for reference purpose */
