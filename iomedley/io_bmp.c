 ...
 *
 */
int iom_ReadBMP(FILE* fp, char* filename, int* xout, int* yout, int* zout, unsigned char** dout)
{
	int i, c, rv;
	...
	*dout = (unsigned char*)NULL;
	...
	if ((biBitCount != 1 && biBitCount != 4 && biBitCount != 8 && biBitCount != 24 && biBitCount != 32) ||
		...
		if (iom_is_ok2print_errors()) {
			printf("Bogus BMP File!  (bitCount=%d, Planes=%d, Compression=%d)", biBitCount,
			       biPlanes, biCompression);
		}
	...
	if (((biBitCount == 1 || biBitCount == 24 || biBitCount != 32) && biCompression != BI_RGB) ||
	    (biBitCount == 4 && biCompression == BI_RLE8) || (biBitCount == 8 && biCompression == BI_RLE4)) {
		if (iom_is_ok2print_errors()) {
			printf("Bogus BMP File!  (bitCount=%d, Compression=%d)", biBitCount, biCompression);
		}
		return 0;
	}
	...
	*dout = (unsigned char*)calloc((size_t)*xout * *yout * *zout, (size_t)1);

	// printf("biBitCount: %d\n", biBitCount);

	/* load up the image, have to pass down the color map for 1, 4, and 8 bit */
	if (biBitCount == 1)
		rv = loadBMP1(fp, *dout, *xout, *yout, *zout, red, blue, green);
	else if (biBitCount == 4)
		rv = loadBMP4(fp, *dout, *xout, *yout, *zout, biCompression, red, blue, green);
	else if (biBitCount == 8)
		rv = loadBMP8(fp, *dout, *xout, *yout, *zout, biCompression, red, blue, green);
	else
		rv = loadBMP24(fp, *dout, *xout, *yout, biBitCount);
	// printf("Returned from loadBMP call: %d\n", rv);

	if (rv) printf("File appears truncated.  Winging it.\n");

	if (biCompression == BI_RLE4)
		cmpstr = (char*)", RLE4 compressed";
	else if (biCompression == BI_RLE8)
		cmpstr = (char*)", RLE8 compressed";
	else
		cmpstr = (char*)"";
...
int loadBMP8(FILE* fp, unsigned char* data, int w, int h, int band, int comp, unsigned char* red,
             unsigned char* blue, unsigned char* green)
{
...
int iom_WriteBMP(char* filename, unsigned char* indata, struct iom_iheader* h, int force)
{
	int x, y, z;
	unsigned char* data;
	FILE* fp = NULL;
	...
	if (nbits == 8)
		writeBMP8(fp, data, x, y);
	else if (nbits == 24)
		writeBMP24(fp, data, x, y);
...
unsigned int getint(FILE* fp)
{
	...
	c2 = getc(fp);
	c3 = getc(fp);
	return ((unsigned int)c) + (((unsigned int)c1) << 8) + (((unsigned int)c2) << 16) +
	       (((unsigned int)c3) << 24);
}
