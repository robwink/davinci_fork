...
int iom_GetJPEGHeader(FILE* fp, char* filename, struct iom_iheader* h)
{
...
int iom_ReadJPEG(FILE* fp, char* filename, int* xout, int* yout, int* zout, int* bits, unsigned char** dout)
{
	...
	*yout = y;
	*zout = z;
	*bits = 8; /* This should always be true unless libjpeg is compiled bizarrely.  FIX: Not sure
	              how to check. */
	*dout = data;
...
int iom_WriteJPEG(char* filename, unsigned char* indata, struct iom_iheader* h, int force)
{
	...
	cinfo.err =
	    jpeg_std_error(&jerr);    /* Setup libjpeg error handler. FIX: this causes exit on err? */
