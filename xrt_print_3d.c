...
typedef enum { PRINT_POSTSCRIPT = 1, PRINT_XWD, PRINT_CGM } PrintType;

typedef enum { PRINT_PRINTER = 1, PRINT_FILE } PrintTo;
...
static ButtonItem error_button[] = {{CANCEL, "Cancel", NULL, NULL, True}, {NULL, NULL, NULL, NULL, False}};
static Widget graph;
...
static Widget CreatePrintDialog(Widget parent, XtPointer client_data, XtPointer call_data);
static Widget CreatePSPropsDialog(Widget parent, XtPointer client_data, XtPointer call_data);

static void print_callback(Widget w, XtPointer client_data, XtPointer call_data);
...
static void format_callback(Widget w, XtPointer client_data, XtPointer call_data);
static void dest_callback(Widget w, XtPointer client_data, XtPointer call_data);
static void dismiss_PSprops_callback(Widget w, XtPointer client_data, XtPointer call_data);
static void dismiss_print_callback(Widget w, XtPointer client_data, XtPointer call_data);
...
static void print_reset();
static void unmanage_all();
static void props_callback(Widget w, XtPointer client_data, XtPointer call_data);
static void PSupdate();
static void PSextract();
static void PSok_callback(Widget w, XtPointer client_data, XtPointer call_data);
static void PSreset_callback(Widget w, XtPointer client_data, XtPointer call_data);
static char* UniqueName();
static Widget MakeSFListPopup(Widget parent, char* title);
static void CGMupdate();
static void CGMextract();
static void dismiss_CGMprops_callback(Widget w, XtPointer client_data, XtPointer call_data);
static void CGMok_callback(Widget w, XtPointer client_data, XtPointer call_data);
static void CGMreset_callback(Widget w, XtPointer client_data, XtPointer call_data);
#endif
...
static Widget param_width_txt_w, param_height_txt_w, param_margin_txt_w;
static Widget pos_left_txt_w, pos_bottom_txt_w, pos_width_txt_w, pos_height_txt_w;
static Widget font_header_txt_w, font_footer_txt_w, font_legend_txt_w;
static Widget fill_bg_w, color_radio_w;
static Xrt3dDrawpsProperties ps_props = {
    True, 8.5, 11.0, 0.25, False, 0.0, 0.0, 0.0, 0.0, "", "", "", 1, XRT3D_PS_COLOR_AS_IS, 1};
...
static Xrt3dDrawCGMProperties cgm_props = {SCALE_DEFAULT, XRT3D_SF_ROMAN_DUPLEX,
                                           XRT3D_SF_ROMAN_DUPLEX, XRT3D_SF_ROMAN_DUPLEX};

static SFontList sfont_list[] = {{XRT3D_SF_ROMAN_DUPLEX, "Roman Duplex"},
...
#else
    CreatePrintDialog(parent, client_data, call_data) Widget parent;
Widget client_data;
Widget call_data;
#endif
{
	...
	static ButtonItem format_items[] = {
	    {"postscript", "PostScript", format_callback, (XtPointer)PRINT_POSTSCRIPT, True},
	    {"xwd", "XWD", format_callback, (XtPointer)PRINT_XWD, False},
	    {"cgm", "CGM", format_callback, (XtPointer)PRINT_CGM, False},
	    {NULL, NULL, NULL, NULL, False}};
	static ButtonItem dest_items[] = {{"printer", "Printer", dest_callback, (XtPointer)PRINT_PRINTER, True},
	                                  {"file", "File", dest_callback, (XtPointer)PRINT_FILE, False},
	                                  {NULL, NULL, NULL, NULL, False}};
	static FieldItem pdf_items[] = {{"printer", "Printer:", NULL, NULL, 32, printer, True},
	...
	container_w = XtVaCreateWidget("container_w", xmFormWidgetClass, parent, NULL);

	format_radio = MtBuildRadioBox(container_w, "format_radio", "Format:", format_items, XmVERTICAL,
	                               XmRADIOBUTTON);
	dest_radio = MtBuildRadioBox(container_w, "destination_radio", "Destination:", dest_items,
	...
	XtVaSetValues(format_radio, XmNtopAttachment, XmATTACH_FORM, XmNleftAttachment, XmATTACH_FORM, NULL);
	XtVaSetValues(dest_radio, XmNtopAttachment, XmATTACH_FORM, XmNleftAttachment, XmATTACH_WIDGET,
	              XmNleftWidget, format_radio, NULL);
	XtVaSetValues(pdf_w, XmNtopAttachment, XmATTACH_WIDGET, XmNtopWidget, format_radio,
...
#else
    GetActualRadioBox(radio) Widget radio;
#endif
{
	WidgetList children;
	Widget contained;
	...
	XtVaGetValues(contained, XmNchildren, &children, XmNnumChildren, &num_children, NULL);
	if (num_children > 1) {
...
#else
    SetPrintRadioValue(radio, data) Widget radio;
XtPointer data;
#endif
{
	/*
	...
	XtVaGetValues(tmp_radio, XmNchildren, &children, XmNnumChildren, &n, NULL);
	for (i = 0; i < n; i++) {
		XtVaGetValues(children[i], XmNuserData, &tval, NULL);
		XtVaSetValues(children[i], XmNset, (tval == data), NULL);
	}
}
...
#else
    GetPrintRadioValue(radio) Widget radio;
#endif
{
	/*
	...
	XtVaGetValues(tmp_radio, XmNchildren, &children, XmNnumChildren, &n, NULL);
	for (i = 0; i < n; i++) {
		XtVaGetValues(children[i], XmNset, &set, XmNuserData, &tval, NULL);
		if (set) {
			return ((int)tval);
		}
	}
...
#else
    SetRadioSensitive(radio, sval) Widget radio;
Boolean sval;
#endif
{
	/*
...
#else
    PrintGraph(button) Widget button;
#endif
{
	/*
	...
	if ((print_format == PRINT_POSTSCRIPT) && (print_dest == PRINT_PRINTER)) {
		...
		if ((fp = fopen(tmp_psfname, "w")) == NULL) {
			sprintf(msg, "Can't open temporary Postscript file %s for writing.\n", tmp_psfname);
			MtDisplayMessageDialog(button, PRINT_ERROR, msg, XmDIALOG_ERROR, error_button);
			goto done;
		}
#else
...
#endif /* __VMS */
		status =
		    Xrt3dDrawPS(graph, fp, msg, ps_props.inches, ps_props.pwidth, ps_props.pheight,
		                ps_props.pmargin, ps_props.landscape, ps_props.xoffset, ps_props.yoffset,
		                ps_props.imagewidth, ps_props.imageheight,
		                (ps_props.header_font[0] == (char)NULL) ? (char*)NULL : ps_props.header_font,
		                (ps_props.footer_font[0] == (char)NULL) ? (char*)NULL : ps_props.footer_font,
		                (ps_props.legend_font[0] == (char)NULL) ? (char*)NULL : ps_props.legend_font,
		                ps_props.fill_background, ps_props.color, ps_props.showpage);
#ifdef __VMS
		fclose(fp);
		...
		if (!*file) {
			MtDisplayMessageDialog(button, PRINT_ERROR, "Please enter a file name.", XmDIALOG_ERROR,
			                       error_button);
			goto done;
		}
#ifdef __VMS
		...
		case PRINT_POSTSCRIPT:
			status = Xrt3dDrawPS(
			    graph, fp, msg, ps_props.inches, ps_props.pwidth, ps_props.pheight,
			    ps_props.pmargin, ps_props.landscape, ps_props.xoffset, ps_props.yoffset,
			    ps_props.imagewidth, ps_props.imageheight,
			    (ps_props.header_font[0] == (char)NULL) ? (char*)NULL : ps_props.header_font,
			    (ps_props.footer_font[0] == (char)NULL) ? (char*)NULL : ps_props.footer_font,
			    ...
			    ps_props.fill_background, ps_props.color, ps_props.showpage);
			break;
		case PRINT_XWD: status = Xrt3dOutputXwd(graph, fp, msg); break;
		case PRINT_CGM:
			status = Xrt3dDrawCGM(graph, fp, msg, cgm_props.metric, cgm_props.header,
			                      cgm_props.footer, cgm_props.legend);
			break;
		}
		fclose(fp);
	}
...
static void print_reset()
{
	...
	XtVaSetValues(dir_txt, XmNvalue, getcwd(directory, sizeof(directory) - 1), NULL);
...
#else
    print_callback(w, client_data, call_data) Widget w;
...
#else
    props_callback(w, client_data, call_data) Widget w;
XtPointer client_data;
XtPointer call_data;
#endif
{
...
#else
    dismiss_print_callback(w, client_data, call_data) Widget w;
XtPointer client_data;
XtPointer call_data;
#endif
{
...
#else
    format_callback(w, client_data, call_data) Widget w;
XtPointer client_data;
XtPointer call_data;
#endif
{
	...
	if (set) {
		...
		switch (type) {
		case PRINT_POSTSCRIPT: SetRadioSensitive(dest_radio, TRUE); break;
		case PRINT_XWD:
...
#else
    dest_callback(w, client_data, call_data) Widget w;
XtPointer client_data;
XtPointer call_data;
#endif
{
...
#else
    MtShowPrintCallback(w, client_data, call_data) Widget w;
XtPointer client_data;
XtPointer call_data;
#endif
{
...
#else
    dismiss_PSprops_callback(w, client_data, call_data) Widget w;
XtPointer client_data;
XtPointer call_data;
#endif
{
...
#else
    PSok_callback(w, client_data, call_data) Widget w;
XtPointer client_data;
XtPointer call_data;
#endif
{
...
#else
    PSreset_callback(w, client_data, call_data) Widget w;
XtPointer client_data;
XtPointer call_data;
#endif
{
...
#else
    CreatePSPropsDialog(parent, client_data, call_data) Widget parent;
XtPointer client_data;
XtPointer call_data;
#endif
{
	Widget form, controls, frame1, frame1_1, frame1_2, frame1_3, frame2, fill_radio_w,
	    paper_param_w, printed_pos_w, fonts_w;

	static ButtonItem measure_items[] = {{"inches", "INCHES", NULL, (XtPointer)True, True},
	                                     {"cm", "CM", NULL, (XtPointer)False, False},
	                                     {NULL, NULL, NULL, NULL, False}};
	static ButtonItem orient_items[] = {{"portrait", "Portrait", NULL, (XtPointer)False, True},
	...
	static ButtonItem color_items[] = {
	    ...
	    {NULL, NULL, NULL, NULL, False}};
	static FieldItem paper_items[] = {{"width", "Width:", NULL, NULL, 8, "8.50", True},
	                                  {"height", "Height:", NULL, NULL, 8, "11.0", True},
	                                  {"margin", "Margin:", NULL, NULL, 8, "0.25", True},
	                                  {NULL, NULL, NULL, NULL, 0, NULL, False}};
	static FieldItem position_items[] = {{"left", "From Left:", NULL, NULL, 8, "0.00", True},
	                                     {"bottom", "From Bottom:", NULL, NULL, 8, "0.00", True},
	                                     {"width", "Width:", NULL, NULL, 8, "0.00", True},
	                                     {"height", "Height:", NULL, NULL, 8, "0.00", True},
	                                     {NULL, NULL, NULL, NULL, 0, NULL, False}};
	static FieldItem font_items[] = {{"header", "Header:", NULL, NULL, 20, NULL, True},
	                                 {"footer", "Footer:", NULL, NULL, 20, NULL, True},
	                                 {"legend", "Legend:", NULL, NULL, 20, NULL, True},
	                                 {NULL, NULL, NULL, NULL, 0, NULL, False}};
	static ButtonItem fill_items[] = {{"fill_bg", "Fill Background", NULL, (XtPointer)NULL, True},
	                                  {NULL, NULL, NULL, NULL, False}};

	form = XtVaCreateWidget("container_w", xmFormWidgetClass, parent, NULL);

	controls = XtVaCreateManagedWidget("controlsRC", xmRowColumnWidgetClass, form, XmNorientation,
	                                   XmVERTICAL, XmNtopAttachment, XmATTACH_FORM,
	                                   XmNleftAttachment, XmATTACH_FORM, XmNrightAttachment,
	                                   XmATTACH_FORM, XmNbottomAttachment, XmATTACH_FORM, NULL);

	frame1 = XtVaCreateManagedWidget("formatRC1", xmRowColumnWidgetClass, controls, XmNorientation,
	                                 XmHORIZONTAL, NULL);

	frame1_1 = XtVaCreateManagedWidget("formatRC1_1", xmRowColumnWidgetClass, frame1,
	                                   XmNorientation, XmVERTICAL, NULL);

	measure_radio_w = MtBuildRadioBox(frame1_1, "measure_radio", "Measure In:", measure_items,
	                                  XmVERTICAL, XmRADIOBUTTON);

	orient_radio_w = MtBuildRadioBox(frame1_1, "orient_radio", "Orientation:", orient_items,
	                                 XmVERTICAL, XmRADIOBUTTON);

	frame1_2 = XtVaCreateManagedWidget("formatRC1_2", xmRowColumnWidgetClass, frame1,
	                                   XmNorientation, XmVERTICAL, NULL);

	color_radio_w = MtBuildRadioBox(frame1_2, "color", "Color:", color_items, XmVERTICAL, XmRADIOBUTTON);

	fill_radio_w = MtBuildRadioBox(frame1_2, "fill", NULL, fill_items, XmHORIZONTAL, XmCHECKBUTTON);

	fill_bg_w = MtGetChild(fill_radio_w, "fill_bg_button");

	frame1_3 = XtVaCreateManagedWidget("formatRC1_3", xmRowColumnWidgetClass, frame1,
	                                   XmNorientation, XmVERTICAL, NULL);

	paper_param_w = MtCreateFieldBox(frame1_3, "paper", "Paper Parameters:", paper_items, XmVERTICAL);
	...
	frame2 = XtVaCreateManagedWidget("formatRC2", xmRowColumnWidgetClass, controls, XmNorientation,
	                                 XmHORIZONTAL, NULL);

	printed_pos_w = MtCreateFieldBox(frame2, "positon", "Printed Position:", position_items, XmVERTICAL);
	...
	fonts_w           = MtCreateFieldBox(frame2, "fonts", "Fonts:", font_items, XmVERTICAL);
...
#else
    ShowPSProps(parent, ps_props) Widget parent;
Xrt3dDrawpsProperties* ps_props;
#endif
{
	static ButtonItem ps_buttons[] = {{"Print", "Print", PSok_callback, False},
	...
	if (!pspropsheet) {
		pspropsheet =
		    MtCreateShell(MtGetTopLevel(parent), "pspropsheet", "PostScript Print Properties",
		                  CreatePSPropsDialog, NULL, ps_buttons, False);
	}
...
#else
    SFOptionsCB(w, client, call) Widget w;
...
#else
    MakeSFListPopup(parent, title) Widget parent;
char* title;
#endif
{
	int i;
	...
	for (i = 0; sfont_list[i].name; i++) {
		item = XtVaCreateManagedWidget(sfont_list[i].name, xmPushButtonGadgetClass, menu, NULL);
		XtAddCallback(item, XmNactivateCallback, (XtCallbackProc)SFOptionsCB, (XtPointer)i);
	}
...
#else
    dismiss_CGMprops_callback(w, client_data, call_data) Widget w;
XtPointer client_data;
XtPointer call_data;
#endif
{
...
#else
    CGMok_callback(w, client_data, call_data) Widget w;
XtPointer client_data;
XtPointer call_data;
#endif
{
...
#else
    CGMreset_callback(w, client_data, call_data) Widget w;
XtPointer client_data;
XtPointer call_data;
#endif
{
...
#else
    CreateCGMProps(parent, client_data, call_data) Widget parent;
XtPointer client_data;
XtPointer call_data;
#endif
{
	...
	static FieldItem scale_item = {"scale_w", "Scale (mm/pixel):", NULL, NULL, 8, "-1.00", True};

	controls =
	    XtVaCreateManagedWidget("controlsRC", xmRowColumnWidgetClass, parent, XmNnumColumns, 1, NULL);
...
#else
    ShowCGMProps(parent, cgm_props) Widget parent;
...
#endif
{
	 ...
	 *  Get the current print properties and display them.
	 */
	static ButtonItem cgm_buttons[] = {{"Print", "Print", CGMok_callback, False},
	                                   {"Reset", "Reset", CGMreset_callback, False},
	                                   {CANCEL, "Cancel", dismiss_CGMprops_callback, False},
	                                   {NULL, NULL, NULL, False}};

	if (!cgmpropsheet) {
		cgmpropsheet = MtCreateShell(MtGetTopLevel(parent), "cgmpropsheet", "CGM Print Properties",
		                             CreateCGMProps, NULL, cgm_buttons, False);
	}
...
MtShow3dPrintDialog(Widget graph_to_print, char *default_printer, 
                    char *default_file)
#else
    MtShow3dPrintDialog(graph_to_print, default_printer, default_file) Widget graph_to_print;
char* default_printer;
char* default_file;
#endif
{
	static ButtonItem print_buttons[] = {{"print", "Print", print_callback, NULL, True},
	...
	if (!print_dialog) {
		print_dialog = MtCreateShell(MtGetShell(graph_to_print), "print_dialog", "Print XRT/3d",
		                             CreatePrintDialog, NULL, print_buttons, False);
	}
