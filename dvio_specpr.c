...
#define swap4byte(s) \
	(swap1byte(((byteptr)(s))[0], ((byteptr)(s))[3]), swap1byte(((byteptr)(s))[1], ((byteptr)(s))[2]))

void byteorder(a, iflag) int *a, iflag;
{
	int itmp, i;
...
int read_record(int fd, int i, char* label)
{
...
int read_specpr(int fd, int i, struct _label* label, char** data)
{
...
int write_record(int fd, int i, struct _label* label)
{
	if (i < 0) {
...
struct _label* make_label(int npixels, int waves, char* title, char* ahist, char* mhist)
{
	int i;
	...
	if (title != NULL)
		for (i = 0; i < 40; i++) label.ititl[i] = (i >= strlen(title) ? ' ' : title[i]);
	if (ahist != NULL)
		for (i = 0; i < 60; i++) label.ihist[i] = (i >= strlen(ahist) ? ' ' : ahist[i]);
	if (mhist != NULL)
		for (i = 0; i < 296; i++) label.mhist[i] = (i >= strlen(mhist) ? ' ' : mhist[i]);
...
Var* dv_LoadSpecpr(FILE* fp, char* filename, struct iom_iheader* h)
{
...
Var* LoadSpecpr(FILE* fp, char* filename, int rec)
{
	struct _label label;
	float* data;
	Var* s;
...
int WriteSpecpr(Var* v, char* filename, char* title)
{
	Var* e;
	float* fdata;
	int i;
	...
	if (VERBOSE > 1) {
		fprintf(stderr, "Writing %s: SpecPR record: %ld channels\nTitle: %s\n", filename, V_DSIZE(v), title);
	}
 ...
 **/
int LoadSpecprHeader(FILE* fp, char* filename, int rec, char* element, Var** val)
{
	struct _label label;
	struct _tlabel* tlabel;
	float* data;
	...
	if (!strcasecmp(element, "icflag"))
		...
		tval  = label.usernm;
		range = 8;
	} else if (!strcasecmp(element, "iscta"))
		ival = &label.iscta;
	else if (!strcasecmp(element, "isctb"))
		ival = &label.isctb;
	else if (!strcasecmp(element, "jdatea"))
		ival = &label.jdatea;
	else if (!strcasecmp(element, "jdateb"))
		ival = &label.jdateb;
	else if (!strcasecmp(element, "istb"))
		ival = &label.istb;
	else if (!strcasecmp(element, "isra"))
		ival = &label.isra;
	else if (!strcasecmp(element, "isdec"))
		ival = &label.isdec;
	else if (!strcasecmp(element, "itchan"))
		ival = &label.itchan;
	else if (!strcasecmp(element, "irmas"))
		ival = &label.irmas;
	else if (!strcasecmp(element, "revs"))
		ival = &label.revs;
	else if (!strcasecmp(element, "iband")) {
		ival  = label.iband;
		range = 2;
	} else if (!strcasecmp(element, "irwav"))
		ival = &label.irwav;
	else if (!strcasecmp(element, "irespt"))
		ival = &label.irespt;
	else if (!strcasecmp(element, "irecno"))
		ival = &label.irecno;
	else if (!strcasecmp(element, "itpntr"))
		ival = &label.itpntr;
		...
		tval  = label.mhist;
		range = 296;
	} else if (!strcasecmp(element, "nruns"))
		ival = &label.nruns;
	else if (!strcasecmp(element, "siangl"))
		ival = &label.siangl;
	else if (!strcasecmp(element, "seangl"))
		ival = &label.seangl;
	else if (!strcasecmp(element, "sphase"))
		ival = &label.sphase;
	else if (!strcasecmp(element, "iwtrns"))
		ival = &label.iwtrns;
	else if (!strcasecmp(element, "itimch"))
		ival = &label.itimch;
	else if (!strcasecmp(element, "xnrm"))
		fval = &label.xnrm;
	else if (!strcasecmp(element, "scatin"))
		fval = &label.scatin;
	else if (!strcasecmp(element, "timint"))
		fval = &label.timint;
	else if (!strcasecmp(element, "tempd"))
		fval = &label.tempd;

	else if (!strcasecmp(element, "itxtpt"))
		ival = &tlabel->itxtpt;
	else if (!strcasecmp(element, "itxtch"))
		ival = &tlabel->itxtch;
 ...
 ** LoadSpecprHeader() - extract a specific header element from a specpr file
 **/
Var* LoadSpecprHeaderStruct(FILE* fp, char* filename, int rec)
{
	struct _label label;
	struct _tlabel* tlabel;
	float* data;
