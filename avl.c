...
#if PSPP
static inline avl_node* new_node(arena** owner)
{
	if (owner != NULL)
...
/* Copy the contents of TREE to a new tree in arena OWNER.  If COPY is
   ...
   new tree. */
avl_tree* avl_copy(MAYBE_ARENA const avl_tree* tree, avl_copy_func copy)
{
...
/* Walk tree TREE in inorder, calling WALK_FUNC at each node.  Passes
   PARAM to WALK_FUNC.  */
void avl_walk(const avl_tree* tree, avl_node_func walk_func, void* param)
{
...
/* Each call to this function for a given TREE and TRAV return the
   ...
   out of elements.  */
void* avl_traverse(const avl_tree* tree, avl_traverser* trav)
{
	assert(tree && trav);
...
void* avl_find(const avl_tree* tree, const void* item)
{
	const avl_node* p;
...
void* avl_find_close(const avl_tree* tree, const void* item)
{
	const avl_node* p;
...
/* Searches AVL tree TREE for an item matching ITEM.  If found, the
   ...
   returns NULL. */
void* avl_delete(avl_tree* tree, const void* item)
{
...
void print_structure(avl_node* node, int level)
{
	...
	if (node->link[0] || node->link[1]) print_structure(node->link[0], level + 1);
	if (node->link[1]) print_structure(node->link[1], level + 1);
	printf("%c", rc[level % 5]);
}
...
void print_contents(avl_tree* tree)
{
...
int recurse_tree(avl_node* node, int* count, int parent, int dir)
{
	if (node) {
		...
		if (nr - nl != node->bal) {
			printf(
			    " Node %d is unbalanced: right height=%d, left height=%d, "
...
void compare_trees(avl_node* a, avl_node* b)
{
	...
	if (a->data != b->data || a->bal != b->bal || ((a->link[0] != NULL) ^ (b->link[0] != NULL)) ||
	    ((a->link[1] != NULL) ^ (b->link[1] != NULL))) {
		printf(" Copied nodes differ: %d b=%d a->bal=%d b->bal=%d a:", (int)a->data, (int)b->data,
		       a->bal, b->bal);
		if (a->link[0]) printf("l");
		if (a->link[1]) printf("r");
		printf(" b:");
		if (b->link[0]) printf("l");
		if (b->link[1]) printf("r");
		printf("\n");
		abort();
	}
	if (a->link[0] != NULL) compare_trees(a->link[0], b->link[0]);
	if (a->link[1] != NULL) compare_trees(a->link[1], b->link[1]);
}
...
int main(int argc, char** argv)
{
	...
	for (iteration = 1; iteration <= N_ITERATIONS; iteration++) {
		...
		for (i = 0; i < TREE_SIZE; i++) array[i] = i;
		shuffle(array, TREE_SIZE);

		tree = avl_create(compare_ints, NULL);
		for (i = 0; i < TREE_SIZE; i++) avl_force_insert(tree, (void*)(array[i]));
		verify_tree(tree);
