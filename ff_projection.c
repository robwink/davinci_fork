...
Var* ff_projection(vfuncptr func, Var* arg)
{
	...
	if (pType == NULL) {
		...
		return NULL;
	} else if (!strcasecmp(pType, "MARS"))
		type = 1;
	else if (!strcasecmp(pType, "EARTH"))
		type = 2;
	else if (!strcasecmp(pType, "SPHERE"))
		type = 3;
	...
	for (i = 0; i < newY; i++) {
		...
		for (j = 0; j < newX; j++) {
			...
			for (k = 0; k < z; k++) {
				...
				memcpy(&((char*)data)[jump + step + k * size], &((char*)V_DATA(obj))[index * size], size);
			}
		}
	}
