...
static char* find_next_nonwhite(char* line)
{
...
static char* copy_nonwhite(char* from, char* to, int tolen)
{
	tolen--;
...
static char* grab_keyval(char* line, char* key, char* val, int keylen, int vallen)
{
	*key = (char)0;
	*val = (char)0;

	/* Find keyword */
	if ((line = find_next_nonwhite(line)) == 0) return 0;
	...
	/* Find value */
	if ((line = find_next_nonwhite(line + 1)) == 0) return 0;
...
static int parse_label(char* hdr, char*** scopes, int* nscopes)
{
	char* label = hdr;
#define KEYLEN 20
#define VALLEN 20
...
Var* LoadTDB(char* filename)
{
	...
	while (1) {
		...
		if (c->n_df_syms) {
			...
			add_struct(member, "scope", newString(strdup(scopes[c->whichscope])));
			for (i = 0; i < c->n_df_syms; i++) {
				...
				case T_DOUBLE: {
					...
					v1 = newVal(BSQ, d->dim_size[0] ? d->dim_size[0] : 1,
					            d->dim_size[1] ? d->dim_size[1] : 1,
					            d->dim_size[2] ? d->dim_size[2] : 1, DV_DOUBLE, dbl);
					break;
				}
				case T_FLOAT:
				case T_INTEGER:
				default: fprintf(stderr, "Unrecognized type: %d\n", d->type); break;
				}
...
Var* ff_load_tdb(vfuncptr func, Var* arg)
{
...
static Var* collect_keys(Var* data, Var* scopes)
{
	...
	for (j = 0; j < get_struct_count(scopes); j++) {
		...
		for (i = 0; i < get_struct_count(data); i++) {
			...
			if (find_struct(v1, "scope", &v2) != -1 && !strcmp(V_STRING(v2), scope_name)) {
	...
	for (i = 0; i < get_struct_count(data); i++) {
		...
		if (strlen(V_STRING(xname)) > 0) {
			for (j = 0; j < get_struct_count(data); j++) {
				...
				if (find_struct(xmember, "name", &name) != -1 && !strcmp(V_STRING(xname), V_STRING(name))) {
