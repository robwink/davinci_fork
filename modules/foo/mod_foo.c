...
/* API CLI definitions */
APIARGS sqrt_args[] = {{"sqrt", DV_DOUBLE}, {"val", DV_DOUBLE}};

void sqrt_wrapper(int ac, APIARGS* av)
{
...
APIARGS ceil_args[] = {{"ceil", DV_DOUBLE}, {"val", DV_DOUBLE}};

void ceil_wrapper(int ac, APIARGS* av)
{
	av[0].argval             = malloc(sizeof(double));
	*((double*)av[0].argval) = ceil(*((double*)av[1].argval)); /* double val */
}

APIARGS floor_args[] = {{"floor", DV_DOUBLE}, {"val", DV_DOUBLE}};

void floor_wrapper(int ac, APIARGS* av)
{
	av[0].argval             = malloc(sizeof(double));
...
static Var* foo_dispatch(vfuncptr func, Var* args)
{
	Var* result = NULL;
	int i;
