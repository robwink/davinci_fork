	...
	for (i = 0; i < newcols; i++) {
		for (j = 0; j < newrows; j++) {
			for (k = 0; k < z; k++) {
				...
				case BYTE: ((u_char*)data)[k1] = (u_char)GetPixel(obj, x, y, orgX, orgY, k); break;
				case SHORT: ((short*)data)[k1] = (short)GetPixel(obj, orgX, x, y, orgY, k); break;
				case INT: ((int*)data)[k1]     = (int)GetPixel(obj, orgX, x, y, orgY, k); break;
				case FLOAT: ((float*)data)[k1] = (float)GetPixel(obj, x, y, orgX, orgY, k); break;
				case DOUBLE: ((double*)data)[k1] = GetPixel(obj, x, y, orgX, orgY, k); break;
	...
	for (i = 0; i < nx; i++) {
		for (j = 0; j < ny; j++) {
			for (k = 0; k < z; k++) {
				...
				switch (V_FORMAT(output)) {
				case BYTE: ((u_char*)data)[k1]   = color; break;
				case SHORT: ((short*)data)[k1]   = color; break;
				case INT: ((int*)data)[k1]       = color; break;
				case FLOAT: ((float*)data)[k1]   = color; break;
				case DOUBLE: ((double*)data)[k1] = color; break;
	...
	for (i = 0; i < x; i++) {
		for (j = 0; j < y; j++) {
			for (k = 0; k < z; k++) {
				...
				k2 = cpos(i, j, k, obj);
				switch (V_FORMAT(obj)) {
				case BYTE: ((u_char*)data)[k1]   = (u_char)extract_int(obj, k2); break;
				case SHORT: ((short*)data)[k1]   = (short)extract_int(obj, k2); break;
				case INT: ((int*)data)[k1]       = extract_int(obj, k2); break;
				case FLOAT: ((float*)data)[k1]   = extract_float(obj, k2); break;
				case DOUBLE: ((double*)data)[k1] = extract_double(obj, k2); break;
				default:
					printf("\n\nUnsupported type for pad\n\n");
					return (0);
					break;
...
static void fix_stripe(void* data, Var* obj, int x, int k1, int k2, int k3, int detectors)
{
	...
	if ((x + 1) % detectors == 0) {
		switch (V_FORMAT(obj)) {
		case BYTE: ((u_char*)data)[k1] = (extract_int(obj, k2) + extract_int(obj, k3)) / 2; break;
		case SHORT: ((short*)data)[k1] = (extract_int(obj, k2) + extract_int(obj, k3)) / 2; break;
		case INT: ((int*)data)[k1]     = (extract_int(obj, k2) + extract_int(obj, k3)) / 2; break;
		case FLOAT:
		...
		switch (V_FORMAT(obj)) {
		case BYTE: ((u_char*)data)[k1]   = extract_int(obj, k1); break;
		case SHORT: ((short*)data)[k1]   = extract_int(obj, k1); break;
		case INT: ((int*)data)[k1]       = extract_int(obj, k1); break;
		case FLOAT: ((float*)data)[k1]   = extract_float(obj, k1); break;
		case DOUBLE: ((double*)data)[k1] = extract_double(obj, k1); break;
		default: break;
		}
	}
}
