...
static float* smoothy(float* obj, float* kernel, int ox, int oy, int oz, int kx, int ky, int kz,
...
Var* kjn_y_shear(vfuncptr func, Var* arg)
{
...
Var* kjn_kfill(vfuncptr func, Var* arg)
{
	Var* pic_v = NULL; /* the picture */
	...
	if (pic_v == NULL && wgt_v == NULL) {
		...
		parse_error(
		    "Fills in null points of a 2 dimensional float array by linear interpolation of "
		    "nearest neighbors.");
		...
		parse_error(
		    "Set iterations to limit the maximum number of pixels away nearest neighbor must be to "
		    "be valid.");
		...
		for (j = 0; j < y; j++) {
			for (i = 0; i < x; i++) {
				...
				if (pic[pi] == 0.0) {
					...
					/* loop through the data to find the nearest neighbors in the 4 cardinal
					 * directions */
					while (r <= wth) {
					...
					if (nn > 1) {
						...
						for (mj = wth - pu; mj <= wth + pd; mj++) {
							for (mi = wth - pl; mi <= wth + pr; mi++) {
								...
								if (pic[(j + mj) * pic_x + (i + mi)] > 0.0) {
									sum_non_zero_mask += wgt[mj * w + mi];
								}
							}
						}

						if (sum_non_zero_mask == 0.0) {
							sum_non_zero_mask = 1.0;
						} /* sanity */
...
Var* kjn_smoothy(vfuncptr func, Var* arg)
{
	...
	if (obj == NULL && kernel == NULL) {
		...
		parse_error(
		    "Convolve function that will NOT fill in nullvalues of smoothed array with "
		    "interpolated points.");
		...
		parse_error(
		    "normalize - whether or not to normalize the output to the weight of the kernel. "
		    "Default is 1 (yes).");
		parse_error(
		    "ignore - the null value of pixels to be ignored in calculation. Default is 0.");
		parse_error(
		    "kernreduce - whether to reduce the kernel on both sides as calculation approaches "
		    "object boundary. Default is 0 (no).\n");
		return NULL;
	}
	if (obj == NULL) {
			...
			for (a = 0; a < krn_x; a++) {  /* current x position of kernel */
				...
				for (b = 0; b < krn_y; b++) {  /* current y position of kernel */
					...
					for (c = 0; c < krn_z; c++) { /* current z position of kernel */
						z_pos =
						    z + c - kz_center; /* where the current operation is being done in z */
						if (z_pos < 0 || z_pos >= obj_z) continue;
						j = cpos(x_pos, y_pos, z_pos,
						         obj); /* position in 1-D object at (x_pos,y_pos,z_pos) */
...
float* smoothy(float* obj, float* kernel, int ox, int oy, int oz, int kx, int ky, int kz, int norm,
               float ignore)
{
...
Var* kjn_ramp(vfuncptr func, Var* arg)
{
	...
	if (pic_1 == NULL || pic_2 == NULL) {
		...
		parse_error(
		    "You need to pass me two overlapping pictures contained in arrays of identical size, "
		    "and an ignore value.\n");
	...
	for (j = r1; j <= r2; j++) {
		for (i = c1; i <= c2; i++) {
			...
			if (ol1[t1] != -1 && ol2[t1] != -1) {
				...
				if (ol1[left] == -1 || ol1[right] == -1 || ol1[up] == -1 || ol1[down] == -1) {
					ol1[t1] = 1;
				}
				if (ol2[left] == -1 || ol2[right] == -1 || ol2[up] == -1 || ol2[down] == -1) {
					ol2[t1] = 1;
				}
			}
		}
...
Var* kjn_corners(vfuncptr func, Var* arg)
{
	/* last updated 07/06/2005 to change "null" arguments to accept "ignore". - kjn*/
...
Var* kjn_sawtooth(vfuncptr func, Var* arg)
{
...
Var* kjn_deplaid(vfuncptr func, Var* arg)
{
	/* last updated 07/06/2005 to change "null" arguments to accept "ignore". - kjn*/
	/* this version differs from the "thm.deplaid" because it still contains the legacy code
	 * commented out */
	...
	if (data == NULL) {
		...
		parse_error(
		    "example b = "
		    "kjn.deplaid(data=a,ignore=-32768,tmask_max=1.25,tmask_min=.85,filt_len=200,b10=3)");
		...
		parse_error(
		    "You should designate b10 when NOT feeding deplaid 10 band data, b10 = 0 for no "
		    "band-10 data.");
	...
	for (k = 0; k < z; k++) {
		...
		for (j = 0; j < y; j++) {
			for (i = 0; i < x; i++) {
				...
				if (tv != nullval) {
					rdata[x * y * k + x * j + i] =
					    tv - row_avg[k * y + j] - col_avg[k * chunks * x + ck * x + i];
				} else {
...
Var* kjn_rectify(vfuncptr func, Var* arg)
{
	/* last updated 07/06/2005 to change "null" arguments to accept "ignore". - kjn*/
	...
	if (obj == NULL) {
		...
		parse_error(
		    "ignore - the value in non-data pixels of projected cube. Default is "
		    "-3.402822655e+38.");
		parse_error("trust - where in the image the angle should be determined.");
		parse_error(
		    "        0 = top, 1 = bottom, .25 = 25 percent down the image, etc.  Default is 0 "
		    "(top).\n");
		parse_error("TROUBLESHOOTING");
...
Var* kjn_coreg(vfuncptr func, Var* arg)
{
	/* last updated 07/06/2005 to change "null" arguments to accept "ignore". - kjn*/
	...
	if (pic1_in == NULL || pic2_in == NULL) {
		...
		parse_error(
		    "Feed this two pictures of the same size, and it will return the x and y shift for "
		    "coregistration.\n");
	...
	for (k = 0; k < z; k++) {
		for (j = 0; j < y; j++) {
			for (i = 0; i < x; i++) {
				...
				if (tv != nullval &&
				    blackmask[x * j + i] > 1) { /* if not a null val and tempmask ok */
...
Var* kjn_supersample(vfuncptr func, Var* arg)
{
...
Var* kjn_ss_coreg(vfuncptr func, Var* arg)
{
	...
	if (pic == NULL) {
		parse_error("\nss_coreg() - 6/09/04\n");
		parse_error(
		    "coregisters up to 10 bands of byte data.  Usually used with ss_pic()'d arrays.");
		...
		parse_error(
		    "b10: the band 10 picture (coregistration not searched for this band). Default is "
		    "10.\n");
		return NULL;
	}
...
Var* kjn_ss_pic(vfuncptr func, Var* arg)
{
...
Var* kjn_coreg_fill(vfuncptr func, Var* arg)
{
	...
	for (k = 0; k < z; k++) {
		...
		for (j = 0; j < y; j++) {
			for (i = 0; i < x; i++) {
				cssdata[k * (x + p) * (y + p) + (j + (p / 2) - y_shift) * (x + p) + i + (p / 2) - x_shift] =
				    extract_float(data, cpos(i, j, k, data));
			}
		}
	}
...
Var* kjn_rad2tb(vfuncptr func, Var* arg)
{
	/* last updated 07/06/2005 to change "null" arguments to accept "ignore". - kjn*/
	...
	if (rad == NULL) {
		...
		parse_error(
		    "example: b = "
		    "kjn.rad2tb(rad=a,bandlist=4//9//10,ignore=-32768,temp_rad_path=\"/themis/calib/"
		    "temp_rad_v3\")");
...
Var* kjn_tb2rad(vfuncptr func, Var* arg)
{
	...
	if (btemps == NULL) {
		...
		parse_error(
		    "example: b = "
		    "kjn.tb2rad(btemps=a,bandlist=4//9//10,ignore=-32768,temp_rad_path=\"/themis/calib/"
		    "temp_rad_v3\")");
		parse_error("rad - any float valued 3-D radiance array.");
		parse_error("bandlist - an ordered list of THEMIS bands in rad. Default is 1:10.");
		parse_error("ignore - non-data pixel value. Default is -32768 AND 0.");
...
Var* kjn_themissivity(vfuncptr func, Var* arg)
{
	/* last updated 07/06/2005 to change "null" arguments to accept "ignore". - kjn*/
	...
	alist[6] = make_alist("null", DV_FLOAT, NULL,
	                      &nullval); // can be deleted when all legacy programs are dead
	alist[7].name = NULL;
	...
	if (rad == NULL) {
		...
		parse_error(
		    "example: b = "
		    "kjn.themissivity(rad=a,bandlist=4//9//10,ignore=-2,temp_rad_path=\"/themis/calib/"
		    "temp_rad_v3\")");
...
Var* kjn_emiss2rad(vfuncptr func, Var* arg)
{
	/* created 09/22/2005 - kjn*/
	...
	if (estruct == NULL) {
		...
		parse_error(
		    "example: b = "
		    "kjn.emiss2rad(rad=a,bandlist=4//9//10,ignore=-32768,temp_rad_path=\"/themis/calib/"
		    "temp_rad_v3\")");
		parse_error(
		    "emiss - the product of kjn.themissivity containing .emiss and .maxbtemp components.");
		parse_error("bandlist - an ordered list of THEMIS bands in rad. Default is 1:10.");
		parse_error("ignore - non-data pixel value. Default is -32768 AND 0.");
		parse_error(
		    "temp_rad_path - alternate path to temp_rad lookup table.  Default is "
...
Var* kjn_whitey(vfuncptr func, Var* arg)
{
	/* last updated 07/06/2005 to change "null" arguments to accept "ignore". - kjn*/
...
Var* do_ipi(Var* coords_array, Var* values_array)
{
	...
	result[0] = coords[1] -
	            .5 * (pow(coords[1] - coords[0], 2) * (values[1] - values[2]) -
	                  pow(coords[1] - coords[2], 2) * (values[1] - values[0])) /
	                ((coords[1] - coords[0]) * (values[1] - values[2]) -
	                 (coords[1] - coords[2]) * (values[1] - values[0]));
...
Var* kjn_ipi(vfuncptr func, Var* arg)
{
	...
	if ((coords_array == NULL) || (values_array == NULL)) {
		...
		parse_error(
		    "The function calculates the coordinates of a minimum by taking in a 3x1x1 array of "
		    "coordinate and its correlated");
		parse_error(
		    "values, also in a 3x1x1 array, and applying an inverse parabolic interpolation "
		    "algorithm");
		parse_error(
		    "The function outputs a single float number indicating the coordinate at which the "
		    "minimum is located.\n");
...
Var* kjn_src_col_fill(vfuncptr func, Var* arg)
{
...
Var* kjn_deplaid2(vfuncptr func, Var* arg)
{
	/* last updated 07/06/2005 to change "null" arguments to accept "ignore". - kjn*/
	/* this version differs from the "thm.deplaid" because it still contains the legacy code
	 * commented out */
	...
	for (k = 0; k < z; k++) {
		for (j = 0; j < y; j++) {
			...
			// if(z > 1 && k != b10) {                                               /* if there is
			// more than one band and current band isn't band 10 */
			// row_bright[j] += row_avg[k*y + j];                                  /* add up the
			// brightness information */
			// row_wt[j] += 1;
			//}

			// if(z > 1 && k == z-1) row_bright[j] /= (float)row_wt[j];              /* make
			// row_bright an avg rather than a sum */
		}
	}
	...
	// if(k != b10) {
	// for(j=0; j<y; j++) {
	// row_avg[k*y + j] -= row_bright[j];                         /* brightness difference removed
	// from row_avg */
	//}
	//}
	//}
	...
	for (k = 0; k < z; k++) {
		for (j = 0; j < chunks; j++) {
			for (i = 0; i < x; i++) {
				...
				// if(z>1 && k != b10) {                                                        /*
				// if more than one band and not band 10 */
				// col_bright[j*x + i] += col_avg[k*chunks*x + j*x + i];                      /* sum
				// column brightness excluding band 10 */
				// col_wt[j*x + i] += 1;                                                      /* sum
				// column weight array excluding band 10 */
				//}
				// if(z > 1 && k == z-1) col_bright[j*x + i] /= (float)col_wt[j*x + i];         /*
				// make col_bright an avg rather than a sum */
			}
		}
	}
	...
	// for(j=0; j<chunks; j++) {
	// for(i=0; i<x; i++) {
	// col_avg[k*chunks*x + j*x + i] -= col_bright[j*x + i];                  /* brightness
	// difference removed from col_avg */
	//}
	//}
	//}
	//}
