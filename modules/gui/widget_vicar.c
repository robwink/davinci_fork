...
typedef enum { DrawBitmap, DrawRectf } OverlayDraw_t;
...
/* inputMode values. */
typedef enum { DV_VICAR_INPUT_SELECT = 1, DV_VICAR_INPUT_POINT } VicarInputMode;
...
static VicarGraphics* addVicarGraphics(VicarData*, XvicID, int, XvicColor, double, double);
...
int gui_isVicar(const char* name)
{
...
Widget gui_initVicar(const char* dvName, WidgetClass class, Widget parent, Var* dvResources,
                     void** instanceData, Narray* publicResources, Widget* outerWidget)
{
...
#if DEBUG
	fprintf(stderr,
	        "DEBUG: gui_initVicar(dvName = '%s', class = %ld, "
	...
	newWidget = XvicCreateImage(parent, DV_VICAR_WIDGET_APP_NAME, xtArgs, xtArgCount);
	...
	XtVaGetValues(newWidget, XvicNtileWidth, &tileWidth, XvicNtileHeight, &tileHeight, NULL);
	...
	XParseColor(XtDisplay(newWidget), DefaultColormapOfScreen(XtScreen(newWidget)),
...
void setZoom(Widget widget, Var* value)
{
...
static int setImage(Widget widget, Var* value)
{
	...
	case DV_UINT8: dataType  = XvicBYTE; break;
	case DV_INT16: dataType  = XvicHALF; break;
	case DV_INT32: dataType  = XvicFULL; break;
	case DV_FLOAT: dataType  = XvicREAL; break;
	case DV_DOUBLE: dataType = XvicDOUBLE; break;
	default:
		parse_error(
		    "VICAR image data must be DV_UINT8, DV_INT16, DV_INT32, DV_FLOAT, or DV_DOUBLE.");
		return 0;
	}
	...
	XtVaGetValues(widget, XvicNviewWidth, &viewWidth, XvicNviewHeight, &viewHeight, NULL);
	...
	if (imageDepth == 1) {
		...
		vicImageData.grn_pixels =
		    ((unsigned char*)V_DATA(value)) + (vicImageData.width * vicImageData.height);
		vicImageData.blu_pixels =
		    ((unsigned char*)V_DATA(value)) + (vicImageData.width * vicImageData.height * 2);
	}
	...
	n = 0;
	XtSetArg(xtArgs[n], XvicNimageWidth, (XtArgVal)imageWidth);
	n++;
	XtSetArg(xtArgs[n], XvicNimageHeight, (XtArgVal)imageHeight);
	n++;
	XtSetArg(xtArgs[n], XvicNdataType, (XtArgVal)dataType);
	n++;
	XtSetArg(xtArgs[n], XvicNimageMode, (XtArgVal)(imageDepth == 1 ? XvicBW : XvicCOLOR));
	n++;
	/* FIX: this probably needs to vary depending on the X server setting.. */
	XtSetArg(xtArgs[n], XvicNvisualType, (XtArgVal)XvicUSE_24BIT);
	n++;
	...
	XtSetArg(xtArgs[n], XvicNtileWidth, (XtArgVal)tileWidth);
	n++;
	XtSetArg(xtArgs[n], XvicNtileHeight, (XtArgVal)tileHeight);
	n++;
	viewWidth  = MIN(viewWidth, imageWidth);
	viewHeight = MIN(viewHeight, imageHeight);
	XtSetArg(xtArgs[n], XvicNviewWidth, (XtArgVal)viewWidth);
	n++;
	XtSetArg(xtArgs[n], XvicNviewHeight, (XtArgVal)viewHeight);
	n++;
...
static Var* getSelection(Widget widget)
{
...
static Var* getPoint(Widget widget)
{
	MyWidgetList widgetPtr = gui_getWidgetListEntryFromWidget(widget);
	VicarData* vicarData   = (VicarData*)widgetPtr->data;
	Var* o                 = new_struct(0);
...
static Var* getInputMode(Widget widget)
{
	MyWidgetList widgetPtr = gui_getWidgetListEntryFromWidget(widget);
	VicarData* vicarData   = (VicarData*)widgetPtr->data;
	...
	case DV_VICAR_INPUT_POINT: inputModeString  = "MODE_POINT"; break;
	case DV_VICAR_INPUT_SELECT: inputModeString = "MODE_SELECT"; break;
#if 0
...
static void setInputMode(Widget widget, Var* value)
{
	MyWidgetList widgetListEntry;
	VicarData* vicarData;
	char* inputModeString;
...
static void setSelection(Widget widget, Var* value)
{
	VicarData* vicarData;
	...
	if (numElements) {
		if (numElements != 4) {
			parse_error(
			    "VICAR image selection must contain x, y, width, and height values or be empty.");
			return;
		}
	...
	if (numElements) {
		...
		selectionId = XvicImageDrawRectangle(widget, 0, vicarData->overlayGC, graphics->color, x, y,
		                                     width, height);
...
void setOverlay(Widget widget, Var* value)
{
	...
	if (find_struct(value, "id", &vId) < 0 || V_TYPE(vId) != ID_VAL || V_FORMAT(vId) > DV_INT32) {
	...
	if (find_struct(value, "color", &vColor) >= 0) {
		if (V_TYPE(vColor) != ID_STRING) {
			...
			delete = False;
			/* Parse the color. */
			colorStatus = XParseColor(XtDisplay(widget), DefaultColormapOfScreen(XtScreen(widget)),
			                          colorString, &color);
	...
	if (vBitmap) {
		...
		if (V_TYPE(vBitmap) != ID_VAL || V_FORMAT(vBitmap) != DV_UINT8 || GetZ(vBitmap) != 1) {
			...
			bitmapData = (unsigned char*)calloc((size_t)(bytesPerLine * height), sizeof(char));
			if (bitmapData == NULL) {
			...
			for (row = 0; row < height; row++) {
				for (col = 0; col < width; col++) {
					if (dvData[row * width + col]) {
						*(bitmapData + (bytesPerLine * row) + (col / 8)) |= (1 << (col % 8));
					}
				}
			}
			...
			pixmap = XCreatePixmapFromBitmapData(XtDisplay(widget),
			                                     RootWindowOfScreen(XtScreen(widget)), (char*)bitmapData,
			                                     width, height, 1, /* Foreground pixel value. */
		...
		if (V_TYPE(vRectf) != ID_VAL || GetX(vRectf) != 2 || GetY(vRectf) != 1 || GetZ(vRectf) != 1) {
	...
	if (new || redraw) {
		...
		case DrawBitmap:
			...
			vicarOverlayId = XvicImageDrawBitmap(widget, 0, vicarData->overlayGC, vicarColor, x, y,
			                                     pixmap, width, height, 0, 0);
			if (vicarOverlayId == 0) {
				/* This should never happen. */
			...
			addVicarGraphicsBitmap(vicarData, vicarOverlayId, davinciOverlayId, vicarColor, x, y, pixmap);
			break;
		case DrawRectf:
			/* Create Vicar overlay filled rectangle object */
			vicarOverlayId = XvicImageFillRectangle(widget, 0, vicarData->overlayGC, vicarColor, x,
			                                        y, recwidth, recheight);
			addVicarGraphicsRectf(vicarData, vicarOverlayId, davinciOverlayId, vicarColor, x, y,
			                      recwidth, recheight);
			break;
		default:
		...
		XvicImageMoveObject(widget, graphics->vicarOverlayId, x - graphics->x, y - graphics->y);
		graphics->x = x;
		graphics->y = y;
	}
...
static VicarGraphics* addVicarGraphics(VicarData* vicarData, XvicID vicarOverlayId,
                                       int davinciOverlayId, XvicColor color, double x, double y)
{
	VicarGraphics* graphics;

#if DEBUG
	fprintf(stderr,
	        "DEBUG: addVicarGraphics(vicarData = %ld, XvicID = %d, "
	        "davinciOverlayId = %d, XvicColor = %d, x = %lf, y = %lf, "
	        "pixmap = %d)\n",
	        vicarData, vicarOverlayId, davinciOverlayId, color, x, y, pixmap);
#endif
...
static void addVicarGraphicsBitmap(VicarData* vicarData, XvicID vicarOverlayId, int davinciOverlayId,
                                   XvicColor color, double x, double y, Pixmap pixmap)
{
	VicarGraphics* graphics;
...
static void addVicarGraphicsRectf(VicarData* vicarData, XvicID vicarOverlayId, int davinciOverlayId,
                                  XvicColor color, double x, double y, double width, double height)
{
	VicarGraphics* graphics;
	graphics       = addVicarGraphics(vicarData, vicarOverlayId, davinciOverlayId, color, x, y);
...
static void deleteVicarGraphicsObject(VicarData* vicarData, VicarGraphics* graphics, Boolean cleanTypeData)
{
	...
	if (cleanTypeData == True) {
		...
		case DrawBitmap:
			if (graphics->bitmap.pixmap != 0) {
				...
				XFreePixmap(XtDisplay(vicarData->imageWidget), graphics->bitmap.pixmap);
			}
			break;
		case DrawRectf:
...
static VicarGraphics* getVicarGraphicsByDavinciId(VicarData* vicarData, int id)
{
	VicarGraphics* cur;
...
static void handleExpose(Widget widget, XtPointer clientData, XtPointer callData)
{
...
/* FIX: this doesn't take different data sizes into account. */
	...
	if (imageMode == XvicBW) {
		vicImageData.bw_pixels = ((unsigned char*)V_DATA(imageData)) + (imageWidth * cb->y) + cb->x;
	} else {
		vicImageData.red_pixels = ((unsigned char*)V_DATA(imageData)) + (imageWidth * cb->y) + cb->x;
		vicImageData.grn_pixels =
		    ((unsigned char*)V_DATA(imageData)) + (imageWidth * (imageHeight + cb->y)) + cb->x;
		vicImageData.blu_pixels =
		    ((unsigned char*)V_DATA(imageData)) + (imageWidth * (imageHeight * 2 + cb->y)) + cb->x;
	}
...
static void handleInput(Widget widget, XtPointer clientData, XtPointer callData)
{
	...
	dbgprintf(
	    "handleInput(widget = %ld, clientData = %ld,"
	    "callData = %ld)\n",
	    widget, clientData, callData);
	...
	if (cb->input_num_params > 0) {
			...
			if (strcmp(cb->input_params[0], "Draw") == 0) {
				...
				if (cb->input_num_params > 1) {
					if (strcmp(cb->input_params[1], "start") == 0) {
						...
						drawSelection(widget, vicarData, vicarData->rubberBandGC, cb->x_fp, cb->y_fp, True);
						if (!cb->on_screen) {
							/* Setup auto-pan. */
							...
							timerData.y      = cb->y;
							/* FIX: #define for 500 */
							vicarData->timeoutId = XtAppAddTimeOut(XtWidgetToApplicationContext(widget),
						...
						drawSelection(widget, vicarData, vicarData->overlayGC, cb->x_fp, cb->y_fp, False);
						/* Trigger a pseudo-callback. */
		...
		default: parse_error("Internal error: invalid VICAR inputMode."); break;
		}
	}
}

static XvicGC createNewGC(Widget widget, int rubber)
{
	XvicGC gc;
	XGCValues values;
...
static void drawSelection(Widget widget, VicarData* vicarData, XvicGC gc, double x, double y, int rubberBand)
{
	XvicID objectId;
	double height, width;
	double xx, yy;
	...
	dbgprintf(
	    "drawSelection (graphics = %ld, vicarData = %ld, "
...
static void autoPan(XtPointer clientData, XtIntervalId* id)
{
...
void gui_getVicarPseudoResources(Widget widget, Var* dvStruct)
{
...
void gui_setVicarPseudoResources(Widget widget, Var* dvStruct, Narray* publicResources)
{
...
#if DEBUG
	fprintf(stderr,
	        "DEBUG: gui_setVicarPseudoResources(widget = %ld, "
...
Narray* gui_getVicarPublicResources()
{
	Narray* resList;
	int i, num;
...
static Var* convertToBSQ(Var* from)
{
	Var* new;
...
void gui_destroyVicar(Widget widget, void* instanceData)
{
	VicarData* vicarData;
