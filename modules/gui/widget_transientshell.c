...
int gui_isTransientShell(const char* name)
{
...
Widget gui_initTransientShell(const char* dvName, WidgetClass class, Widget parent, Var* dvResources,
                              void** instanceData, Narray* publicResources, Widget* outerWidget)
{
...
#if DEBUG
	fprintf(stderr,
	        "DEBUG: gui_initTransientShell(dvName = \"%s\", class = %ld, "
	        "parent = %ld, instanceData = %ld)\n",
	        dvName, class, parent, instanceData);
#endif
	...
	XtSetArg(xtArgs[xtArgCount], XmNscreen, DefaultScreenOfDisplay(XtDisplay(parent)));
	xtArgCount++;

	shell = XtCreatePopupShell(dvName, transientShellWidgetClass, parent, xtArgs, xtArgCount);
...
Narray* gui_getTransientShellPublicResources()
{
