...
static const char* lineBoxPublicResources[] = {
    LBoxNlineMode, LBoxNleftMask, LBoxNrightMask, LBoxNshowLeftMask, LBoxNshowRightMask,
    LBoxNhistXMin, LBoxNhistXMax, LBoxNhistYMin,  LBoxNhistYMax};
...
int gui_isLineBox(const char* name)
{
...
WidgetClass gui_getLineBoxClass(void)
{
...
CallbackList gui_getLineBoxCallbacks(void)
{
...
Widget gui_initLineBoxWidget(const char* dvName, WidgetClass class, Widget parent, Var* dvResources,
                             void** optData, Narray* publicResources, Widget* outerWidget)
{
	/* call default init function */
	Widget lineBox =
	    gui_initDefault(dvName, class, parent, dvResources, optData, publicResources, outerWidget);
...
void gui_getLineBoxPseudoResources(Widget widget, Var* dvStruct)
{
	int i;
	for (i = 0; pseudo_resources[i].name != NULL; i++) {
		if (pseudo_resources[i].get != NULL) {
			add_struct(dvStruct, pseudo_resources[i].name, pseudo_resources[i].get(widget));
		}
	}
}
...
void gui_setLineBoxPseudoResources(Widget widget, Var* dvStruct, Narray* publicResources)
{
	int i, j;
	char* name;
	Var* value;
...
static int setHistXMin(Widget w, Var* var)
{
	if (V_TYPE(var) != ID_VAL || GetX(var) != 1 || GetY(var) != 1 || GetZ(var) != 1) {
