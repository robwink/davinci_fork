		...
		for (j = 1; j < ac; j++) {
			...
			if (V_TYPE(s) == ID_STRING) {
				...
				for (i = 0; i < V_DSIZE(v); i++) {
					...
					case BYTE:
					case SHORT:
					case INT: fprintf(fp, "%d", extract_int(v, i)); break;
					case FLOAT:
					case DOUBLE: fprintf(fp, "%.12g", extract_double(v, i)); break;
					}
					if (((i + 1) % s0) == 0) {
						fputc('\n', fp);
					} else {
				...
				for (i = 0; i < Ord[Mode[2]]; i++) {
					for (j = 0; j < Ord[Mode[1]]; j++) {
						...
						for (k = 0; k < Ord[Mode[0]]; k++) {
							...
							case FLOAT:
							case DOUBLE: y[k] = extract_float(v, obj_index);
							}
							...
							if (xFlag) {
								...
								case INT:
									...
									break; // drd added -- break was missing
								case FLOAT:
								case DOUBLE: x[k] = extract_float(Xaxis, rpos(obj_index, v, Xaxis));
								}
							} else {
								x[k] = (float)k;
							}
