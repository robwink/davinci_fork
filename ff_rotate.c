...
Var* rotation(Var* obj, float angle, float ign)
{
	...
	switch (V_FORMAT(obj)) {
	case DV_UINT8: ign_resized.b  = ign; break;
	case DV_INT16: ign_resized.s  = ign; break;
	case DV_INT32: ign_resized.i  = ign; break;
	case DV_FLOAT: ign_resized.f  = ign; break;
	case DV_DOUBLE: ign_resized.d = ign; break;
	default: break;
	}
	...
	for (i = 0; i < xdim_shr2; i++) {
		yshear[i] = rot_round((float)i * sin(ang));
	}
	for (j = 0; j < ydim_shr1; j++) {
		xshear[j] = rot_round((float)j * -1 * tan(ang / 2.0));
	}
	...
	for (k = 0; k < rx * ry * z; k++) {
		memcpy(robj + k * nbytes, &ign_resized, nbytes);
	}
				...
				for (k = 0; k < z; k++) {
					...
					if (tf != ign) {
						memcpy(((char*)V_DATA(s)) + cpos(x_2 - lef, y_1 - top, k, s) * nbytes,
						       ((char*)V_DATA(obj)) + cpos(xx, yy, zz, obj) * nbytes, nbytes);
					}
				}
			}
		}
