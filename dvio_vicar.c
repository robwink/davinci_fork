...
Var* dv_LoadVicar(FILE* fp, char* filename, struct iom_iheader* s)
{
	...
	if (iom_GetVicarHeader(fp, filename, &h) == 0) {
		return (NULL);
	}
	...
	sprintf(hbuf, "%s: VICAR %s image: %dx%dx%d, %d bits", filename, iom_Org2Str(h.org),
...
int dv_WriteVicar(Var* ob, char* filename, int force)
{
	struct iom_iheader h;
	int status;
...
int dv_LoadVicarHeader(FILE* fp, char* filename, int rec, char* element, Var** var)
{
	Var* v = NULL;
	char *p, *q, *str;
	int s = 0;
	...
	if ((q = get_value(p, element)) != NULL) {
		 ...
		 **/
		if (!strcasecmp(element, "lblsize") || !strcasecmp(element, "bufsize") ||
		    !strcasecmp(element, "dim") || !strcasecmp(element, "recsize") ||
		    !strcasecmp(element, "nl") || !strcasecmp(element, "ns") ||
		    !strcasecmp(element, "nb") || !strcasecmp(element, "n1") ||
		    !strcasecmp(element, "n2") || !strcasecmp(element, "n3") ||
		    !strcasecmp(element, "n4") || !strcasecmp(element, "nbb") ||
