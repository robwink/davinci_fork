...
} rlist[] = {{"scale", RV_set_int},
             {"verbose", RV_set_int},
             {"debug", RV_set_int},
             {"depth", RV_set_int},
             {NULL, NULL}};

int is_reserved_var(char* name)
{
	struct _rlist* r;
...
Var* set_reserved_var(Var* id, Var* range, Var* exp)
{
	struct _rlist* r;
	for (r = rlist; r->name != NULL; r++) {
...
Var* RV_set_int(Var* id, Var* range, Var* exp)
{
	if (exp == NULL) return NULL;

	if (V_TYPE(exp) != ID_VAL || V_FORMAT(exp) != INT || V_DSIZE(exp) != 1) {
		sprintf(error_buf, "Improper value for reserved variable: %s", V_NAME(id));
		parse_error(NULL);
		return NULL;
	}
