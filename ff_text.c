...
Var* ff_read_text(vfuncptr func, Var* arg)
{
	...
	while (dv_getline(&ptr, fp) != EOF) {
		if ((int)strlen(ptr) > x) x = (int)strlen(ptr);
		y++;
...
Var* ff_delim(vfuncptr func, Var* arg)
{
	...
	if (V_TYPE(ob) == ID_TEXT) return (ff_delim_textarray(ob, item, delim));
...
char* string_dirname(Var* ob1)
{
	...
	if (V_STRING(ob1) == NULL) return (NULL);
...
int list_contains(char* str, Var* S)
{
	...
	for (i = 0; i < V_TEXT(S).Row; i++)
		if (strcmp(str, V_TEXT(S).text[i]) == 0) return 1;
...
Var* minus(Var* ob1, Var* S)
{
	...
	for (i = 0; i < V_TEXT(ob1).Row; i++)
		if (list_contains(V_TEXT(ob1).text[i], S) == 0) count++;
...
Var* set_text(Var* to, Range* r, Var* from)
{
	...
	for (i = lo[1]; i <= hi[1]; i += step[1]) {
		...
		if (lo[0] >= cur_line_leng && lo[0] != 0) continue; /*Skip it */
		tmp_hi = min(hi[0], (cur_line_leng - 1));
		length = (tmp_hi - lo[0] + 1);
		free(V_TEXT(to).text[i]);
		V_TEXT(to)
		    .text[i] = (char*)calloc(string_length + cur_line_leng - length + 1, sizeof(char));
		memcpy(V_TEXT(to).text[i], V_TEXT(dest).text[i], lo[0]);
		memcpy((V_TEXT(to).text[i] + lo[0]), string, string_length);
		memcpy((V_TEXT(to).text[i] + lo[0] + string_length), (V_TEXT(dest).text[i] + tmp_hi + 1),
		       (cur_line_leng - tmp_hi - 1));
		V_TEXT(to).text[i][lo[0] + string_length + (cur_line_leng - tmp_hi - 1)] = '\0';
	}
...
Var* ff_create_text(vfuncptr func, Var* arg)
{
	...
	if (count < 0) {
		parse_error("Invalid argument to %s.  %s must be greater than 0.", func->name, alist[0].name);
	}
