...
static const char DV_MOD_PATH_DEF[] = DV_MOD_PATH;
...
void del_module(Var* v)
{
	...
	if (m->stage >= MOD_LST_ADDED) {
		...
		if (v->name) {
			remove_from_list_of_loaded_modules(v);
		}
	}
	...
	if (m->functions) {
		Narray_free(m->functions, (Narray_FuncPtr)free_module_function);
	}
	if (m->path) {
		free(m->path);
	}
	if (m->handle) {
		close_dv_module_file(m->handle);
	}
}
...
static int uninit_dv_module(dvModule* m /* module to be uninitialized */
                            )
{
...
static char* extract_dv_mod_ver_from_fname(const char* mod_name, char* mod_file_name)
{
	char* p;
...
static struct dirent* find_next_file_with_prefix(DIR* d, const char* pfx)
{
	struct dirent* de;
...
static int cmp_mod_ver(const char* v1, const char* v2)
{
...
*/
// path: A ":" separated list of dirs to find the name in  */
static int get_module_versions(const char* mod_name, char* path, cvector_str* modules)
{
#define MAX_STR_LEN 1024
	...
	if ((strsz = snprintf(name, 1024, "%s.%s", mod_name, DVM_EXT)) >= MAX_STR_LEN) {
		parse_error("Module name is too long (limit 1023 chars):\n%s.%s\n", dir_name, fname);
		return 0;
	}
	for (q = path; (dir_name = strtok(q, PATH_SEPARATOR)); q = NULL) {
		...
		if (d == NULL) {
			...
			if (debug > DEBUG_THRESHOLD) {
				parse_error("Unusable dir %s in path. Reason: %s.", dir_name, strerror(errno));
			}
			continue;
		}
		...
		while ((de = find_next_file_with_prefix(d, name))) {
			fname = de->d_name;
			if ((strsz = snprintf(fullpath, 1024, "%s/%s", dir_name, fname)) >= MAX_STR_LEN) {
				parse_error("Module path is too long (limit 1023 chars):\n%s/%s\n", dir_name, fname);
				break;
			}
			cvec_push_str(modules, fullpath);
		}
	}
	closedir(d);
	return modules->size;
...
// ver == NULL get latest
// offset same as above function if ver != NULL
static char* locate_dv_module(const char* name, const char* ver, int offset)
{
	...
	if (dv_mod_path == NULL) {
		if (debug > DEBUG_THRESHOLD) {
			parse_error("%s env is not set using default path %s.", DV_MOD_PATH_ENV, DV_MOD_PATH_DEF);
		}
		...
		if (debug > DEBUG_THRESHOLD) {
			parse_error("%s env found as being set to %s.", DV_MOD_PATH_ENV, dv_mod_path);
		}
	...
	if (loc) {
		parse_error("Loading module %s from %s.", name, loc);
	} else {
		parse_error("Module %s not found in path %s.", name, dv_mod_path_save);
	}
...
/* name of the module being loaded < mod_fname */
/* module version = NULL for "unspecified/any" */
// offset see above
static int load_dv_module(char* mod_name, char* mod_ver, int offset, dvModule* mod)
{
	dvModuleHandle* mh;
	...
	if (mod->handle == NULL) {
		parse_error("Unable to open module file %s. See previous messages.", mod->path);
		return 0;
	}
	...
	if (!init_dv_module(mod)) {
		parse_error("Module initialization failed for %s. See previous messages. ", mod->name);
		// next three lines are taken care of in del_module() as well
		// close_dv_module_file(mod->handle); mod->handle = NULL;
		// free(mod->name); mod->name = NULL;
		// free(mod->path); mod->path = NULL;
	...
	*/
	...
	if (!autoload_dep(mod->init_stuff.dep, mod->init_stuff.ndep)) {
		parse_error(
		    "Atleast one of the module dependencies "
...
static int dv_module_ver_matches_reqd_ver(const char* candidate, const char* reqd)
{
	...
	for (i = 0; i < ndep; i++) {
		if (!(m = search_in_list_of_loaded_modules(dep[i].name))) {
			if (!autoload_dv_module(dep[i].name, dep[i].ver)) {
				...
				parse_error("Unable to load %s ver:%s.", dep[i].name, printable_ver_str(dep[i].ver));
			...
			if (!dv_module_ver_matches_reqd_ver(V_MODULE(m).ver, dep[i].ver)) {
				...
				parse_error("%s v:%s already loaded, cannot load %s v:%s. ", m->name,
				            printable_ver_str(V_MODULE(m).ver), dep[i].name,
				            printable_ver_str(dep[i].ver));

				parse_error("Unload %s ver:%s first.", m->name, printable_ver_str(V_MODULE(m).ver));
...
static int init_dv_module(dvModule* m)
{
	...
	init_func = (dvModuleInitFunc)locate_dv_module_func_in_slib(m->handle, DV_MODULE_INIT_FUNC_NAME);
	if (init_func == NULL) {
...
static vfuncptr make_vfuncptr(char* func_name, vfunc func_ptr, void* func_data)
{
	vfuncptr fptr;
...
static int load_dv_module_functions(dvModule* m)
{
	...
	for (i = 0; i < nfdesc; i++) {
		...
		*/
		if (func_ptr == NULL) {
			parse_error("%s not found in %s. Reason: %s.", func_name, m->name, strerror(errno));
			continue;
		}
#else
		...
		*/
		...
		if (!(rval = add_func_to_module(m, fptr))) {
			parse_error("Error adding %s() to the %s.", fptr->name, mod_name);
			free(fptr);
		} else {
			if (rval != fptr) {
				parse_error("Duplicate definition of %s in %s.", fptr->name, mod_name);
				free(rval);
			} else {
				if (debug > DEBUG_THRESHOLD) {
...
void module_help(char* module, char* keyword)
{
	char* path              = NULL;
