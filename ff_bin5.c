...
static int typeSizes[B5WT_ULONGLONG + 1] = {0, 1, 2, 4, 4, 8, 0, 0, 0, 0, 0, 0, 2, 4, 8, 8};

static int internalTypes[B5WT_ULONGLONG + 1] = {0, BYTE, SHORT, INT, FLOAT, DOUBLE, 0, 0,
                                                0, 0,    0,     0,   SHORT, INT,    0, 0};

typedef enum { ARCH_X86 = 1, ARCH_SUN = 2 } archIDs; /* encoded architecture ids */

char arch_strs[2][6] = {"x86  \0", "sun  \0"};

int arch_ids[2] = {ARCH_X86, ARCH_SUN};
...
static int get_arch_id(const char* arch)
{
	int i, n;
...
static int read_bin5_header(const char* fname, B5H** b5h)
{
	FILE* fp             = NULL;
	char* buff           = NULL;
	int buff_sz          = 1;
	...
	} while (strncmp(&buff[buff_sz - 1 - strlen(lbl_end_marker)], lbl_end_marker,
	                 strlen(lbl_end_marker)) != 0);
...
static size_t cpos_n(int n, int* dim, int* pos)
{
	size_t cpos = 0;
	int i;
...
Var* ff_load_bin5(vfuncptr func, Var* arg)
{
	B5H* bh       = NULL;
	Var** structs = NULL;
		...
		for (pos[n - 3] = 0; pos[n - 3] < dim[n - 3]; pos[n - 3]++) {
			for (pos[n - 2] = 0; pos[n - 2] < dim[n - 2]; pos[n - 2]++) {
				for (pos[n - 1] = 0; pos[n - 1] < dim[n - 1]; pos[n - 1]++) {
					...
					if (bh->arch_id == ARCH_X86) {
						byte_swap(item_buff, item_bytes);
					}
#else
					if (bh->arch_id == ARCH_SUN) {
						byte_swap(item_buff, item_bytes);
					}
