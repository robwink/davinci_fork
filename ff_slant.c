...
Var* ff_unslant(vfuncptr func, Var* arg)
{
	...
	if (find_struct(obj, "data", &data) == -1 || find_struct(obj, "leftedge", &leftedge) == -1 ||
...
Var* ff_unslant_shear(vfuncptr func, Var* arg)
{
	...
	parse_error("elements computed: %i  s: %f  l: %i lsize: %i array_size: (%i,%i,%i)", elements, s,
	            l, lsize, w, (y - abs(lsize)), z);
	if (elements <= 0) {
