...
static void clean_table(Symtable* s)
{
	Symtable* t;
...
static void clean_stack(Scope* scope)
{
	Stack* stack = scope->stack;
	Var* v;
...
static Var* mem_claim_struct(Var* v)
{
	int i;
	int count;
	Var* data;
...
static void mem_free(Scope* scope)
{
	if (scope->tmp) Darray_free(scope->tmp, (Darray_FuncPtr)free_var);
}

static void unload_symtab_modules(Scope* scope)
{
