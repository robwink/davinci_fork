...
Var* dv_LoadENVI(FILE* fp, char* filename, struct iom_iheader* s)
{
	...
	*/
	...
	sprintf(hbuf, "%s: ENVI %s image: %dx%dx%d, %d bits", filename, iom_Org2Str(h.org),
...
int dv_WriteENVI(Var* ob, char* filename, int force)
{
	struct iom_iheader h;
	int status;
...
int dv_LoadENVIHeader(FILE* fp, char* filename, int rec, char* element, Var** var)
{
	Var* v = NULL;
	char *p, *q, *str;
	int s = 0;
