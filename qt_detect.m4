...
dnl Stolen shamelessly for Davinci by Randy Kaelber <randy@mars.asu.edu>, it's all
dnl GPLv2, so it's all good. ;-)
dnl Changes:
dnl    - fixed library guessing to include some common 64-bit lib names
dnl    - added a reason argument to explain why a non-Qt program may be requiring
dnl      Qt
...
dnl            [ACTION-IF-FOUND [, ACTION-IF-NOT-FOUND [, QT_REQUIRED_FOR_REASON]]])
AC_DEFUN([AM_PATH_QT],
[
AC_CHECKING([for Qt ...])
...
GUESS_QT_LIB_DIRS="$QTDIR/lib $QTDIR/lib64 /usr/lib /usr/lib64 /usr/local/lib /usr/local/lib64 /usr/X11R6/lib /usr/local/qt/lib /usr/lib/qt/lib /usr/lib/qt4/lib /usr/lib/qt-4/lib /usr/lib/qt-4.0/lib /usr/lib/qt-4.1/lib /usr/lib/qt-4.2/lib /usr/lib/qt-4.3/lib /usr/lib/qt-4.4/lib /sw/lib"
...
then
  ...
  qt_ld_flag="-lQtCore -lQtGui -lQtSql"
else
  ...
  do
    ...
    then
      qt_libdir=$dir
      qt_ld_flag="-lQtCore4 -lQtGui4 -lQtSql4"
      AC_MSG_RESULT([assuming $dir])
      break
    fi
    ...
    then
      qt_libdir=$dir
      qt_ld_flag="-F$dir -framework QtCore -framework QtGui -framework QtSql"
      AC_MSG_RESULT([using frameworks, assuming $dir])
      break
    fi
  done
...
then
  AC_MSG_CHECKING(for qt - version >= $min_qt_version required for $5)
