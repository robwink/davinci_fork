...
#ifndef GPLOT_CMD
#define GPLOT_CMD                                                                  \
	"gnuplot -xrm 'gnuplot*background:black' -xrm 'gnuplot*axisColor:white' -xrm " \
...
static int plot_chopper(Var** av, int start_ct, int end_ct, int Onum, char* CommandBuffer,
                        Var* Xaxis, char* Axis, double* globalNums, char* Style, char* dir);
...
static int name_check(const char* actual_input, const char* name, int limit)
{
	...
	if (inputlen > namelen)
		return (-1);
	else if (inputlen == namelen)
		return (strcmp(actual_input, name));
	else
		return (strncmp(actual_input, name, inputlen));
}
...
Var* ff_vplot(vfuncptr func, Var* arg)
{
	...
	/* 9/14/2008 version:
	  ...
	  -added xerrorlines, yerrorlines, xyerrorlines to permissible styles
	   ...
	   Neditation version:
	  -Fixed a bug with errorbars that was pretty important.  All code changed was in
	  make_errorbar_indices()
	...
	for (i = 1; i < ac; i += 1) {
		...
		if (V_TYPE(av[i]) == ID_KEYWORD) {
			...
			if (!name_check(V_NAME(av[i]), "Xaxis", 1)) {
				...
				if (Axis != NULL && strcasecmp(Axis, "x") && strcasecmp(Axis, "y") &&
				    strcasecmp(Axis, "z")) {
				...
				if (V_KEYVAL(av[i]) != NULL) {
					...
					if (name_check(Style, "lines", 1) && name_check(Style, "linespoints", 6) &&
					    ...
					    name_check(Style, "boxes", 1) && name_check(Style, "steps", 1) &&
					    name_check(Style, "impulses", 1) && name_check(Style, "xerrorbars", 7) &&
					    name_check(Style, "yerrorbars", 7) && name_check(Style, "xyerrorbars", 8) &&
					    name_check(Style, "xerrorlines", 7) && name_check(Style, "yerrorlines", 7) &&
					    name_check(Style, "xyerrorlines", 8)) {
						parse_error("Invalid designation for Style");
						parse_error(
						    "Only: lines, points, dots, linespoints, boxes, steps, impulses,");
	...
	** each object and add to the CommandBuffer.                           */
	...
	while (end_ct < ac && start_ct < ac) {
		...
		plotchoppererror = plot_chopper(av, start_ct, end_ct, Onum, CommandBuffer, Xaxis, Axis,
		                                globalNums, Style, dir);
		j += 1;

		// There was an error somewhere in plot_chopper()
		if (plotchoppererror == 1) return (NULL);
	...
	if (gcommand != 0) printf("\n%s\n", CommandBuffer);

	free(av);
	return (NULL);
}

int plot_chopper(Var** av, int start_ct, int end_ct, int Onum, char* CommandBuffer, Var* Xaxis,
                 char* Axis, double* globalNums, char* Style, char* dir)
{
			...
			if (V_TYPE(s) == ID_STRUCT) {
				...
				if (v != NULL) {
					if (V_TYPE(v) == ID_VAL) {
						...
						parse_error(
						    "xaxis must be a numeric array. Continuing with default xaxis.\n");
					}
					v = NULL;
				}
				...
				if (v != NULL) {
					if (V_TYPE(v) == ID_STRING) {
						...
						if (axis_i != NULL && strcasecmp(axis_i, "x") && strcasecmp(axis_i, "y") &&
						    strcasecmp(axis_i, "z")) {
							parse_error("Invalid axis designation. x y z only.");
							return (1);
						}
				...
				if (v != NULL) {
					if (V_TYPE(v) == ID_VAL)
						...
						parse_error(
						    "errorbars must be a numeric array. Continuing without errorbars.\n");
					}
					v = NULL;
				}
				...
				if (v != NULL) {
					if (V_TYPE(v) == ID_STRING) {
						...
						parse_error(
						    "label must be a STRING or TEXT. Continuing with default label.\n");
					}
					v = NULL;
				}
				...
				if (v != NULL) {
					if (V_TYPE(v) == ID_STRING) {
						...
						if (name_check(smooth, "bezier", 1) && name_check(smooth, "csplines", 1)) {
						...
						parse_error(
						    "smooth must be string \'bezier\' or \'csplines\'. Continuing without "
						    "smooth.\n");
					}
					v = NULL;
				}
	...
	if (globalNums[10]) color = globalNums[11];

	if (globalNums[12]) width = globalNums[13];

	if (globalNums[14]) sep = 1;

	if (globalNums[15]) offset = globalNums[15];
	...
	if (V_TYPE(s) == ID_VAL) {
		...
		if (style) {
			if (name_check(style, "xerrorbars", 7) && name_check(style, "yerrorbars", 7) &&
			    name_check(style, "xyerrorbars", 8) && name_check(style, "xerrorlines", 7) &&
			    name_check(style, "yerrorlines", 7) && name_check(style, "xyerrorlines", 8)) {
				errorb = NULL;
			}
		}
		...
		if (errorb) {
			...
			if (i >= 2) Err2 = calloc(Ord[Mode[0]], sizeof(double));
			if (i >= 3) {
		...
		for (i = 0; i < Ord[Mode[2]]; i++) {
			for (j = 0; j < Ord[Mode[1]]; j++) {
				...
				for (k = 0; k < Ord[Mode[0]]; k++) {
					...
					case BYTE:
					case SHORT:
					case INT: y[k] = (double)extract_int(v, obj_index); break;
					case FLOAT:
					case DOUBLE: y[k] = extract_double(v, obj_index);
					}
					...
					if (xFlag) {
						...
						case INT:
							...
							break;
						case FLOAT:
						case DOUBLE: x[k] = extract_double(xaxis_i, rpos(obj_index, v, xaxis_i));
						}
					} else {
						x[k] = (double)k;
					}
					...
					sef = (ixaflag != 0 && ixbflag != 0 && x[k] > ignore_x_above && x[k] < ignore_x_below)
					          ? sef + 1
					          : sef;

					// places a break in the plot if ignore_x block occurs between data points
					sef2 = 0;
					sef2 = (k > 0 && ixaflag + ixbflag == 2 && x[k - 1] < ignore_x_above && x[k] > ignore_x_below)
					           ? sef2 + 1
					           : sef2;
					sef2 = (k > 0 && ixaflag + ixbflag == 2 && x[k - 1] > ignore_x_below && x[k] < ignore_x_above)
					           ? sef2 + 1
					           : sef2;
					...
					if (sef) {
						...
						if (errorb == NULL) {
							...
							case BYTE:
							case SHORT:
							case INT: fprintf(fp, "%d\t %d\n", (int)x[k], (int)yval); break;
							case FLOAT:
							case DOUBLE: fprintf(fp, "%f\t %f\n", x[k], yval);
							}
							...
							if (Err1 && Err2 == NULL) {
								...
								case INT:
									...
									break;
								case FLOAT:
								case DOUBLE: fprintf(fp, "%f\t %f\t %f\t %f\n", x[k], yval, e1, e2);
								}
				...
				if (sep) {
					...
					if (label != NULL)
						...
						sprintf(CommandBuffer + strlen(CommandBuffer), " title 'Obj %d vector %d'",
						        Onum, vnum);
					...
					// Add width
					if (width > 0) sprintf(CommandBuffer + strlen(CommandBuffer), " lw %d", width);
		...
		if (!(sep)) {
			...
			// Add color
			if (color > -2) sprintf(CommandBuffer + strlen(CommandBuffer), " lt %d", color);

			// Add width
			if (width > 0) sprintf(CommandBuffer + strlen(CommandBuffer), " lw %d", width);

			// Add style
			if (style != NULL) sprintf(CommandBuffer + strlen(CommandBuffer), " with %s", style);
...
static int handle_errorbars(Var* errorb, Var* v, int* Mode, char* style, int Onum)
{
	...
	if (e_dim[0] == 1 && e_dim[1] == 1 && e_dim[2] == 1) {
		if (!name_check(style, "xyerrorbars", 2)) {
			parse_error(
			    "You only provided a single error value for a style that requires 2 "
...
int* make_errorbar_indices(Var* v, int* Mode, int* CE, Var* errorb)
{
	...
	erroraxis                          = Mode[1];
	if (v_dim[Mode[1]] != 1) erroraxis = Mode[2];
...
Var* ff_ploop(vfuncptr func, Var* arg)
{
