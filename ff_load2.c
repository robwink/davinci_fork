...
#if 0
Var *
LoadMedley(FILE *fp, char *fname, struct _iheader *slice)
{
	int success;
	struct _iheader h;  /* header as loaded from file */
    Var *v = NULL;
    void *data = NULL;  /* actual data read           */

	success = LoadHeader(fp, fname, &h);
	if (success){
        PrintImageHeader(stderr, fname, &h);

		/* merge header info and slice info */
		MergeHeaderAndSlice(&h, slice);

        if (supports_read_qube_data(&h)){
            /* read the sub-selected data */
            data = read_qube_data(fileno(fp), &es);
        }
        else {
            data = detach_iheader_data(&h);
	...
	/* cleanup memory allocated within iheader */
    cleanup_iheader(&h);
