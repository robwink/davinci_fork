...
typedef enum { TINT, TREAL, TSTR, TNA } coltype;
...
static void fill_col_min_size(coldef* cols, int n)
{
	...
	for (i = 0; i < n; i++) {
		...
		case TREAL: cols[i].size = sizeof(double); break;
...
static void fill_pseudo_symbols(const char* fdelim, const char* rdelim, int psyms[257])
{
	unsigned int i;
	psym_cat c;
...
static vlexc* new_vlexc(char* data, off_t len, const char* fs, const char* rs, int zflag)
{
	vlexc* t;
...
static void free_vlexc(vlexc* h)
{
	free(h);
}

static void reset_vlex(vlexc* h)
{
	h->q = h->data;
}
...
*/
static vlextokt vlex_next_tok(vlexc* h,  /* vanilla lexer context */
...
*/
static int vpass1(vlexc* h, coldef** cols, int* ncols)
{
	vlextokt t;
	char *p, *q;
...
*/
static int davinci_type(coltype c, /* column type: TINT, TREAL, ... */
                        ...
                        )
{
	...
	case TINT:
		switch (csize) {
		case 1: dav_type  = DV_UINT8; break;
...
**    line[i].
*/
static int vpass2(void* data, off_t len, coldef** fields, int nfields, int nrecs, void** data_ptrs)
{
	int i, j, maxlen = 0;
	char* buff  = NULL;
	char** vals = NULL;
			...
			*/
			for (j = 0, t = fields[i]; t; t = t->next, j++) {
				...
				case TINT:
					...
					case DV_UINT8: byte_data[i][idx] = atoi(vals[t->sno]); break;

					case DV_INT16: short_data[i][idx] = atoi(vals[t->sno]); break;

					default:
					case DV_INT32: int_data[i][idx] = atoi(vals[t->sno]); break;
					}
					break;
				...
				case TREAL:
					...
					switch (fields[i]->dav_type) {
					case DV_FLOAT: float_data[i][idx] = atof(vals[t->sno]); break;

					default:
					case DV_DOUBLE: double_data[i][idx] = atof(vals[t->sno]); break;
					}
					break;
...
*/
static int vanread(const char* filename, const char* fdelims, const char* rdelims, Var** v_return)
{
	int fd;
	int len;
...
***************************************************************************/
Var* ff_loadvan(vfuncptr func, Var* args)
{
	...
	*coldefs = (coldef*)calloc(n, sizeof(coldef));
	...
	for (i = 0; i < n; i++) {
		...
		*/
		...
		sprintf((*coldefs)[i].text, "%*.*s", cnamelen - 1, cnamelen - 1, colnames[i * 2]);
