			...
			*/
			for (j = 0, t = fields[i]; t; t = t->next, j++) {
				...
				case TINT:
					...
					case BYTE: byte_data[i][idx] = atoi(vals[t->sno]); break;

					case SHORT: short_data[i][idx] = atoi(vals[t->sno]); break;

					default:
					case INT: int_data[i][idx] = atoi(vals[t->sno]); break;
					}
					break;
				...
				case TREAL:
					...
					switch (fields[i]->dav_type) {
					case FLOAT: float_data[i][idx] = atof(vals[t->sno]); break;

					default:
					case DOUBLE: double_data[i][idx] = atof(vals[t->sno]); break;
					}
					break;
