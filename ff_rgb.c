...
Var* ff_rgb(vfuncptr func, Var* arg)
{
	...
	for (i = 0; i < 3; i++) {
		...
		if (V_ORG(v[i]) != V_ORG(v[0])) {
			parse_error("%s: Image planes must have the same org", func->name);
			return (NULL);
		}
		if (V_SIZE(v[i])[0] != V_SIZE(v[0])[0]) {
			parse_error("%s: Image planes must be the same size", func->name);
			return (NULL);
		}
		if (GetBands(V_SIZE(v[i]), V_ORG(v[i])) != 1) {
			parse_error("%s: Image planes must have depth=1", func->name);
			return (NULL);
		}
	}
