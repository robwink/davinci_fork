	...
	for (k = 0; k < z; k++) {
		...
		for (j = 0; j < y; j++) {
			...
			for (i = 0; i < x; i++) {
				...
				switch (format) {
				case BYTE: cdata[count++]   = saturate_byte(strtol(ptr, NULL, 10)); break;
				case SHORT: sdata[count++]  = saturate_short(strtol(ptr, NULL, 10)); break;
				case INT: idata[count++]    = saturate_int(strtol(ptr, NULL, 10)); break;
				case FLOAT: fdata[count++]  = strtod(ptr, NULL); break;
				case DOUBLE: ddata[count++] = strtod(ptr, NULL); break;
				}
				ptr = NULL;
			}
		}
