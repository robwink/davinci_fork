...
static int extract_data_band_as_bsq(Var* v, int band_no, void* buff)
{
	...
	for (y = 0; y < GetY(v); y++) {
		for (x = 0; x < GetX(v); x++) {
			...
			switch (V_FORMAT(v)) {
			case BYTE: byte_buff[idx++]     = (unsigned char)extract_int(v, pos); break;
			case SHORT: short_buff[idx++]   = (short)extract_int(v, pos); break;
			case INT: int_buff[idx++]       = extract_int(v, pos); break;
			case FLOAT: float_buff[idx++]   = extract_float(v, pos); break;
			case DOUBLE: double_buff[idx++] = extract_double(v, pos); break;
			default:
				fprintf(stderr,
...
static int propagate_keywords_0(int fid, Var* s, char* object, char* group)
{
	...
	for (i = 0; i < n; i++) {
		...
		case ID_VAL: /* numeric array or a single value */
			...
			case BYTE:
			case SHORT:
			case INT: propagate_int_keyword(fid, object, group, name, v); break;

			case FLOAT: propagate_float_keyword(fid, object, group, name, v); break;

			case DOUBLE: propagate_double_keyword(fid, object, group, name, v); break;
...
static int propagate_history_group_keywords(Var* s, const char* group, int indent, int level)
{
	...
	for (i = 0; i < n; i++) {
		...
		case ID_VAL:
			...
			case BYTE:
			case SHORT:
			case INT: propagate_history_int_keyword(indent, name, v); break;
			case FLOAT:
			case DOUBLE: propagate_history_real_keyword(indent, name, v); break;
			default:
