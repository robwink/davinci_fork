...
static void* dv_read_qube_suffix(int fd, struct iom_iheader* h, int s_bytes, int plane_number,
                                 int s_item_byte_e, /* external data type as stored in file */
                                 int* s_item_byte,  /* internal representation in memory */
                                 int ordinate, size_t* size);
...
static int convert_data(int from_type, void* from_data, int to_type, void* to_data, int nelements);
static void get_suffix_buff_sizes(int core_items[3], int suffix_items[3], int suffix_item_sizes[3]);
...
//
// like ff_read_suffix_plane has way too much duplication of code in dv_LoadISISSuffixesFromPDS_New()
...
Var* write_PDS_Qube(Var* core, Var* side, Var* bottom, Var* back, FILE* fp)
{
	...
	if (suffix[0] == NULL && suffix[1] == NULL &&
	...
	for (i = 0; i < 3; i++) {
		if (suffix[i] != NULL) {
			for (j = 0; j < get_struct_count(suffix[i]); j++) {
				...
				switch (i) {
				case 0: /* side */
					if (size[1] != suf_size[1] || size[2] != suf_size[2] || suf_size[0] != 1)
						error++;
					break;
				case 1: /* bottom */
					if (size[0] != suf_size[0] || size[2] != suf_size[2] || suf_size[1] != 1)
						error++;
					break;

				case 2: /* back */
					if (size[0] != suf_size[0] || size[1] != suf_size[1] || suf_size[2] != 1)
						error++;
					break;
				}
				if (error) {
					parse_error(
					    "%s: Suffix plane %s does not match core size: "
					    "%d [%dx%dx%d] vs %dx%dx%d\n",
					    func_name, name, suf_size[0], suf_size[1], suf_size[2], size[0], size[1],
					    size[2]);
					return (NULL);
				}
			}
...
Var* dvLoadQube(FILE* fp, char* fn, int dptr, int org, int items[], int itemBytes, char* itemType,
                double coreBase, double coreMult, int suffixItems[], int suffixBytes)
{
	...
	if (strstr(itemType, "UNSIGNED")) _unsigned = 1;
	...
	*/
	...
	if (itemBytes == 2 && _unsigned) data = fix_unsigned(h, data);
...
/* Want to read label and pull out only the minimal key words
   ...
   the label will be used (classic qube label reading can get confused)
   */
static int initHeaderFromQubeLabel(OBJDESC* qube, char* fn, struct iom_iheader* h)
{
...
static int initHeaderFromHistogramLabel(OBJDESC* hist, char* fn, struct iom_iheader* h)
{
	...
	if ((key = OdlFindKwd(hist, "ITEMS", NULL, 0, scope))) size[0] = atof(OdlGetKwdValue(key));
	...
	if ((key = OdlFindKwd(hist, "OFFSET", NULL, 0, scope))) offset = atof(OdlGetKwdValue(key));
	...
	if (((key1 = OdlFindKwd(hist, "DATA_TYPE", NULL, 0, scope)) ||
		...
		format = iom_ConvertISISType(OdlGetKwdValue(key1), key2 == NULL ? NULL : OdlGetKwdValue(key2),
	...
	h->org = org;
	for (i = 0; i < 3; i++) h->size[i] = size[i];
	h->eformat                         = (iom_edf)format;
	h->offset                          = offset;
	h->gain                            = gain;
	for (i = 0; i < 3; i++) h->prefix[i] = prefix_size[i];
	for (i = 0; i < 3; i++) h->suffix[i] = suffix_size[i];
	h->corner                            = suffix[0] * suffix[1] * suffix_bytes;

	return 1;
}

static int initHeaderFromImageLabel(OBJDESC* image, char* fn, struct iom_iheader* h)
{
	KEYWORD *key, *key1, *key2;
	int size[3]        = {1, 1, 1};
	int prefix[3]      = {0, 0, 0};
	...
	if ((key = OdlFindKwd(image, "OFFSET", NULL, 0, scope))) offset = atof(key->value);
	if ((key = OdlFindKwd(image, "SCALING_FACTOR", NULL, 0, scope))) gain = atof(key->value);
...
Var* dv_LoadHistogram_New(FILE* fp, char* fn, int dptr, OBJDESC* hist)
{
	...
	if ((key1 = OdlFindKwd(hist, "DATA_TYPE", NULL, 0, scope)) ||
	    (key1 = OdlFindKwd(hist, "ITEM_TYPE", NULL, 0, scope)))
		if (strstr(OdlGetKwdValue(key1), "UNSIGNED")) _unsigned = 1;
	...
	for (i = 0; i < 3; i++) {
		...
		if (suffix[iom_orders[h.org][i]]) {
			...
			for (j = 0; j < suffix[iom_orders[h.org][i]]; j++) {
				...
				data    = dv_read_qube_suffix(fileno(fp), &h, suffix_bytes, j, eformat, &iformat,
				                           iom_orders[h.org][i], &size);
...
Var* dv_LoadISIS(FILE* fp, char* filename, struct iom_iheader* s)
{
	...
	// TODO(rswinkle) don't use open use fopen and get rid of this #if def crap
	// that ruins code folding
	if (datafile != NULL) {
#if defined(__CYGWIN__) || defined(__MINGW32__)
	...
	sprintf(hbuf, "%s: ISIS %s image: %dx%dx%d, %s (%d bits)", filename, iom_Org2Str(h.org),
	        iom_GetSamples(h.size, h.org), iom_GetLines(h.size, h.org), iom_GetBands(h.size, h.org),
...
int dv_LoadISISHeader(FILE* fp, char* filename, int rec, char* element, Var** var)
{
	...
	ob = (OBJDESC*)OdlParseLabelFptr(fp, err_file, ODL_EXPAND_STRUCTURE | ODL_EXPAND_CATALOG, VERBOSE == 0);
...
static void* dv_read_qube_suffix(int fd, struct iom_iheader* h,
                                 ...
                                 size_t* size)
{
	...
	if (ordinate == 2) {
		plane = (size_t)h->size[0] * (size_t)h->size[1] * (size_t)s_bytes +
		        h->suffix[0] * (size_t)h->size[1] + h->suffix[1] * (size_t)h->size[0] + h->corner;
		...
		offset1 = (size_t)h->size[1] * (size_t)h->size[0] * (size_t)c_bytes +
		          (size_t)h->size[1] * (size_t)h->suffix[0];
		offset2 = ((size_t)h->size[0] * (size_t)s_bytes + (size_t)h->suffix[0]) * (size_t)plane_number;
		offset3 = 0;
	}

	else {
		plane = (size_t)h->size[0] * (size_t)h->size[1] * (size_t)c_bytes +
		        (size_t)h->size[1] * (size_t)h->suffix[0];
	...
	*size = dsize;
	...
	for (z = h->s_lo[2]; z < h->s_hi[2]; z += h->s_skip[2]) {
		...
		lseek(fd, h->dptr +
		              zz * ((size_t)h->size[0] * (size_t)h->size[1] * (size_t)c_bytes +
		...
		if (ordinate == 2 && h->s_skip[0] == 1 && h->s_skip[1] == 1 && h->suffix[0] == 0 &&
		    h->suffix[1] == 0) {
			memcpy((char*)data, (char*)p_data, plane);
		} else {
			for (y = h->s_lo[1]; y < h->s_hi[1]; y += h->s_skip[1]) {
				if (ordinate == 1 && h->s_skip[0] == 1 && h->s_skip[2] == 1 && h->suffix[0] == 0) {
					memcpy((char*)data + count, (char*)p_data + offset2, h->size[0] * (size_t)s_bytes);

					count += h->size[0] * (size_t)s_bytes;
				} else {
					for (x = h->s_lo[0]; x < h->s_hi[0]; x += h->s_skip[0]) {
						memcpy((char*)data + count,
						       (char*)p_data + offset2 + y * offset3 + (x - h->s_lo[0]) * (size_t)s_bytes,
						       s_bytes);
						count += s_bytes;
					}
				}
...
static int dv_LookupSuffix(OBJDESC* qube, char* name, int* plane, int* type, struct iom_iheader* h,
                           int* suffix)
{
	KEYWORD* key;
	char** list;
...
Var* ff_read_suffix_plane(vfuncptr func, Var* arg)
{
	...
	**************************/
	...
	char *fname, **list1, **list2, fname2[256];
	...
	if ((fname = dv_locate_file(isisfile)) == NULL || (fp = fopen(fname, "rb")) == NULL) {
	...
	if (qube == NULL) qube = OdlFindObjDesc(object, "SPECTRAL_QUBE", NULL, 0, 0, 0);
	...
	if (Suffix == NULL) {
		...
		fprintf(stderr, "Core   size (Samples,Lines,Bands) = (%6d,%6d,%6d)\n",
		        iom_GetSamples(s.size, s.org), iom_GetLines(s.size, s.org), iom_GetBands(s.size, s.org));

		fprintf(stderr, "Suffix size (Samples,Lines,Bands) = (%6d,%6d,%6d)\n",
		        suffix[iom_orders[s.org][0]], suffix[iom_orders[s.org][1]],
		...
		for (i = 0; i < 3; i++) {
			...
			if (suffix[iom_orders[s.org][i]]) {
				...
				name_list_sz = OdlGetAllKwdValuesArray(key, &name_list);
				if (name_list_sz != suffix[iom_orders[s.org][i]]) {
					parse_error("suffix name list is incomplete\n");
					continue;
				}
				...
				if ((key = OdlFindKwd(qube, str2, NULL, 0, scope))) {
					size_list_sz = OdlGetAllKwdValuesArray(key, &size_list);
					if (size_list_sz != suffix[iom_orders[s.org][i]]) {
						parse_error("suffix size list is incomplete\n");
						continue;
					}
				} else {
				...
				if ((key = OdlFindKwd(qube, str2, NULL, 0, scope))) {
					type_list_sz = OdlGetAllKwdValuesArray(key, &type_list);
					if (type_list_sz != suffix[iom_orders[s.org][i]]) {
						parse_error("suffix type list is incomplete\n");
						continue;
					}
				} else {
				...
				for (j = 0; j < suffix[iom_orders[s.org][i]]; j++) {
					...
					fprintf(stderr, "%s Plane %d: '%s' (%s byte %s)\n", str, j, name_list[j],
					        size_list[j], iom_EFormat2Str(format));
				}
				free_str_array(name_list, name_list_sz);
				free_str_array(size_list, size_list_sz);
				free_str_array(type_list, type_list_sz);
	...
	*/
	...
	if ((key2 = OdlFindKwd(qube, suffix_item_byte, NULL, 0, scope))) {
		list2_sz = OdlGetAllKwdValuesArray(key2, &list2);
	}

	list1 = NULL;
	if ((key1 = OdlFindKwd(qube, suffix_item_type, NULL, 0, scope))) {
		list1_sz = OdlGetAllKwdValuesArray(key1, &list1);
	}

	s_suffix_item_bytes =
	    iom_ConvertISISType(list1 ? list1[plane] : NULL, NULL, list2 ? list2[plane] : NULL);

	free_str_array(list1, list1_sz);
	free_str_array(list2, list2_sz);

	if ((key = OdlFindKwd(qube, suffix_item_name, NULL, 0, scope))) {
		name_list_sz = OdlGetAllKwdValuesArray(key, &name_list);
		if (name_list_sz == suffix[iom_orders[s.org][type]]) {
			fprintf(stderr, "Extracting %.*s '%s'\n", (int)strlen(suffix_item_name) - 5,
			        suffix_item_name, name_list[plane]);
		} else {
	...
	data = dv_read_qube_suffix(fileno(fp), &s, suffix_bytes, plane, s_suffix_item_bytes,
	...
	*/
	...
	fclose(fp);
	return (newVal(s.org, s.s_hi[0], s.s_hi[1], s.s_hi[2], ihfmt2vfmt(format), data));
}
...
Var* write_isis_planes(vfuncptr func, Var* arg)
{
	...
	for (i = 0; i < 3; i++) {
		if (suffix[i] != NULL) {
			for (j = 0; j < get_struct_count(suffix[i]); j++) {
				...
				if (error) {
					parse_error(
					    "Suffix plane does not match core size: "
					    "%s [%dx%dx%d] vs %dx%dx%d\n",
					    name, suf_size[0], suf_size[1], suf_size[2], size[0], size[1], size[2]);
					return (NULL);
				}
			}
		}
	...
	if (V_ORG(core) == BSQ) {
		sprintf(buf + strlen(buf), "%d", suffix[0] == NULL ? 0 : get_struct_count(suffix[0]));
		sprintf(buf + strlen(buf), ",%d", suffix[1] == NULL ? 0 : get_struct_count(suffix[1]));
		sprintf(buf + strlen(buf), ",%d", suffix[2] == NULL ? 0 : get_struct_count(suffix[2]));
	} else if (V_ORG(core) == BIP) {
		sprintf(buf + strlen(buf), "%d", suffix[2] == NULL ? 0 : get_struct_count(suffix[2]));
		sprintf(buf + strlen(buf), ",%d", suffix[0] == NULL ? 0 : get_struct_count(suffix[0]));
		sprintf(buf + strlen(buf), ",%d", suffix[1] == NULL ? 0 : get_struct_count(suffix[1]));
	...
	for (i = 0; i < 3; i++) {
		...
		if (suffix[i]) {
			switch (i) {
			case 0: suf_prefix = (char*)"    SAMPLE_SUFFIX"; break;
			case 1: suf_prefix = (char*)"    LINE_SUFFIX"; break;
			case 2: suf_prefix = (char*)"    BAND_SUFFIX"; break;
			}
...
static int write_suffix_zeros(FILE* fp, int nel)
{
	...
	for (i = 0; i < nel; i++) {
		if (fwrite(&zero, 4, 1, fp) != 1) return 0;
	}
	return 1;
}
...
static int write_one(Var* v, int x, int y, int z, FILE* fp)
{
	...
	if (nbytes < 4) {
		if (fwrite(&zero, 4 - nbytes, 1, fp) != 1) return 0;
	}
...
static int write_row_x(Var* v, int y, int z, FILE* fp, int corner1)
{
	...
	for (i = 0; i < x; i++) {
		if (!write_one(v, i, y, z, fp)) return 0;
	}
	for (i = 0; i < corner1; i++) {
		if (fwrite(&zero, 4, 1, fp) != 1) return 0;
	}
...
static int write_row_y(Var* v, int x, int z, FILE* fp, int corner1)
{
	int i;
	int y     = GetY(v);
	long zero = 0;
	...
	for (i = 0; i < y; i++) {
		if (!write_one(v, x, i, z, fp)) return 0;
	}
	for (i = 0; i < corner1; i++) {
		if (fwrite(&zero, 4, 1, fp) != 1) return 0;
	}
...
static int write_row_z(Var* v, int x, int y, FILE* fp, int corner1)
{
	...
	for (i = 0; i < z; i++) {
		if (!write_one(v, x, y, i, fp)) return 0;
	}
	for (i = 0; i < corner1; i++) {
		if (fwrite(&zero, 4, 1, fp) != 1) return 0;
	}
...
static int write_plane(Var* v, int org, int plane, FILE* fp, int corner1, int corner2)
{
	...
	if (plane == 0) {
		if (org == BIL) { /* write rows of Y */
			for (i = 0; i < z; i++) {
				if (!write_row_y(v, 0, i, fp, corner1)) return 0;
			}
		} else if (org == BIP) { /* write rows of Z */
			for (i = 0; i < y; i++) {
				if (!write_row_z(v, 0, i, fp, corner1)) return 0;
			}
		}
	}
	if (plane == 1) {
		if (org == BIL) { /* write rows of x */
			for (i = 0; i < z; i++) {
				if (!write_row_x(v, 0, i, fp, corner1)) return 0;
			}
		} else if (org == BIP) { /* write rows of z */
			for (i = 0; i < x; i++) {
				if (!write_row_z(v, i, 0, fp, corner1)) return 0;
			}
		}
	}
	if (plane == 2) {
		for (i = 0; i < y; i++) { /* write rows of X */
			if (!write_row_x(v, i, 0, fp, corner1)) return 0;
		}
	}
	return 1;
}
...
char* iformat_to_eformat(Var* obj)
{
	...
	switch (V_FORMAT(obj)) {
	case DV_UINT8: sprintf(buf, "MSB_UNSIGNED_INTEGER"); break;
...
#else  /* little-endian */
	switch (V_FORMAT(obj)) {
	case DV_UINT8: sprintf(buf, "PC_UNSIGNED_INTEGER"); break;
...
static void get_suffix_buff_sizes(int core_items[3], int suffix_items[3], int suffix_item_sizes[3])
{
	suffix_item_sizes[0] = suffix_items[0] * core_items[1] * core_items[2] +
	                       suffix_items[0] * core_items[1] * suffix_items[2];

	suffix_item_sizes[1] = core_items[0] * suffix_items[1] * core_items[2] +
	                       suffix_items[0] * suffix_items[1] * (core_items[2] + suffix_items[2]);

	suffix_item_sizes[2] = core_items[0] * core_items[1] * suffix_items[2];
}
...
static int get_org_from_axis_names(char* axis_names[3], int* org)
{
	...
	if (strcasecmp(axis_names[0], "sample") == 0) {
		if (strcasecmp(axis_names[1], "line") == 0) {
			*org = BSQ;
			rc   = 1;
		} else if (strcasecmp(axis_names[1], "band") == 0) {
			*org = BIL;
			rc   = 1;
		}
	} else if (strcasecmp(axis_names[0], "band") == 0) {
		*org = BIP;
		rc   = 1;
	}
 ...
 **/
static int get_isis_order_for_org(int org, int* order)
{
	switch (org) {
...
typedef struct {
	...
	char** suffix_plane_order[3];  /* order in which suffix planes
	                                    appear (org-order) */
...
static CoreDataSpecs* new_CoreDataSpecs()
{
...
static SfxDataSpecs* new_SfxDataSpecs()
{
...
static void free_SfxDataSpecs(SfxDataSpecs** a)
{
	free((*a)->name);
...
static void free_CoreDataSpecs(CoreDataSpecs** a)
{
	int i, k;
 ...
 **/
static int compare_SfxDataSpecs(const void* v1, const void* v2)
{
 ...
 ** (x,y,z) dimensions.
 **/
static int has_dim(Var* v, int x, int y, int z)
{
	if (V_TYPE(v) == ID_VAL) {
 ...
 ** number of rows.
 **/
static int has_n_rows(Var* v, int n)
{
	switch (V_TYPE(v)) {
	case ID_STRING: return (n == 1);
	case ID_TEXT: return (V_TEXT(v).Row == n);
	}
 ...
 **/
static int get_max_strlen(char** strs, int n)
{
	int len = 0;
	int i;
 ...
 **/
static void swap_data(void* data, size_t nitems, int item_size_bytes)
{
 ...
 **/
static CoreDataSpecs* get_data_attrs_from_lbl(Var* qube /* struct.qube / struct.spectral_qube */
                                              )
{
	Var* v             = NULL;
	...
	/* get qube.axis_name */
	if (find_struct(qube, "axis_name", &v) < 0 || V_TYPE(v) != ID_TEXT || !has_n_rows(v, a->axes)) {
	...
	if (find_struct(qube, "data", &a->core_data_var) < 0 ||
	    !has_dim(a->core_data_var, a->core_items[iom_orders[a->org][0]],
	             a->core_items[iom_orders[a->org][1]], a->core_items[iom_orders[a->org][2]])) {
	...
	for (k = 0; k < a->axes; k++) {
		...
		sprintf(buff, "%s_suffix_name", a->axis_name[kk]);
		lcase(buff);
		...
		/* if we reached here then we have at least one suffix plane that
		   we need to process */
		a->suffix_attr[kk] = (SfxDataSpecs**)calloc(a->suffix_items[kk], sizeof(SfxDataSpecs*));

		/* save the order in which the suffix planes appear, and initialize the
		   SfxDataSpecs for each of the suffix planes */
		a->suffix_plane_order[kk] = (char**)calloc(a->suffix_items[kk], sizeof(char*));
		...
		sprintf(buff, "%s_suffix_item_bytes", a->axis_name[kk]);
		lcase(buff);
		...
		sprintf(buff, "%s_suffix_item_type", a->axis_name[kk]);
		lcase(buff);
		...
		for (i = 0; i < a->suffix_items[kk]; i++) {
			...
			sprintf(buff, "%d", a->suffix_attr[kk][i]->item_bytes);
			lcase(buff);
		...
		sprintf(buff, "%s_suffix_unit", a->axis_name[kk]);
		lcase(buff);
		if (find_struct(qube, buff, &v) < 0) {
			for (i = 0; i < a->suffix_items[kk]; i++) {
		...
		sprintf(buff, "%s_suffix_base", a->axis_name[kk]);
		lcase(buff);
		if (find_struct(qube, buff, &v) < 0) {
			for (i = 0; i < a->suffix_items[kk]; i++) {
		...
		sprintf(buff, "%s_suffix_multiplier", a->axis_name[kk]);
		lcase(buff);
		if (find_struct(qube, buff, &v) < 0) {
			for (i = 0; i < a->suffix_items[kk]; i++) {
		...
		for (i = 0; i < a->suffix_items[kk]; i++) {
			...
			if (find_struct(sfx_data, a->suffix_attr[kk][i]->name, &a->suffix_attr[kk][i]->data_var) < 0 ||
				...
				parse_error("Expecting qube.suffix_data.%s.%s of dim: %dx%dx%d\n", buff,
				            a->suffix_attr[kk][i]->name, suffix_plane_size[0], suffix_plane_size[1],
				            suffix_plane_size[2]);
				free_CoreDataSpecs(&a);
				return NULL;
			}
 ...
 ** This specification is used in writing of ISIS cube file.
 **/
static CoreDataSpecs* get_data_attrs_from_data(Var* qube)
{
	...
	if (suffix_data != NULL) {
		for (k = 0; k < a->axes; k++) {
			...
			if (find_struct(suffix_data, buff, &suffix_axis_data) >= 0) {
				a->suffix_items[kk] = get_struct_names(suffix_axis_data, &a->suffix_plane_order[kk], NULL);
 ...
 **/
static int extract_data_band_as_bsq(Var* v, int band_no, void* buff)
{
	int x, y, pos, idx;
	...
	for (y = 0; y < GetY(v); y++) {
		for (x = 0; x < GetX(v); x++) {
			...
			switch (V_FORMAT(v)) {
			case DV_UINT8: byte_buff[idx++]    = (unsigned char)extract_int(v, pos); break;
			case DV_INT16: short_buff[idx++]   = (short)extract_int(v, pos); break;
			case DV_INT32: int_buff[idx++]     = extract_int(v, pos); break;
			case DV_FLOAT: float_buff[idx++]   = extract_float(v, pos); break;
			case DV_DOUBLE: double_buff[idx++] = extract_double(v, pos); break;
			default:
				fprintf(stderr,
				        "extract_data_band_as_bsq passed an "
 ...
 **/
static int
convert_data_to_ext_fmt(char* in_buff,   /* input buffer in on of davinci internal data formats */
 ...
 **/
static int concat_column_data(char* buff,          /* buff accumulator of data */
 ...
 ** may be larger than what we are concating.
 **/
static int concat_row_data(char* buff,          /* buff accumulator of data */
                           int rec_len,         /* total width of the buffer */
 ...
 **/
static int extract_suffix_data_for_band(CoreDataSpecs* core_specs, int band_no, void* side_buff, void* bot_buff)
{
	...
	for (i = 0; i < side_items; i++) {
		...
		/* The data variable from which data extraction has been done
		   ...
		   the other */
		if (!convert_data(V_FORMAT(s->data_var), buff1, s->item_type, buff2,
		...
		concat_column_data(side_buff, side_items * core_specs->suffix_bytes,
		                   core_specs->core_items[line_idx], buff2, core_specs->suffix_bytes, &col_pos);
	}
	...
	for (i = 0; i < bot_items; i++) {
		...
		concat_row_data(bot_buff, core_specs->core_items[sample_idx] * core_specs->suffix_bytes,
		                core_specs->core_items[sample_idx], buff2, core_specs->suffix_bytes, &row_pos);
	}
 ...
 ** as loaded by get_data_attr_from_lbl.
 **/
static int extract_core_data_for_band(CoreDataSpecs* core_specs, int band_no, void* core_buff)
{
	int sample_idx = iom_orders[core_specs->org][0];
	int line_idx   = iom_orders[core_specs->org][1];
	int n;
	...
	/* convert from davinci variable type to a davinci variable type suitable
	   for output */
	if (!convert_data(V_FORMAT(core_specs->core_data_var), buff1, core_specs->core_item_type, core_buff, n)) {
 ...
 ** as loaded by get_data_attr_from_lbl.
 **/
static int extract_backplane_data_for_band(CoreDataSpecs* core_specs, int band_no, void* back_buff)
{
	int sample_idx = iom_orders[core_specs->org][0];
	int line_idx   = iom_orders[core_specs->org][1];
 ...
 ** string.
 **/
static char* get_generic_item_type(const char* item_type)
{
	char* s;
 ...
 ** as loaded by get_data_attr_from_lbl.
 **/
static int set_suffix_names(int fid, CoreDataSpecs* core_specs)
{
	int k, kk, i;
	int sfx_idx;
	int rc;
	...
	for (k = 0; k < 3; k++) {
		...
		for (i = 0; i < core_specs->suffix_items[kk]; i++) {
			...
			q_set_suffix_keys(fid, /* add */ 1, core_specs->axis_name[kk], &sfx_idx, s->name,
			                  s->item_bytes, item_type_str, s->scale, s->unit, &rc);
			free(item_type_str);
 ...
 **/
static char* remove_enclosing_quotes(char* s)
{
	if (s[0] == '\"' || s[0] == '\'') {
		strcpy(s, &s[1]);
		s[strlen(s) - 1] = '\0';
 ...
 **/
static int propagate_string_keyword(int fid, char* object, char* group, char* name, Var* v)
{
	int nvals;
	char* vals;
	int maxlen;
	...
	if (double_quoted) {
		/* value is a string */
		p_set_str_key(fid, object, group, name, /* set */ 1, &nvals, vals, maxlen, vlen, &rc);
	} else {
		/* value is a literal */
		p_set_lit_key(fid, object, group, name, /* set */ 1, &nvals, vals, maxlen, vlen, &rc);
	}
...
static int propagate_string_array_keyword(int fid, char* object, char* group, char* name, Var* v)
{
	int nvals;
	char* vals;
	int maxlen;
...
static int propagate_int_keyword(int fid, char* object, char* group, char* name, Var* v)
{
	int nvals;
	int* vals;
	int rc;
...
static int propagate_float_keyword(int fid, char* object, char* group, char* name, Var* v)
{
	int nvals;
	float* vals;
	int rc;
...
static int propagate_double_keyword(int fid, char* object, char* group, char* name, Var* v)
{
	int nvals;
	double* vals;
	int rc;
 ...
 **
 **/
static int keyword_already_exists(int fid, char* object, char* group, char* name)
{
	int nvals = 0;
	int rc;
 ...
 **/
static char* structure_element_to_keyword(const char* s)
{
	char* kw = strdup(s);
 ...
 ** are not.
 **/
static int overridable_keyword(const char* obj, const char* group, const char* kw)
{
	if (strcasecmp(obj, "qube") == 0 && strcasecmp(group, "") == 0) {
		if (strcasecmp(kw, "core_null") == 0) {
			return 1;
		}
		if (strcasecmp(kw, "core_valid_minimum") == 0) {
			return 1;
		}
		if (strcasecmp(kw, "core_low_repr_saturation") == 0) {
			return 1;
		}
		if (strcasecmp(kw, "core_low_instr_saturation") == 0) {
			return 1;
		}
		if (strcasecmp(kw, "core_high_repr_saturation") == 0) {
			return 1;
		}
		if (strcasecmp(kw, "core_high_instr_saturation") == 0) {
			return 1;
		}
	}
 ...
 **/
static int barred_keyword(const char* obj, const char* group, const char* kw)
{
	if (strcasecmp(kw, "Object") == 0) {
		return 1;
	}
	if (strcasecmp(kw, "data") == 0) {
		return 1;
	}
	if (strcasecmp(kw, "suffix_data") == 0) {
		return 1;
	}
 ...
 **/
static int propagate_keywords_0(int fid, Var* s, char* object, char* group)
{
	int n              = 0;
	...
	for (i = 0; i < n; i++) {
		...
		case ID_VAL: /* numeric array or a single value */
			...
			case DV_UINT8:
			case DV_INT16:
			case DV_INT32: propagate_int_keyword(fid, object, group, name, v); break;

			case DV_FLOAT: propagate_float_keyword(fid, object, group, name, v); break;

			case DV_DOUBLE: propagate_double_keyword(fid, object, group, name, v); break;
 ...
 **/
static int propagate_keywords(int fid, Var* s)
{
...
static int propagate_history_string_keyword(int indent, char* name, Var* v)
{
	int nvals;
	char* vals;
	int maxlen;
...
static int propagate_history_string_array_keyword(int indent, char* name, Var* v)
{
	int nvals;
	char* vals;
	int maxlen;
...
static int propagate_history_int_keyword(int indent, char* name, Var* v)
{
	int nvals;
	int* vals;
	int rc;
...
static int propagate_history_real_keyword(int indent, char* name, Var* v)
{
	int nvals;
	float* vals;
	int rc;
 ...
 **/
static int propagate_history_group_keywords(Var* s, const char* group, int indent, int level)
{
	int i, n;
	Var* v;
	char* name;
	...
	for (i = 0; i < n; i++) {
		...
		case ID_STRUCT:
			if (level == 1 && strcasecmp(name, "parameters") == 0) {
				...
				parse_error("Invalid section %s within history group %s. Ignored.", name, group);
				continue;
			}
			break;

		case ID_STRING:
			propagate_history_string_keyword(indent, name, v);
			break;
			break;

		case ID_TEXT:
			propagate_history_string_array_keyword(indent, name, v);
			break;
			break;
		...
		case ID_VAL:
			...
			case DV_UINT8:
			case DV_INT16:
			case DV_INT32: propagate_history_int_keyword(indent, name, v); break;
			case DV_FLOAT:
			case DV_DOUBLE: propagate_history_real_keyword(indent, name, v); break;
			default:
				parse_error("Invalid format for %s within history group %s. Ignored.", name, group);
				break;
			}
			break;

		default:
			parse_error("Invalid type for %s within history group %s. Ignored.", name, group);
			break;
		}
	}
 ...
 **/
static int propagate_history_group(int fid, char* group_name, Var* hg)
{
	int rc;
 ...
 **/
static int propagate_history(int fid, Var* h /* history structure */
                             )
{
	int i, n;
	Var* v;
 ...
 **
 **/
int dv_WriteISISStruct(Var* s, char* fname, int force)
{
	...
	if (a == NULL) {
		parse_error(
		    "Error while getting attributes from the structure. "
	...
	q_set_sys_keys(&fid, a->axes, order, a->core_items, a->suffix_items, axname,
	               max_axis_name_len + 1, a->core_item_bytes, core_item_type_str, a->core_scale, &rc);
	 ...
	 ** sparately.
	 **/
	core_plane_size_items = a->core_items[iom_orders[a->org][0]] * a->core_items[iom_orders[a->org][1]];
	side_plane_size_items =
	    a->suffix_items[iom_orders[a->org][0]] *
	    (a->core_items[iom_orders[a->org][1]] + a->suffix_items[iom_orders[a->org][1]]);
	bot_plane_size_items = a->suffix_items[iom_orders[a->org][1]] * a->core_items[iom_orders[a->org][0]];
	...
	if (core_data == NULL || side_data == NULL || bot_data == NULL) {
		parse_error("Mem alloc error %d bytes.\n",
		            core_plane_size_items * a->core_item_bytes +
		                (side_plane_size_items + bot_plane_size_items) * a->suffix_bytes);
		free_CoreDataSpecs(&a);
		q_close(fid, DELETE_FILE, &rc);
		return 0;
	}
