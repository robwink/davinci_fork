...
Var* ff_bindct(vfuncptr func, Var* arg)
{
	...
	out = newVal(V_ORG(obj), V_SIZE(obj)[0], V_SIZE(obj)[1], V_SIZE(obj)[2], DV_INT32, odata);
...
void jpeg_fdct_bin_c1(int* data)
{
	...
	for (ctr = DCTSIZE - 1; ctr >= 0; ctr--) {
		...
		dataptr[DCTSIZE * 2] =
		    tmp13 -
		    (((dataptr[DCTSIZE * 6] << 3) + (dataptr[DCTSIZE * 6] << 1) + dataptr[DCTSIZE * 6]) >> 5);
...
void jpeg_idct_bin_c1(int* coef_block, int* output_buf)
{
	...
	for (ctr = 0; ctr < DCTSIZE; ctr++) {
		...
		/* X[6] and X[2]:11/32, 13/32 */
		tmp13 = wsptr[2] + (((wsptr[6] << 3) + (wsptr[6] << 1) + wsptr[6]) >> 5);
