...
LABEL* LoadLabelFromObjDesc(OBJDESC* tbl, const char* fname)
{
	OBJDESC* col;
	KEYWORD* kw;
	FIELD* f;
...
FIELD* MakeField(OBJDESC* col, LABEL* l)
{
	FIELD* f;
	VARDATA* vardata;
	KEYWORD* kw;
	...
	do {
		...
		if ((f->eformat = ConvertType(f->type)) == INVALID_EFORMAT) {
			parse_error("Unrecognized type: %s, %s %d bytes", f->name, f->type, f->size);
			break;
		}
		...
		if ((kw = GetKey(col, "VAR_RECORD_TYPE")) != NULL) {
			...
			if (!strcmp(ptr, "VAX_VARIABLE_LENGTH"))
				vardata->type = VAX_VAR;
			else if (!strcmp(ptr, "Q15"))
				vardata->type = Q15;
			else {
			...
			if ((kw = GetKey(col, "VAR_ITEM_BYTES")) != NULL) {
				...
				if ((kw = GetKey(col, "VAR_DATA_TYPE")) != NULL) {
					...
					parse_error("VAR_DATA_TYPE not specified for field: %s", f->name);
					break;
				}
				if ((vardata->eformat = ConvertType(ptr)) == INVALID_EFORMAT) {
					parse_error("Unrecognized vartype: %s, %s %d bytes", f->name, ptr, vardata->size);
				}
				...
				parse_error("VAR_ITEM_BYTES not specified for field: %s", f->name);
				break;
			}
		}
...
IFORMAT eformat_to_iformat(EFORMAT e)
{
	...
	case LSB_INTEGER:
	case MSB_INTEGER:
	case ASCII_INTEGER: return (VINT);

	case MSB_UNSIGNED_INTEGER:
	case LSB_UNSIGNED_INTEGER:
	case MSB_BIT_FIELD:
	case LSB_BIT_FIELD:
	case BYTE_OFFSET: return (UVINT);

	case IEEE_REAL:
	case PC_REAL:
	case ASCII_REAL: return (REAL);

	case CHARACTER: return (STRING);

	default: fprintf(stderr, "Unrecognized etype: %d\n", e);
	}
...
EFORMAT ConvertType(char* type)
{
	if (!strcasecmp(type, "MSB_INTEGER") || !strcasecmp(type, "MSB_SIGNED_INTEGER") ||
		...
		return (MSB_INTEGER);
	           ...
	           !strcasecmp(type, "MAC_UNSIGNED_INTEGER") || !strcasecmp(type, "UNSIGNED_INTEGER") ||
	           !strcasecmp(type, "BOOLEAN")) {
		return (MSB_UNSIGNED_INTEGER);
	} else if (!strcasecmp(type, "IEEE_REAL") || !strcasecmp(type, "SUN_REAL") ||
	           !strcasecmp(type, "MAC_REAL") || !strcasecmp(type, "REAL")) {
		return (IEEE_REAL);
		...
		return (LSB_BIT_FIELD);
	           ...
	           !strcasecmp(type, "PC_INTEGER") || !strcasecmp(type, "VAX_INTEGER")) {
		return (LSB_INTEGER);
