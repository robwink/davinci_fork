...
char* createThemisGhostImage(int ghostDown, int ghostRight, const char* msg, const char* stamp_id,
                             int imageW, int imageH, int bytesPerPixel, void* imageDataBuff,
                             void* ghostDataBuff, int useTrackServer);
Var* make_band(Var* in, int band);

Var* ff_deghost(vfuncptr func, Var* arg)
{

	Var *obj = NULL, *down = NULL, *right = NULL, *bands = NULL, *v = NULL, *out = NULL;
	char* id;
	int z, y, x, nbytes, i;
	size_t xy;
	void* out_data;
	...
	for (i = 0; i < z; i++) {
		...
		if (extract_int(down, i) == 0 && (right == NULL || extract_int(right, i) == 0)) {
			...
			msg = createThemisGhostImage(extract_int(down, i),                /* down */
