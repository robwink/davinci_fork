...
ff_exit(vfuncptr func, Var * arg)
{
  ...
  // NOTE(gorelick): Tue Aug  4 00:10:26 PDT 2009
  // It's a bad idea for this to call exit() directory, since
  // the cleanup that happens in quit() wont.  Changing to call quit() instead.
  if (obj == NULL) {
    quit(0);
  } else {
