...
static char* footer_ptrs[] = {footer_buffer1, footer_buffer2, NULL};

void CreatePopup2(void);

static void MoveMarker(int x, int y)
{

	if (!gc) {
		gc = XCreateGC(XtDisplay(graph), XtWindow(graph), 0, NULL);
		XSetForeground(XtDisplay(graph), gc,
		               WhitePixel(XtDisplay(graph), DefaultScreen(XtDisplay(graph))));
	}
	if (!pm) {
		pm = XCreatePixmap(XtDisplay(graph), XtWindow(graph), 3, 3,
		                   DefaultDepth(XtDisplay(graph), DefaultScreen(XtDisplay(graph))));
	}

	if (marker_x != -1 && marker_y != -1) {
		XCopyArea(XtDisplay(graph), pm, XtWindow(graph), gc, 0, 0, 3, 3, marker_x - 1, marker_y - 1);
	}
	if (x != -1 && y != -1) {
		XCopyArea(XtDisplay(graph), XtWindow(graph), pm, gc, x - 1, y - 1, 3, 3, 0, 0);
...
static void ResetFooter()
{
	...
	XtVaSetValues(graph, XtNxrt3dFooterStrings, footer, NULL);
}
...
static void MovePickMarker(int xindex, int yindex)
{
...
static void MoveMapMarker(double x, double y, double z)
{
	Xrt3dMapResult map;
...
static void TestPick(Widget xrt_3d, XButtonEvent* event)
{
	...
	if (region == XRT3D_RGN_IN_GRAPH) {
		sprintf(footer_buffer2, "Closest Grid Index: %d, %d", pick.xindex, pick.yindex);
	} else {
		switch (region) {
		case XRT3D_RGN_IN_HEADER: sprintf(footer_buffer2, "In header."); break;
		case XRT3D_RGN_IN_FOOTER: sprintf(footer_buffer2, "In footer."); break;
		case XRT3D_RGN_IN_LEGEND: sprintf(footer_buffer2, "In legend."); break;
		default: sprintf(footer_buffer2, "Nowhere."); break;
		}
	}

	MovePickMarker(pick.xindex, pick.yindex);
	XtVaSetValues(xrt_3d, XtNxrt3dFooterStrings, footer_ptrs, NULL);
}
...
static void TestMap(Widget xrt_3d, XButtonEvent* event)
{
	...
	if (region == XRT3D_RGN_IN_GRAPH) {
		if (map.z == XRT3D_HUGE_VAL) {
			...
			sprintf(footer_buffer2, "Surface: %g, %g, %g", map.x, map.y, map.z);
		}
	} else {
		switch (region) {
		case XRT3D_RGN_IN_HEADER: sprintf(footer_buffer2, "In header."); break;
...
Var* ff_xrt3d(vfuncptr func, Var* arg)
{
	...
	grid = Xrt3dMakeGridData(x, y, XRT3D_HUGE_VAL, x / 10.0, y / 10.0, 0.0, 0.0, TRUE);
	...
	if (graph == NULL) {
		...
		if (new_window) {
			...
			*/
			while ((parent = XtParent(graph)) != NULL && strcmp(XtName(graph), "shellWindow")) {
				graph = parent;
			}
...
static MenuItem file_menu[] = {
    {"Print", "Print...", &xmPushButtonGadgetClass, PrintCB, (XtPointer)ACTION_PRINT, 0, NULL},
    {"FileSeparator", NULL, &xmSeparatorGadgetClass, NULL, NULL, 0, NULL},
    {"Quit", "Quit", &xmPushButtonGadgetClass, MenuCB, (XtPointer)ACTION_QUIT, 0, NULL},
    {NULL},
};

static MenuItem graph_menu[] = {
    {"Mesh", "Mesh", &xmToggleButtonWidgetClass, MenuCB, (XtPointer)ACTION_MESH, 0, NULL},
    {"Shaded", "Shaded", &xmToggleButtonWidgetClass, MenuCB, (XtPointer)ACTION_SHADED, 0, NULL},
    {"Contours", "Contours", &xmToggleButtonWidgetClass, MenuCB, (XtPointer)ACTION_CONTOURS, 0, NULL},
    {"Zones", "Zones", &xmToggleButtonWidgetClass, MenuCB, (XtPointer)ACTION_ZONES, 0, NULL},
    {"Solid", "Solid", &xmToggleButtonWidgetClass, MenuCB, (XtPointer)ACTION_SOLID, 0, NULL},
    {NULL},
};
...
static Widget CreateMain(Widget parent)
{
	static char translations[] =
	    "#augment \n\
                                  ...
                                  <Btn3Motion>:  TestMap() \n\
                                  <Btn3Up>:      TestMap()";
	static XtActionsRec actions[] = {{"TestPick", (XtActionProc)TestPick},
	                                 {"TestMap", (XtActionProc)TestMap}};

	XtAppAddActions(app, actions, XtNumber(actions));

	graph = XtVaCreateManagedWidget(
	    "graph", xtXrt3dWidgetClass, parent, XmNtopAttachment, XmATTACH_WIDGET, XmNtopAttachment,
	    XmATTACH_FORM, XmNbottomAttachment, XmATTACH_FORM, XmNleftAttachment, XmATTACH_FORM,
	    XmNrightAttachment, XmATTACH_FORM, XtNxrt3dSurfaceData, grid, XtNxrt3dXAxisTitle, "X Axis",
	    XtNxrt3dYAxisTitle, "Y Axis", XtNxrt3dZAxisTitle, "Z Axis", XtNxrt3dDrawMesh, TRUE,
	    XtNxrt3dDrawShaded, TRUE, XtNxrt3dDrawContours, TRUE, XtNxrt3dDrawZones, TRUE, XtNxrt3dHeaderBorder,
	    XRT3D_BORDER_PLAIN, XmNtranslations, XtParseTranslationTable(translations), NULL);
	return (graph);
}
...
void CreatePopup2(void)
{
	...
	base = XtVaAppCreateShell(NULL, "shellWindow", topLevelShellWidgetClass, XtDisplay(top),
	                          XtNtitle, "davinci/XRT3D", NULL);

	MtCreateShell(base, NULL, NULL, (Widget(*)())CreateMain, menu_items, NULL, False);
