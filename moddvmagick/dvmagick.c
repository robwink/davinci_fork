...
static dvModuleFuncDesc export_funcs[] = {{"WriteImage", (void*)dvWriteImage},
...
static dvModuleInitStuff dvinit = {export_funcs, 49, NULL, 0};
 ...
 */
DV_MOD_EXPORT int dv_module_init(const char* modname, dvModuleInitStuff* ini)
{
	int imversion;
	char shortversion[10];
	*ini = dvinit;
	if (deps == NULL)
		; /* no op to keep gcc from carping about deps */
	(void)GetMagickVersion(&imversion);
	snprintf(shortversion, 10, "%i.%i.%i", imversion >> 8, (imversion & 0xF0) >> 4, (imversion & 0xF));
...
static inline StorageType dv_to_im_storage_type(Var* img)
{
	...
	switch (V_FORMAT(img)) {
	//TODO(rswinkle): Even though this module is completely ifdef'd out
	//in the main davinci code (ff_load.c, ff_write.c) if anyone ever
	//wants to use this again they'll probably have to do more work than
	//just fixing the type enums
	case DV_UINT8: return CharPixel;
	case DV_INT16: return ShortPixel;
	case DV_INT32: return IntegerPixel;
	case FLOAT: return FloatPixel;
	case DOUBLE: return DoublePixel;

	//deprecated
	case VAX_FLOAT:
		return FloatPixel; /* When you make an assumption... */
	case VAX_INTEGER: return IntegerPixel;
...
static Image* constitute_dvobject(Var* obj, char* planes, unsigned int z)
{
	...
	rtn = ConstituteImage(x, y, planes, CharPixel, V_DATA(obj) + cpos(0, 0, z, obj), &exc);
	if (rtn == (Image*)NULL) {
...
static Image* assemble_image(Var* obj, unsigned int ctype, unsigned int zplane)
{
  ...
  */
	...
	if (ctype == 3) {
		...
		for (z = 0; z < 3; z++) {
			...
			dvrange.lo[2]   = z + zplane;
			dvrange.hi[2]   = z + zplane;
			dvrange.step[0] = 1;
			dvrange.step[1] = 1;
			dvrange.step[2] = 1;
...
static Image* dv_to_im(Var* obj, int type)
{
	...
	*/
	...
	z = GetZ(obj);
	switch (z) {
	case 1: img = assemble_image(obj, 1, 1); break;
	case 3: img = assemble_image(obj, 3, 1); break;
	default:
		switch (type) {
		case 2:               /* a color movie is expressly requested */
			...
			movietype = 3;
			break;
		case 1: /* we can always do a grayscale */ movietype = 1; break;
		default:
...
MODULE_TYPE int paramdvWriteImage(Var* image, char* fname, char* itype, int force)
{
	...
	*/
	...
	if (stat(fname, &sbuf) == -1) {
		switch (errno) {
		case ENOENT: /* File doesn't exist. That's OK */ break;
		default:
	...
	*/
	...
	if (z == 2 || z > 3) {
		if (strcmp(itype, "mpgc") && strcmp(itype, "mpgg") && strcmp(itype, "gifc") &&
		    strcmp(itype, "gifg")) {
...
MODULE_TYPE Var* dvWriteImage(vfuncptr f, Var* args)
{
	...
	char* valid_types[] = {"avs",  "bmp",  "cmyk",  "gif",  "gifc", "gifg", "hist", "jbig", "jpeg",
...
static Var* dvAddNoiseImage(vfuncptr f, Var* args)
{
   ...
   */
	...
	int i;
	char* noisetype;
	char* noisetypes[] = {"uniform", "gaussian", "multiplicative", "impulse", "laplacian",
	                      "poisson", ""};
...
static Var* dvResizeImage(vfuncptr f, Var* args)
{
   ...
   */
	...
	char* filtertypes[] = {"bessel",   "blackman",  "box",     "catrom",   "cubic",
	                       "gaussian", "hanning",   "hermite", "lanczos",  "mitchell",
	                       "point",    "quadratic", "sinc",    "triangle", ""};
	FilterTypes ftype[] = {BesselFilter,  BlackmanFilter, BoxFilter,     CatromFilter,
	                       CubicFilter,   GaussianFilter, HanningFilter, HermiteFilter,
	                       LanczosFilter, MitchellFilter, PointFilter,   QuadraticFilter,
