...
int Keep(unsigned char B, unsigned short F, CMD Cmd, int Frame, int Band)
{
	...
	case FRAME:
		if (Frame == (F & 0xFFFF) && (B!=0x0F && B!=0xE))
			return 1;
		break;
...
int Process_SC_Vis(FILE *infile,unsigned char **data,int *vis_width, int *nob, int verb, int *rot,char *filename)
{
	msdp_Header mh;
...
void Rotate_Buffer(unsigned char *buf, int height, int width, int verb)
{
	...
	for (i=0; i<nof; i++) {
		...
		for (j=0; j<FRAMELET_HEIGHT; j++) {
			for(k=0; k<width; k++) {
				hold[(FRAMELET_HEIGHT-1-j)*width+(width-1-k)] = buf[i*chunk+j*width+k];
			}
		}
...
void RedunBegone(char **buf, int *Lines, int Col, PACIstatus *Ps,int quiet, int eflag, Var *err,int *BandCount)
{
	...
	for (i=0; i<(*Lines-1); i++) {
		...
		frameB = ((data[(i+1)*Col+_FRAMES] & 0xFF) << 8) | (data[(i+1)*Col+_FRAMES+1] & 0xFF);
...
int Check_Swap(unsigned char *buf, int len)
{
	...
	while (i < (chunk-3)) {
		if (buf[i  ]==sync_sc[0] && buf[i+1]==sync_sc[1] &&
		    buf[i+2]==sync_sc[2] && buf[i+3]==sync_sc[3]) {
	...
	while (i < (chunk-3)) {
		if (buf[i  ]==sync_gse[0] && buf[i+1]==sync_gse[1] &&
		    buf[i+2]==sync_gse[2] && buf[i+3]==sync_gse[3]) {
	...
	if (!report) {
		...
		while ((i < (old_len-_DATA)) && !(Done)) {
			...
			if (Keep(c_Band,c_Frame,Cmd,Frame,Band) && ((Output) <= 328 )) {
				...
				if (Data_Only && Cmd!=ALL){
					memcpy(((unsigned char *)data)+Index, ((unsigned char *)buf)+i-Output+_DATA, Output-_SIGINFO);
					Index+=(Output-_SIGINFO);
					Lines++;
					BandCount[c_Band]++;
				} else {
