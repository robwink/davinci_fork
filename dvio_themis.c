...
void RedunBegone(char** buf, int* Lines, int Col, PACIstatus* Ps, int quiet, int eflag, Var* err,
                 int* BandCount);
...
extern unsigned char* read_predictive(FILE* infile, int* total_size, int db, int size_guess);

extern unsigned char* read_DCT(FILE* infile, int* total_size, int guess_size, int verb);
...
int Read_Ahead_For_Best_Collumn_Guess(int* Width, unsigned char* buf, int len)
{
	...
	while (!Done) {
		...
		if (Guesses[LineCount] <= (MAXDATA + _SIGINFO) && Guesses[LineCount] >= (MINDATA + _SIGINFO)) {
			LineCount++;
		}
...
int Keep(unsigned char B, unsigned short F, CMD Cmd, int Frame, int Band)
{
	switch (Cmd) {
	case BAND:
		if (Band == (B & 0x0F)) return 1;
		break;

	case FRAME:
		if (Frame == (F & 0xFFFF) && (B != 0x0F && B != 0xE)) return 1;
		break;

	case ALL:
		if (B != 0x0F && B != 0xE) return 1;
		break;
	}
...
int Count_Out_Bands(int b, int* rot)
{
	int i;
	int nob = 0;
	int bands;
	...
	for (i = 0; i < 5; i++) {
		if ((bands & (1 << i))) parse_error("%s", vis_bands[i]);
	}

	return nob;
}

int Process_SC_Vis(FILE* infile, unsigned char** data, int* vis_width, int* nob, int verb, int* rot,
                   char* filename)
{
	msdp_Header mh;
	...
	*nob = Count_Out_Bands((int)first_mh.bands, rot);
	...
	if ((height * width) != size) {
		/*		parse_error("Height*Width doesn't equate to size: %d vs
		 * %d\n",(height*width),size);*/
		size   = ((height * width) > size ? (height * width) : (size));
		height = size / width; /*We took the larger of the two sizes, need to correct for height*/
	}
	...
	*/
	...
	if (first_mh.comp[0] == 1) { /*Predictively Compressed*/
		...
		height = (size - 1024) /
		         width; /*Subtract off the header bytes which are added in durring the read*/
...
#else
		parse_error(
		    "Sorry, you do not have the correct decompression library for this file...aborting");
		return 0;
#endif
	} else {
		...
		*data = (unsigned char*)calloc(size, sizeof(char));
		while (1) {
			...
			if (in_chunk != NULL) free(in_chunk);
...
Var* Make_Vis_Cube(int nob, int height, int width, unsigned char* buf)
{
	...
	/*How many individual frame-segments (framelets) in the images. fl is how many framelets left */
	fl = framelets = height / fh;
	entries        = framelets / nob; /*How many framelets for each band*/
	divider =
	    height / nob *
	    width; /*divider * band#-1 will be the starting address of each band in the linear array*/
...
void Rotate_Buffer(unsigned char* buf, int height, int width, int verb)
{
	...
	if (verb) parse_error("Rotating all individual frames 180 degrees");
...
Var* ff_GSE_VIS_Read(vfuncptr func, Var* arg)
{
	...
	if (!(fname = dv_locate_file(filename)) || !(infile = fopen(fname, "r"))) {
	...
	if (gse) {
		...
		if (header.format ==
		    iom_SHORT) { /*Data is actually 12-bit and needs the upper 4 bits cleaned off*/
		...
		v = newVal(ihorg2vorg(header.org), header.size[0], header.size[1], header.size[2],
		           ihfmt2vfmt(header.format), data);
		...
		if (height) {
			if (rotate) Rotate_Buffer(buf, height, width, verb);
			if (nocube)
				return newVal(BSQ, width, height, 1, DV_UINT8, buf);
			else
...
Var* ff_Frame_Grabber_Read(vfuncptr func, Var* arg)
{
	...
	case 14:
		...
		num_bytes = 2;
		break;
	case 20: parse_error("IEEE data file: (Consists of just header)"); break;
	} /* switch fileType */

	fclose(infile);

	return newVal(BSQ, X, Y, Z, num_bytes,
	              ((num_bytes == 2) ? ((void*)sh_data_words) : ((void*)data_words)));
}
...
void Add_Fake_Frame(void* newbuf, int band, int frame, int num_frames, int width, int eflag, Var* err)
{
	int i;
	...
	if (eflag) V_INT(err) |= 2; /*Or it with 2*/
...
void Packet_Swap(unsigned char** buf, unsigned char* fbuf, int* len)
{
	int i;
	int l   = *len;
	int idx = 0;
...
void RedunBegone(char** buf, int* Lines, int Col, PACIstatus* Ps, int quiet, int eflag, Var* err, int* BandCount)
{
	int i;
	char* data = *buf;
	char A[320], B[320];
	...
	for (i = 0; i < (*Lines - 1); i++) {
		frameA = ((data[i * Col + _FRAMES] & 0xFF) << 8) | (data[i * Col + _FRAMES + 1] & 0xFF);
		...
		if (frameA == frameB) { /*Uh Oh! Duplicate frame #'s*/
			...
			if (cmp_themis_lines(A, B, (Col - _SIGINFO))) { /* oh Yea! Redudant data!*/
				...
				if (eflag) V_INT(err) |= 1; /*Or it with 1*/
			} else {
				...
				if (eflag) V_INT(err) |= 4; /*Or it with 4*/
			}
		}
	}
}
	...
	for (i = 0; i < 10; i++) {
		if (Ps->Mapping[i]) {
			if (Ps->AddedFrames[i] || Ps->RepeatedFrames[i] || Ps->CorruptFrames[i]) {
				...
				parse_error("Band:%02d (slot %02d)\t%s%s%s", i, Ps->Mapping[i], msg1, msg2, msg3);
			}
		}
	}
}
...
int Check_Swap(unsigned char* buf, int len)
{
	int chunk;
	int i;
	...
	while (i < (chunk - 3)) {
		if (buf[i] == sync_sc[0] && buf[i + 1] == sync_sc[1] && buf[i + 2] == sync_sc[2] &&
		    buf[i + 3] == sync_sc[3]) {
	...
	while (i < (chunk - 3)) {
		if (buf[i] == sync_gse[0] && buf[i + 1] == sync_gse[1] && buf[i + 2] == sync_gse[2] &&
		    buf[i + 3] == sync_gse[3]) {
...
Var* ff_PACI_Read(vfuncptr func, Var* arg)
{
	...
	*/
	...
	// Add room for an additional packet; this will prevent
	...
	if (!report) {
			...
			if (Keep(c_Band, c_Frame, Cmd, Frame, Band) && ((Output) <= 328)) {
				...
				if (Data_Only && Cmd != ALL) {
					if (Output - _SIGINFO < 320) {
						...
						if (decompress != NULL) free(decompress);
					...
					if (Output - _SIGINFO < 320) {
						...
						if (len <= (Lines * Col + 320)) { /*We're running out of room!*/
							...
							if (!(quiet)) parse_error("Increasing Buffer sizes\n");
...
#else // else don't have LIBUSDS
				if (Data_Only && Cmd != ALL) {
					memcpy(((unsigned char*)data) + Index,
					       ((unsigned char*)buf) + i - Output + _DATA, Output - _SIGINFO);
					Index += (Output - _SIGINFO);
					Lines++;
					BandCount[c_Band]++;
				} else {
		...
		if (Cmd == ALL) {
			...
			/*Pack Bands: this throws out empty bands*/
			{
				...
				int idx = 0;
				int i;
				// we don't always take all sequential bands, have to pack what we have
...

			{
				...
				for (i = 0; i < Lines; i++) {
					...
					if (c_frame >
					    (FrameBandStart[current_band] + band_frame_count)) { /*Missing frames*/
						if (!(quiet)) {
						...
						Add_Fake_Frame((((unsigned char*)newbuf) + Total_Frame_Count * Col),
						               (int)c_band, (FrameBandStart[current_band] + band_frame_count),
						               (c_frame - (FrameBandStart[current_band] + band_frame_count)),
						               Col, eflag, err);
						if (!(quiet)) {
		...
		if (Data_Only) {
			...
			for (i = 0; i < Total_Frame_Count; i++) {
				memcpy((((unsigned char*)do_buff) + (i * (Col - _SIGINFO))),
				       (((unsigned char*)newbuf) + (i * Col) + _DATA), (Col - _SIGINFO));
			}
			free(newbuf);
...
Var* ff_GSE_VIS_upshift(vfuncptr func, Var* arg)
{
	...
	if (V_FORMAT(obj) != DV_UINT8) {
		parse_error(
		    "This function upshifts DV_UINT8 values to DV_INT16 values\n"
...
unsigned char find_Value(unsigned short v)
{
	...
	if (v > 2048) return (255);

	if (i < 0) i = 0;

	while (i < 256) {
		if (value >= XformTable[i * 4 + 1] && value <= XformTable[i * 4 + 2])
...
Var* ff_GSE_VIS_downshift(vfuncptr func, Var* arg)
{
	...
	if (V_FORMAT(obj) != DV_INT16) {
		parse_error(
		    "This function downshifts DV_INT16 values to DV_UINT8 values\n"
		    "based on the MSSS Square-Root Encoding Table. Please only\n"
