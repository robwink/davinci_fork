...
static LINEBUF* storeline(char* text)
{
	...
	if (l == NULL) memory_error(errno, sizeof(LINEBUF));
	if (text != NULL) {
		l->line = (char*)malloc((unsigned int)(strlen(text) + 1));
		if (l->line == NULL) memory_error(errno, strlen(text)+1);
...
static LINKEY* storekey(char* key)
{
	...
	if (l == NULL) memory_error(errno, sizeof(LINKEY));
	l->key = (char*)malloc((unsigned int)(strlen(key) + 1));

	if (l->key == NULL) memory_error(errno, strlen(key)+1);
...
static void sortkeys()
{
	...
	if (keys == NULL) memory_error(errno, (keycount+1)*sizeof(KEY));
