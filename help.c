         ...
         )
{
	 ...
	 */
	errno = 0;
	if (strncmp(oldpath, path, PATHSIZE) != SAME) FreeHelp();

	if (keys == NULL) {
		status = LoadHelp(path);
		if (status == H_ERROR) return (status);
...
static int LoadHelp(char* path)
{
	...
	while (!feof(helpfp)) {
		...
		while (buf[0] == KEYFLAG) {
			...
			primary      = FALSE;
			pos          = ftell(helpfp);
			if (fgets(buf, BUFSIZ - 1, helpfp) == (char*)NULL) break;
		}
		/*
		...
		head      = firsthead;
		while ((fgets(buf, BUFSIZ - 1, helpfp) != (char*)NULL) && (buf[0] != KEYFLAG)) {
			/* save text line */
...
static LINEBUF* storeline(char* text)
{
	...
	if (l == NULL) parse_error("not enough memory to store help file");
	if (text != NULL) {
		l->line = (char*)malloc((unsigned int)(strlen(text) + 1));
		if (l->line == NULL) parse_error("not enough memory to store help file");
...
static LINKEY* storekey(char* key)
{
	...
	if (l->key == NULL) parse_error("not enough memory to store help file");
...
static void sortkeys()
{
	...
	if (keys == NULL) parse_error("not enough memory to store help file");
...
static boolean Ambiguous(KEY* key, int len)
{
	...
	for (prev = first = key->key, compare = 0, key++; key->key != NULL && compare == 0; key++) {
 ...
 */
static void PrintHelp(KEY* key, boolean* subtopics /* (in) - subtopics only? */
                                                   /* (out) - are there subtopics? */
                      )
{
	LINEBUF* t;
	...
	if (subtopics == NULL || !*subtopics) {
		for (t = key->text; t != NULL; t = t->next) OutLine(t->line); /* print text line */
	}
...
#define COLLENGTH 15
					...
					if (pos == 0) spacelen = FIRSTCOL;
					for (ispacelen = 0; ispacelen < spacelen; ispacelen++) (void)strcat(line, " ");
					/* commented out *
					   (void) strcat(line, "\t");
					 */
...
static void StartOutput()
{
	...
	if (pager_name != NULL && *pager_name != '\0')
		if ((outfile = popen(pager_name, "w")) != (FILE*)NULL) return; /* success */
	outfile = stderr;
...
static void EndOutput()
{
	if (outfile != stderr) (void)pclose(outfile);
}
	...
	case H_FOUND: {
		...
		do {
			if (subtopics && !only) {
				...
				if (more_help) /* base for next level is all of current helpbuf */
