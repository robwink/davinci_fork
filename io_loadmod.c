...
static int duplicate_filetype(char* check)
{
	...
	while (cur != NULL) {
		if (!strcmp(cur->filetype, check)) {
			parse_error("Duplicate type '%s' already handled by module %s. Skipping.", check,
			            cur->handler->modname);
			return 1;
		}
		cur = cur->next;
...
static void append_typelist(Typelist new[], IOmodPtr newmod)
{
	...
	while (1) {
		...
		if (duplicate_filetype(new[tlcount].filetype)) {
			tlcount++;
			continue;
		}
...
static void deactivate_module(IOmodPtr killme)
{
	...
	*/
	...
	if (killme->prev_list->next_list != NULL) killme->prev_list->next_list = killme->next_list;
...
static void destroy_io_module(IOmodPtr killme)
{
	...
	if (IOmodHeap != NULL) {
		...
		if (killme->next_heap != NULL) killme->next_heap->prev_heap = killme->prev_heap;
		if (killme == IOmodList->prev_heap) IOmodList->prev_heap    = killme->prev_heap;
	}
	if (IOmodList != NULL) {
		if (killme->prev_list != NULL)
			if (killme->prev_heap->next_heap != NULL)
				killme->prev_list->next_list                        = killme->next_list;
		if (killme->next_list != NULL) killme->next_list->prev_list = killme->prev_list;
		if (killme == IOmodList->prev_list) IOmodList->prev_list    = killme->prev_list;
	}
...
static char* get_path_for_module(char* modname)
{
	...
	while (curdir != NULL) {
		...
		if ((path_rtn = malloc(strlen(curdir) + strlen(modname) + strlen(IOMOD_EXT) + 2)) == NULL) {
...
Var* ff_lsmod(struct _vfuncptr* f, Var* args)
{
	...
	if (mname == NULL) {
		...
		printf("-----------              ---- ----- ------- ----\n");
		while (curmod != NULL) {
			if (curmod->implements & IO_MOD_READ)
				read = 'X';
			else
				read = ' ';
			if (curmod->implements & IO_MOD_WRITE)
				write = 'X';
			else
				write = ' ';
			if (curmod->implements & IO_MOD_LOAD_PDS)
				loadpds = 'X';
			else
				loadpds = ' ';
			printf("%-24.24s  %c     %c     %c    %s\n", curmod->modname, read, write, loadpds,
			       curmod->modpath);
...
Var* read_from_io_module(FILE* fh, char* fname)
{
	...
	*/
	...
	use_module = IOmodList;
	while (use_module != NULL) {
		if (use_module->implements & IO_MOD_READ) rtnval = use_module->read_func(fh, fname);
		if (rtnval != NULL) break;
...
Var* write_to_io_module(Var* dv_obj, char* filename, char* type, int force)
{
	...
	if ((force == 0) && (stat(filename, &stat_buf) == 0)) {
		parse_error("File '%s' already exists.  Use force option to overwrite.", filename);
		goto error_exit;
	}
