...
APIDEFS* api_lookup(char* aname)
{
	...
	for (lup = 0; lup < apicount; lup++) {
		if (!strcmp(apidefs[lup].apiname, aname)) return &apidefs[lup];
	}
	return NULL;
}

int find_argbin(int argc, APIARGS* args, char* argname)
{
	int lup;

	for (lup = 0; lup < argc; lup++) {
		if (!strcmp(argname, args[lup].argname)) return lup;
	}
	return -1;
}

int find_free_argbin(int argc, APIARGS* args)
{
	int lup;

	for (lup = 0; lup < argc; lup++) {
		if (args[lup].argusp == NULL) return lup;
	}
	return -1;
}
...
{
	switch (fmt) {
	case DV_UINT8: return ((int)((u_char*)v)[i]);
	case DV_INT16: return ((int)((short*)v)[i]);
	case DV_INT32: return ((int)((int*)v)[i]);
	case DV_FLOAT: return ((int)((float*)v)[i]);
	case DV_DOUBLE: return ((int)((double*)v)[i]);
	}
	return (0);
}
...
{
	switch (fmt) {
	case DV_UINT8: return ((float)((u_char*)v)[i]);
	case DV_INT16: return ((float)((short*)v)[i]);
	case DV_INT32: return ((float)((int*)v)[i]);
	case DV_FLOAT: return ((float)((float*)v)[i]);
	case DV_DOUBLE: return ((float)((double*)v)[i]);
	}
	return (0);
}
...
{
	switch (fmt) {
	case DV_UINT8: return (((u_char*)v)[i]);
	case DV_INT16: return (((short*)v)[i]);
	case DV_INT32: return (((int*)v)[i]);
	case DV_FLOAT: return (((float*)v)[i]);
	case DV_DOUBLE: return (((double*)v)[i]);
	}
	return (0);
}
...
Var* dispatch_api(APIDEFS* api, Var* garg)
{
	...
	for (lup = 0; lup < aargc; lup++) aargv[lup].argusp = NULL;
	...
	for (i = 0; i < givenac; i++) {
		...
		if (V_TYPE(ait) == ID_KEYWORD) {
			if ((abin = find_argbin(aargc, aargv, V_NAME(ait))) < 0) {
				printf("Unknown keyword to %s(... %s= ...)\n", api->apiname, V_NAME(ait));
				return NULL;
			} else {
				if (link_argvalue(V_KEYVAL(ait), &aargv[abin]) < 0) return NULL;
	...
	for (i = 0; i < givenac; i++) {
		...
		if (V_TYPE(ait) != ID_KEYWORD) {
			...
			if (link_argvalue(ait, &aargv[abin]) < 0) return NULL;
	...
	for (lup = 0; lup < aargc; lup++) {
		...
		vv    = aargv[lup].argusp;
		if ((dtype & CONSTBIT) == 0 && (dtype & PTRBIT) == PTRBIT && V_NAME(vv) != '\0') {
			vv = eval(vv);
			if (V_TYPE(vv) == ID_STRING) {
				if (V_STRING(vv) != NULL) free(V_STRING(vv));
				V_STRING(vv) = strdup(aargv[lup].argval);
			} else {
				typeconvert_args(dtype & DTMASK, aargv[lup].argval, V_FORMAT(vv), V_DATA(vv), V_DSIZE(vv));
			}
		}
